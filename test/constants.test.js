...
var packagejson_packagejsonjs = _interopRequireWildcard(_package);
...
describe('constants', function () {
  ...
  describe('number', function () {
    ...
    it('should have pi', function () {
      (0, _approx.equal)(math.pi, 3.14159265358979323846264338327950288419716939937510582097494459230781640628620899862803482534211706798214808651328230664);
      (0, _approx.equal)(math.sin(math.pi / 2), 1);
      (0, _approx.equal)(math.PI, math.pi);
    });

    it('should have tau', function () {
      (0, _approx.equal)(math.tau, 6.28318530717959);
    });

    it('should have phi, golden ratio', function () {
      (0, _approx.equal)(math.phi, 1.61803398874989484820458683436563811772030917980576286213545);
    });

    it('should have e (euler constant)', function () {
      (0, _approx.equal)(math.e, 2.71828182845905);
      _assert2.default.equal(math.round(math.add(1, math.pow(math.e, math.multiply(math.pi, math.i))), 5), 0);
      _assert2.default.equal(math.round(math.eval('1+e^(pi*i)'), 5), 0);
    });

    it('should have LN2', function () {
      (0, _approx.equal)(math.LN2, 0.69314718055994530941723212145817656807550013436025525412068000949339362196969471560586332699641868754200148102057068573);
    });

    it('should have LN10', function () {
      (0, _approx.equal)(math.LN10, 2.30258509299404568401799145468436420760110148862877297603332790096757260967735248023599720508959829834196778404228624863);
    });

    it('should have LOG2E', function () {
      (0, _approx.equal)(math.LOG2E, 1.44269504088896340735992468100189213742664595415298593413544940693110921918118507988552662289350634449699751830965254425);
    });

    it('should have LOG10E', function () {
      (0, _approx.equal)(math.LOG10E, 0.43429448190325182765112891891660508229439700580366656611445378316586464920887077472922494933843174831870610674476630373);
    });

    it('should have PI', function () {
      (0, _approx.equal)(math.PI, 3.14159265358979);
    });

    it('should have SQRT1_2', function () {
      (0, _approx.equal)(math.SQRT1_2, 0.70710678118654752440084436210484903928483593768847403658833986899536623923105351942519376716382078636750692311545614851);
    });

    it('should have SQRT2', function () {
      (0, _approx.equal)(math.SQRT2, 1.41421356237309504880168872420969807856967187537694807317667973799073247846210703885038753432764157273501384623091229702);
    });

    it('should have Infinity', function () {
      _assert2.default.strictEqual(math.Infinity, Infinity);
    });

    it('should have NaN', function () {
      _assert2.default.ok(isNaN(math.NaN));
    });
  });
  ...
  it('should have i', function () {
    _assert2.default.equal(math.i.re, 0);
    _assert2.default.equal(math.i.im, 1);
    _assert2.default.deepEqual(math.i, math.complex(0, 1));
    _assert2.default.deepEqual(math.sqrt(-1), math.i);
    _assert2.default.deepEqual(math.eval('i'), math.complex(0, 1));
  });

  it('should have true and false', function () {
    _assert2.default.strictEqual(math.true, true);
    _assert2.default.strictEqual(math.false, false);
    _assert2.default.strictEqual(math.eval('true'), true);
    _assert2.default.strictEqual(math.eval('false'), false);
  });

  it('should have null', function () {
    _assert2.default.strictEqual(math['null'], null);
  });

  it('should have version number', function () {
    _assert2.default.equal(math.version, {}.version);
  });
});

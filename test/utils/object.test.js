import assert from "assert";
import { clone as libutilsobject_clonejs } from "../../lib/utils/object";
// test object utils
var approx = require('../../tools/approx');

describe ('object', function () {

  describe('clone', function() {

    it('should clone undefined', function () {
      assert.strictEqual(libutilsobject_clonejs(undefined), undefined);
    });

    it('should clone null', function () {
      assert.strictEqual(libutilsobject_clonejs(null), null);
    });

    it('should clone booleans', function () {
      assert.strictEqual(libutilsobject_clonejs(true), true);
      assert.strictEqual(libutilsobject_clonejs(false), false);
      assert.ok(libutilsobject_clonejs(new Boolean(true)) instanceof Boolean);
      assert.equal(libutilsobject_clonejs(new Boolean(true)), true);
      assert.equal(libutilsobject_clonejs(new Boolean(false)), false);
    });

    it('should clone numbers', function () {
      assert.strictEqual(libutilsobject_clonejs(2.3), 2.3);
      assert.ok(libutilsobject_clonejs(new Number(2.3)) instanceof Number);
      assert.equal(libutilsobject_clonejs(new Number(2.3)), 2.3);
    });

    it('should clone strings', function () {
      assert.strictEqual(libutilsobject_clonejs('hello'), 'hello');
      assert.ok(libutilsobject_clonejs(new String('hello')) instanceof String);
      assert.equal(libutilsobject_clonejs(new String('hello')), 'hello');
    });

    it('should (deep) clone objects', function () {
      var obj = {a: {b: 'c', d: new Date(2014,0,1)}};
      var clone = libutilsobject_clonejs(obj);

      assert.deepEqual(obj, clone);

      // check whether the clone remains unchanged when changing the original object
      obj.a.b = 'cc';

      assert.equal(clone.a.b, 'c');

      obj.a.d.setMonth(2);
      assert.equal(clone.a.d.valueOf(), new Date(2014,0,1).valueOf());
    });

    it('should clone dates', function () {
      var d1 = new Date(2014,1,1);
      var d2 = libutilsobject_clonejs(d1);
      assert.equal(d1.valueOf(), d2.valueOf());
      d1.setMonth(2);
      assert.notEqual(d1, d2);
    });

    it('should (deep) clone arrays', function () {
      var d = new Date(2014,0,1);
      var arr = [1, 2, d, {a: 3}]
      var clone = libutilsobject_clonejs(arr);

      assert.deepEqual(arr, clone);
      assert.notStrictEqual(arr, clone);
      assert.notStrictEqual(arr[2], clone[2]);
      assert.notStrictEqual(arr[3], clone[3]);

      // check whether the clone remains unchanged when changing the original object
      arr[2] = null;
      arr[3].a = 1;
      d.setMonth(2);
      assert.equal(clone[2].valueOf(), new Date(2014,0,1).valueOf());
      assert.equal(clone[3].a, 3);
    });

    it('should throw an error in case of an unsupported type', function () {
      assert.throws(function () {libutilsobject_clonejs(/a regexp/)}, /Cannot clone/);
    });
  });

  describe('extend', function() {
    it ('should extend an object with all properties of an other object', function () {
      var e = {};
      var o1 = {a: 2, b: 3};
      var o2 = {a: 4, b: null, c: undefined, d: 5, e: e};
      var o3 = libutilsobject_clonejs(o1, o2);

      assert.strictEqual(o1, o3);
      assert.strictEqual(o1.e, o3.e);
      assert.deepEqual(o3, {a: 4, b: null, c: undefined, d: 5, e: e});
      assert.deepEqual(o2, {a: 4, b: null, c: undefined, d: 5, e: e}); // should be unchanged
    });

    it('should ignore inherited properties when extending an object', function () {
      Object.prototype.foo = 'bar';
      var o1 = {a: 2, b: 3};
      var o2 = libutilsobject_clonejs({}, o1);

      assert.equal(o2['foo'], 'bar');
      assert.equal(o2.hasOwnProperty('foo'), false);

      delete Object.prototype.foo;
    });
  });

  describe('deepExtend', function() {
    it ('should deep extend an object with all properties of an other object', function () {
      var e = {f: {g: 3}};
      var o1 = {a: 2, b: 3};
      var o2 = {a: 4, b: null, c: undefined, d: 5, e: e};
      var o3 = libutilsobject_clonejs(o1, o2);

      assert.strictEqual(o1, o3);
      assert.notStrictEqual(o3.e, o2.e);
      assert.deepEqual(o3, {a: 4, b: null, c: undefined, d: 5, e: {f: {g: 3}}});
      assert.deepEqual(o2, {a: 4, b: null, c: undefined, d: 5, e: {f: {g: 3}}}); // should be unchanged

      e.f.g = 4;
      assert.deepEqual(o3, {a: 4, b: null, c: undefined, d: 5, e: {f: {g: 3}}}); // should be unchanged
      assert.deepEqual(o2, {a: 4, b: null, c: undefined, d: 5, e: {f: {g: 4}}}); // should be changed
    });

    it ('should throw an error when deep extending an array (is not yet supported)', function () {
      assert.throws(function () {libutilsobject_clonejs({}, [])}, /Arrays are not supported by deepExtend/);
      assert.throws(function () {libutilsobject_clonejs({}, {a: []})}, /Arrays are not supported by deepExtend/);
      assert.throws(function () {libutilsobject_clonejs({}, {a: {b: []}})}, /Arrays are not supported by deepExtend/);
    });

    it('should ignore inherited properties when deep extending an object', function () {
      Object.prototype.foo = 'bar';
      var o1 = {a: 2, b: 3};
      var o2 = libutilsobject_clonejs({}, o1);

      assert.equal(o2['foo'], 'bar');
      assert.equal(o2.hasOwnProperty('foo'), false);

      delete Object.prototype.foo;
    });
  });

  describe('deepEqual', function() {

    it('should deep compare two objects', function () {
      assert.equal(libutilsobject_clonejs({}, {}), true);

      assert.equal(libutilsobject_clonejs({a: 2, b: 3}, {a: 2, b: 3}), true);
      assert.equal(libutilsobject_clonejs({a: 2, b: 3}, {a: 2, b: 4}), false);
      assert.equal(libutilsobject_clonejs({a: 2, b: 3}, {a: 2}), false);
      assert.equal(libutilsobject_clonejs({a: 2}, {a: 2, b: 3}), false);
      assert.equal(libutilsobject_clonejs({a: 2, b: 3}, {a: 2, b: {}}), false);
      assert.equal(libutilsobject_clonejs({a: 2, b: {}}, {a: 2, b: {}}), true);

      assert.equal(libutilsobject_clonejs({a: 2, b: {c: 4}}, {a: 2, b: {c: 4}}), true);
      assert.equal(libutilsobject_clonejs({a: 2, b: {c: 4}}, {a: 2, b: {c : 5}}), false);
      assert.equal(libutilsobject_clonejs({a: 2, b: {c: 4}}, {a: 2, b: {}}), false);
      assert.equal(libutilsobject_clonejs({a: 2, b: {}}, {a: 2, b: {c: 4}}), false);
    });

    it('should deep compare two arrays', function () {
      assert.equal(libutilsobject_clonejs([], []), true);
      assert.equal(libutilsobject_clonejs([1, 2], [1, 2]), true);
      assert.equal(libutilsobject_clonejs([1, 2], [1, 2, 3]), false);
      assert.equal(libutilsobject_clonejs([1, 0, 3], [1, 2, 3]), false);

      assert.equal(libutilsobject_clonejs([1, 2, [3, 4]], [1, 2, [3, 4]]), true);
      assert.equal(libutilsobject_clonejs([1, 2, [3]], [1, 2, [3, 4]]), false);
      assert.equal(libutilsobject_clonejs([1, 2, [3, 4]], [1, 2, [3]]), false);
      assert.equal(libutilsobject_clonejs([1, 2, null], [1, 2, [3]]), false);
      assert.equal(libutilsobject_clonejs([1, 2, [3]], [1, 2, null]), false);
      assert.equal(libutilsobject_clonejs([1, 2, 3], [1, 2, [3]]), false);
      assert.equal(libutilsobject_clonejs([1, 2, [3]], [1, 2, 3]), false);
    });

    it('should deep compare mixed objects an arrays', function () {
      assert.equal(libutilsobject_clonejs({}, []), false);
      assert.equal(libutilsobject_clonejs({a: {}}, {a: []}), false);

      assert.equal(libutilsobject_clonejs({a: [1,2,3]}, {a:[1,2,3]}), true);
      assert.equal(libutilsobject_clonejs({a: [1,2,{}]}, {a:[1,2,{}]}), true);
      assert.equal(libutilsobject_clonejs({a: [1,2,{b: 4}]}, {a:[1,2,{b: 4}]}), true);
      assert.equal(libutilsobject_clonejs({a: [1,2,{b: 4}]}, {a:[1,2,{b: 5}]}), false);
      assert.equal(libutilsobject_clonejs({a: [1,2,{b: 4}]}, {a:[1,2,{}]}), false);

      assert.equal(libutilsobject_clonejs([1, 2, {}], [1, 2, {}]), true);
      assert.equal(libutilsobject_clonejs([1, 2, {a: 3}], [1, 2, {a : 3}]), true);
      assert.equal(libutilsobject_clonejs([1, 2, {a: 3}], [1, 2, {a : 4}]), false);
      assert.equal(libutilsobject_clonejs([1, 2, {a: 3}], [1, 2, 3]), false);
      assert.equal(libutilsobject_clonejs([1, 2, 3], [1, 2, {a: 3}]), false);
      assert.equal(libutilsobject_clonejs([1, 2, {a: [3, 4]}], [1, 2, {a: [3, 4]}]), true);
      assert.equal(libutilsobject_clonejs([1, 2, {a: [3, 4]}], [1, 2, {a: [3, 4, 5]}]), false);
    });

    it('should not ignore inherited properties during comparison', function () {
      Object.prototype.foo = 'bar';

      assert.equal(libutilsobject_clonejs({}, {}), true);
      assert.equal(libutilsobject_clonejs({foo: 'bar'}, {}), true);

      delete Object.prototype.foo;
    });
  });

  describe('canDefineProperty', function() {

    it('should test whether defineProperty is available', function () {
      assert.equal(libutilsobject_clonejs(), true);
    });
  });


  describe('lazy', function() {

    it('should get a lazy property', function () {
      var obj = {};
      var count = 0;
      libutilsobject_clonejs(obj, 'x', function () {
        count++;
        return 2;
      });

      var x = obj.x;
      assert.equal(x, 2);
      assert.equal(count, 1);

      var x2 = obj.x;
      assert.equal(x2, 2);
      assert.equal(count, 1);
    });

    it('should set a lazy property', function () {
      var obj = {};
      libutilsobject_clonejs(obj, 'x', function () {
        return 2;
      });

      obj.x = 3;
      var x = obj.x;
      assert.equal(x, 3);
    });

  });

  describe('traverse', function() {

    it('should traverse an existing path into an object', function () {
      var a = {};
      var b = {a: a};
      var c = {b: b};

      assert.strictEqual(libutilsobject_clonejs(c), c);
      assert.strictEqual(libutilsobject_clonejs(c, ''), c);
      assert.strictEqual(libutilsobject_clonejs(c, 'b'), b);
      assert.strictEqual(libutilsobject_clonejs(c, 'b.a'), a);
    });

    it('should append missing piece of a path', function () {
      var a = {};
      var b = {a: a};
      var c = {b: b};

      assert.strictEqual(libutilsobject_clonejs(c), c);
      assert.strictEqual(libutilsobject_clonejs(c, ''), c);
      assert.strictEqual(libutilsobject_clonejs(c, 'b'), b);
      assert.strictEqual(libutilsobject_clonejs(c, 'b.a'), a);
      assert.strictEqual(libutilsobject_clonejs(c, 'b.d'), b.d);
      assert.strictEqual(libutilsobject_clonejs(c, 'b.e.f'), b.e.f);
    });

  });

  describe ('isFactory', function () {

    it('should test whether an object is a factory', function () {
      assert.equal(libutilsobject_clonejs({}), false);
      assert.equal(libutilsobject_clonejs({foo: true}), false);
      assert.equal(libutilsobject_clonejs({name: 'foo'}), false);
      assert.equal(libutilsobject_clonejs({name: 'foo', factory: 'bar'}), false);
      assert.equal(libutilsobject_clonejs({name: 2, factory: function () {}}), true);
      assert.equal(libutilsobject_clonejs({factory: function () {}}), true);

      assert.equal(libutilsobject_clonejs({name: 'foo', factory: function () {}}), true);
      assert.equal(libutilsobject_clonejs({name: 'foo', factory: function () {}, foo: 'bar'}), true);
    });

  })
});
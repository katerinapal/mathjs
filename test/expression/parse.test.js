...
var parse = index.expression.parse;
...
describe('parse', function() {
  ...
  it('should parse a single expression', function() {
    toolsapprox(parse('2 + 6 / 3').compile().eval(), 4);
  });
  ...
  it('should parse a matrix with expressions', function() {
    var scope = {};
    assert.deepEqual(parse(index.matrix(['a=3', 'b=4', 'a*b'])).map(function (node) {
      return node.compile().eval(scope);
    }), index.matrix([3, 4, 12]));
  });
  ...
  it('should parse an array with an empty expression', function() {
    assert.deepEqual(parse(index.matrix([''])).map(function (node) {
      return node.compile().eval();
    }), index.matrix([undefined]));
  });
  ...
  it('should parse unicode characters', function() {
    ...
    index.eval('\u00E9 = 2', scope); // Latin Small Letter E with Acute
    assert.strictEqual(scope['\u00E9'], 2);

    index.eval('\u03A6 = 3', scope); // Greek Capital Letter Phi
    assert.strictEqual(scope['\u03A6'], 3);

    index.eval('\u03A9 = 4', scope); // Greek Capital Letter Omega
    assert.strictEqual(scope['\u03A9'], 4);

    index.eval('k\u00F6ln = 5', scope); // Combination of latin and unicode
    assert.strictEqual(scope['k\u00F6ln'], 5);

    // test unicode characters in the astral plane (surrogate pairs
    index.eval('\uD835\uDD38 = 1', scope); // double struck capital A
  ...
  describe('multiline', function () {
    ...
    it('should spread a matrix over multiple lines', function() {
      assert.deepEqual(parse('[\n1\n,\n2\n]').compile().eval(), index.matrix([1, 2]));
    });

    it('should spread a range over multiple lines', function() {
      assert.deepEqual(parse('2:\n4').compile().eval(), index.matrix([2,3,4]));
      assert.deepEqual(parse('2:\n2:\n6').compile().eval(), index.matrix([2,4,6]));
    });
  ...
  it('should throw an error if called with a wrong type of argument', function() {
    ...
    assert.throws(function () {parse(index.unit('5cm'));}, TypeError);
  ...
  describe('number', function () {
    ...
    it('should parse a number followed by e', function() {
      toolsapprox(parseAndEval('2e'), 2 * Math.E);
    });
  ...
  describe('bignumber', function () {
    ...
    it('should parse bignumbers', function() {
      assert.deepEqual(parseAndEval('bignumber(0.1)'), index.bignumber(0.1));
      assert.deepEqual(parseAndEval('bignumber("1.2e500")'), index.bignumber('1.2e500'));
    });

    it('should output bignumbers if default number type is bignumber', function() {
      var bigmath = index.create({
        number: 'BigNumber'
      });
  ...
  describe('fraction', function () {
    ...
    it('should output fractions if default number type is fraction', function() {
      var fmath = index.create({
        number: 'Fraction'
      });

      assert(fmath.parse('0.1').compile().eval() instanceof index.type.Fraction);
  ...
  describe('unit', function () {
    ...
    it('should correctly parse negative temperatures', function () {
      toolsapprox.deepEqual(parseAndEval('-6 celsius'), new Unit(-6, 'celsius'));
      toolsapprox.deepEqual(parseAndEval('--6 celsius'), new Unit(6, 'celsius'));
      toolsapprox.deepEqual(parseAndEval('-6 celsius to fahrenheit'),
          new Unit(21.2, 'fahrenheit').to('fahrenheit'));
    });

    it('should convert units', function() {
      var scope = {};
      toolsapprox.deepEqual(parseAndEval('(5.08 cm * 1000) to inch', scope),
          index.unit(2000, 'inch').to('inch'));
      toolsapprox.deepEqual(parseAndEval('a = (5.08 cm * 1000) to mm', scope),
          index.unit(50800, 'mm').to('mm'));
      toolsapprox.deepEqual(parseAndEval('a to inch', scope),
          index.unit(2000, 'inch').to('inch'));

      toolsapprox.deepEqual(parseAndEval('10 celsius to fahrenheit'),
          index.unit(50, 'fahrenheit').to('fahrenheit'));
      toolsapprox.deepEqual(parseAndEval('20 celsius to fahrenheit'),
          index.unit(68, 'fahrenheit').to('fahrenheit'));
      toolsapprox.deepEqual(parseAndEval('50 fahrenheit to celsius'),
          index.unit(10, 'celsius').to('celsius'));
    });

    it('should evaluate operator "to" with correct precedence ', function () {
      toolsapprox.deepEqual(parseAndEval('5.08 cm * 1000 to inch'),
          new Unit(2000, 'inch').to('inch'));
    });

    it('should evaluate operator "in" (alias of "to") ', function () {
      toolsapprox.deepEqual(parseAndEval('5.08 cm in inch'),
          new Unit(2, 'inch').to('inch'));
    });

    it('should evaluate unit "in" (should not conflict with operator "in")', function () {
      toolsapprox.deepEqual(parseAndEval('2 in'),          new Unit(2, 'in'));
      toolsapprox.deepEqual(parseAndEval('5.08 cm in in'), new Unit(2, 'in').to('in'));
      toolsapprox.deepEqual(parseAndEval('5 in in in'),    new Unit(5, 'in').to('in'));
      toolsapprox.deepEqual(parseAndEval('2 in to meter'), new Unit(2, 'inch').to('meter'));
      toolsapprox.deepEqual(parseAndEval('2 in in meter'), new Unit(2, 'inch').to('meter'));
      toolsapprox.deepEqual(parseAndEval('a in inch', {a: new Unit(5.08, 'cm')}), new Unit(2, 'inch').to('inch'));
      toolsapprox.deepEqual(parseAndEval('(2+3) in'), new Unit(5, 'in'));
      toolsapprox.deepEqual(parseAndEval('a in', {a: 5}), new Unit(5, 'in'));
      toolsapprox.deepEqual(parseAndEval('0.5in + 1.5in to cm'), new Unit(5.08, 'cm').to('cm'));
    });
  });
  ...
  describe('matrix', function () {
    ...
    it('should parse a matrix', function() {
      ...
      assert.deepEqual(m, index.matrix([[1,2,3],[4,5,6]]));

      var b = parseAndEval('[5, 6; 1, 1]');
      assert.deepEqual(b.size(), [2,2]);
      assert.deepEqual(b, index.matrix([[5,6],[1,1]]));

      // from 1 to n dimensions
      assert.deepEqual(parseAndEval('[ ]'), index.matrix([]));
      assert.deepEqual(parseAndEval('[1,2,3]'), index.matrix([1,2,3]));
      assert.deepEqual(parseAndEval('[1;2;3]'), index.matrix([[1],[2],[3]]));
      assert.deepEqual(parseAndEval('[[1,2],[3,4]]'), index.matrix([[1,2],[3,4]]));
      assert.deepEqual(parseAndEval('[[[1],[2]],[[3],[4]]]'), index.matrix([[[1],[2]],[[3],[4]]]));
    });
    ...
    it('should get a matrix subset', function() {
      ...
      assert.deepEqual(parseAndEval('a[2, :]', scope),        index.matrix([[4,5,6]]));
      assert.deepEqual(parseAndEval('a[2, :2]', scope),       index.matrix([[4,5]]));
      assert.deepEqual(parseAndEval('a[2, :end-1]', scope),   index.matrix([[4,5]]));
      assert.deepEqual(parseAndEval('a[2, 2:]', scope),       index.matrix([[5,6]]));
      assert.deepEqual(parseAndEval('a[2, 2:3]', scope),      index.matrix([[5,6]]));
      assert.deepEqual(parseAndEval('a[2, 1:2:3]', scope),    index.matrix([[4,6]]));
      assert.deepEqual(parseAndEval('a[:, 2]', scope),        index.matrix([[2],[5],[8]]));
      assert.deepEqual(parseAndEval('a[:2, 2]', scope),       index.matrix([[2],[5]]));
      assert.deepEqual(parseAndEval('a[:end-1, 2]', scope),   index.matrix([[2],[5]]));
      assert.deepEqual(parseAndEval('a[2:, 2]', scope),       index.matrix([[5],[8]]));
      assert.deepEqual(parseAndEval('a[2:3, 2]', scope),      index.matrix([[5],[8]]));
      assert.deepEqual(parseAndEval('a[1:2:3, 2]', scope),    index.matrix([[2],[8]]));
    });
    ...
    it('should parse matrix resizings', function() {
      var scope = {};
      assert.deepEqual(parseAndEval('a = []', scope),    index.matrix([]));
      assert.deepEqual(parseAndEval('a[1:3,1] = [1;2;3]', scope), index.matrix([[1],[2],[3]]));
      assert.deepEqual(parseAndEval('a[:,2] = [4;5;6]', scope), index.matrix([[4],[5],[6]]));
      assert.deepEqual(scope.a, index.matrix([[1,4],[2,5],[3,6]]));

      assert.deepEqual(parseAndEval('a = []', scope),    index.matrix([]));
      assert.strictEqual(parseAndEval('a[1,3] = 3', scope), 3);
      assert.deepEqual(scope.a, index.matrix([[0,0,3]]));
      assert.deepEqual(parseAndEval('a[2,:] = [[4,5,6]]', scope), index.matrix([[4,5,6]]));
      assert.deepEqual(scope.a, index.matrix([[0,0,3],[4,5,6]]));

      assert.deepEqual(parseAndEval('a = []', scope),    index.matrix([]));
      assert.strictEqual(parseAndEval('a[3,1] = 3', scope), 3);
      assert.deepEqual(scope.a, index.matrix([[0],[0],[3]]));
      assert.deepEqual(parseAndEval('a[:,2] = [4;5;6]', scope), index.matrix([[4],[5],[6]]));
      assert.deepEqual(scope.a, index.matrix([[0,4],[0,5],[3,6]]));

      assert.deepEqual(parseAndEval('a = []', scope),    index.matrix([]));
      assert.deepEqual(parseAndEval('a[1,1:3] = [[1,2,3]]', scope), index.matrix([[1,2,3]]));
      assert.deepEqual(scope.a, index.matrix([[1,2,3]]));
      assert.deepEqual(parseAndEval('a[2,:] = [[4,5,6]]', scope), index.matrix([[4,5,6]]));
      assert.deepEqual(scope.a, index.matrix([[1,2,3],[4,5,6]]));
    });
    ...
    it('should get/set the matrix correctly', function() {
      ...
      assert.deepEqual(scope.a, index.matrix([[100,2],[3,4]]));
      parseAndEval('a[2:3,2:3] = [10,11;12,13]', scope);
      assert.deepEqual(scope.a.size(), [3,3]);
      assert.deepEqual(scope.a, index.matrix([[100, 2, 0],[3,10,11],[0,12,13]]));
      var a = scope.a;
      // note: after getting subset, uninitialized elements are replaced by elements with an undefined value
      assert.deepEqual(a.subset(index.index(new Range(0,3), new Range(0,2))), index.matrix([[100,2],[3,10],[0,12]]));
      assert.deepEqual(parseAndEval('a[1:3,1:2]', scope), index.matrix([[100,2],[3,10],[0,12]]));
    ...
    it('should get/set the matrix correctly for 3d matrices', function() {
      var scope = {};
      assert.deepEqual(parseAndEval('f=[1,2;3,4]', scope), index.matrix([[1,2],[3,4]]));
      assert.deepEqual(parseAndEval('size(f)', scope), index.matrix([2,2]));

      parseAndEval('f[:,:,2]=[5,6;7,8]', scope);
      assert.deepEqual(scope.f, index.matrix([
        [
          [1,5],
          [2,6]
        ],
      ...
      assert.deepEqual(parseAndEval('size(f)', scope), index.matrix([2,2,2]));
      assert.deepEqual(parseAndEval('f[:,:,1]', scope), index.matrix([[[1],[2]],[[3],[4]]]));
      assert.deepEqual(parseAndEval('f[:,:,2]', scope), index.matrix([[[5],[6]],[[7],[8]]]));
      assert.deepEqual(parseAndEval('f[:,2,:]', scope), index.matrix([[[2,6]],[[4,8]]]));
      assert.deepEqual(parseAndEval('f[2,:,:]', scope), index.matrix([[[3,7],[4,8]]]));

      parseAndEval('a=diag([1,2,3,4])', scope);
      assert.deepEqual(parseAndEval('a[3:end, 3:end]', scope), index.matrix([[3,0],[0,4]]));
      parseAndEval('a[3:end, 2:end]=9*ones(2,3)', scope);
      assert.deepEqual(scope.a, index.matrix([
        [1,0,0,0],
        [0,2,0,0],
        [0,9,9,9],
        [0,9,9,9]
      ]));
      assert.deepEqual(parseAndEval('a[2:end-1, 2:end-1]', scope), index.matrix([[2,0],[9,9]]));
    });
    ...
    it('should parse matrix concatenations', function() {
      ...
      assert.deepEqual(parseAndEval('c=concat(a,b)', scope), index.matrix([[1,2,5,6],[3,4,7,8]]));
      assert.deepEqual(parseAndEval('c=concat(a,b,1)', scope), index.matrix([[1,2],[3,4],[5,6],[7,8]]));
      assert.deepEqual(parseAndEval('c=concat(concat(a,b), concat(b,a), 1)', scope), index.matrix([[1,2,5,6],[3,4,7,8],[5,6,1,2],[7,8,3,4]]));
      assert.deepEqual(parseAndEval('c=concat([[1,2]], [[3,4]], 1)', scope), index.matrix([[1,2],[3,4]]));
      assert.deepEqual(parseAndEval('c=concat([[1,2]], [[3,4]], 2)', scope), index.matrix([[1,2,3,4]]));
      assert.deepEqual(parseAndEval('c=concat([[1]], [2;3], 1)', scope), index.matrix([[1],[2],[3]]));
      assert.deepEqual(parseAndEval('d=1:3', scope), index.matrix([1,2,3]));
      assert.deepEqual(parseAndEval('concat(d,d)', scope), index.matrix([1,2,3,1,2,3]));
      assert.deepEqual(parseAndEval('e=1+d', scope), index.matrix([2,3,4]));
      assert.deepEqual(parseAndEval('size(e)', scope), index.matrix([3]));
      assert.deepEqual(parseAndEval('concat(e,e)', scope), index.matrix([2,3,4,2,3,4]));
      assert.deepEqual(parseAndEval('[[],[]]', scope), index.matrix([[],[]]));
      assert.deepEqual(parseAndEval('[[],[]]', scope).size(), [2, 0]);
      assert.deepEqual(parseAndEval('size([[],[]])', scope), index.matrix([2, 0]));
    });
    ...
    it('should execute map on an array with one based indices', function () {
      ...
      var res = index.eval('map(A, callback)', scope);
    ...
    it('should execute map on a Matrix with one based indices', function () {
      ...
      var res = index.eval('map(A, callback)', scope);
      assert.deepEqual(res, index.matrix([2,3,4]));
    ...
    it('should execute forEach on an array with one based indices', function () {
      ...
      index.eval('forEach(A, callback)', scope);
  ...
  describe('objects', function () {
    ...
    it('should invoke a function on an object with the right context', function () {
      toolsapprox(parseAndEval('(2.54 cm).toNumeric("inch")'), 1);
      assert.deepEqual(parseAndEval('bignumber(2).plus(3)'), index.bignumber(5));
      assert.deepEqual(parseAndEval('bignumber(2)["plus"](3)'), index.bignumber(5));
    });
  ...
  describe('constants', function () {
    ...
    it('should parse constants', function() {
      assert.deepEqual(parseAndEval('i'), new Complex(0, 1));
      toolsapprox(parseAndEval('pi'), Math.PI);
      toolsapprox(parseAndEval('e'), Math.E);
    });
  ...
  describe('variables', function () {
    ...
    it('should parse valid variable assignments', function() {
      ...
      toolsapprox(parseAndEval('pi * 2', scope), 6.283185307179586);
    });
    ...
    it('should parse nested assignments', function() {
      ...
      assert.deepEqual(parseAndEval('a = [1,2,f=3]', scope), index.matrix([1,2,3]));
  ...
  describe('functions', function () {
    ...
    it('should parse functions', function() {
      ...
      toolsapprox.deepEqual(parseAndEval('1+exp(pi*i)'), new Complex(0, 0));
  ...
  describe ('parentheses', function () {
    it('should parse parentheses overriding the default precedence', function () {
      toolsapprox(parseAndEval('2 - (2 - 2)'), 2);
      toolsapprox(parseAndEval('2 - ((2 - 2) - 2)'), 4);
      toolsapprox(parseAndEval('3 * (2 + 3)'), 15);
      toolsapprox(parseAndEval('(2 + 3) * 3'), 15);
    });
  ...
  describe ('operators', function () {
    ...
    it('should parse operations', function() {
      toolsapprox(parseAndEval('(2+3)/4'), 1.25);
      toolsapprox(parseAndEval('2+3/4'), 2.75);
    ...
    it('should parse dotDivide ./', function() {
      assert.equal(parseAndEval('4./2'), 2);
      assert.deepEqual(parseAndEval('4./[2,4]'), index.matrix([2,1]));
      assert.equal(parseAndEval('4 ./ 2'), 2);
      assert.equal(parseAndEval('8 ./ 2 / 2'), 2);

      assert.deepEqual(parseAndEval('[1,2,3] ./ [1,2,3]'), index.matrix([1,1,1]));
    });

    it('should parse dotMultiply .*', function() {
      toolsapprox.deepEqual(parseAndEval('2.*3'), 6);
      toolsapprox.deepEqual(parseAndEval('2e3.*3'), 6e3);
      toolsapprox.deepEqual(parseAndEval('2 .* 3'), 6);
      toolsapprox.deepEqual(parseAndEval('4 .* 2'), 8);
      toolsapprox.deepEqual(parseAndEval('8 .* 2 .* 2'), 32);
      assert.deepEqual(parseAndEval('a=3; a.*4'), new ResultSet([12]));

      assert.deepEqual(parseAndEval('[1,2,3] .* [1,2,3]'), index.matrix([1,4,9]));
    });

    it('should parse dotPower .^', function() {
      toolsapprox.deepEqual(parseAndEval('2.^3'), 8);
      toolsapprox.deepEqual(parseAndEval('2 .^ 3'), 8);
      toolsapprox.deepEqual(parseAndEval('-2.^2'), -4);  // -(2^2)
      toolsapprox.deepEqual(parseAndEval('2.^3.^4'), 2.41785163922926e+24); // 2^(3^4)

      assert.deepEqual(parseAndEval('[2,3] .^ [2,3]'), index.matrix([4,27]));
    });

    it('should parse equal ==', function() {
      assert.strictEqual(parseAndEval('2 == 3'), false);
      assert.strictEqual(parseAndEval('2 == 2'), true);
      assert.deepEqual(parseAndEval('[2,3] == [2,4]'), index.matrix([true, false]));
    });
    ...
    it('should parse mod %', function() {
      toolsapprox(parseAndEval('8 % 3'), 2);
    });

    it('should parse operator mod', function() {
      toolsapprox(parseAndEval('8 mod 3'), 2);
    });

    it('should parse multiply *', function() {
      toolsapprox(parseAndEval('4 * 2'), 8);
      toolsapprox(parseAndEval('8 * 2 * 2'), 32);
    });
    ...
    it('should parse implicit multiplication', function() {
      ...
      assert.deepEqual(parseAndEval('[2, 3] 2'), index.matrix([4, 6]));
      assert.deepEqual(parseAndEval('[2, 3] a', {a:2}), index.matrix([4, 6]));
    ...
    it('should throw an error when having an implicit multiplication between two numbers', function() {
      assert.throws(function () { index.parse('2 3'); }, /Unexpected part "3"/);
      assert.throws(function () { index.parse('2 * 3 4'); }, /Unexpected part "4"/);
      assert.throws(function () { index.parse('2 * 3 4 * 5'); }, /Unexpected part "4"/);
      assert.throws(function () { index.parse('2 / 3 4 5'); }, /Unexpected part "4"/);
      assert.throws(function () { index.parse('2 + 3 4'); }, /Unexpected part "4"/);
    });

    it('should parse pow ^', function() {
      toolsapprox(parseAndEval('2^3'), 8);
      toolsapprox(parseAndEval('-2^2'), -4);  // -(2^2)
      toolsapprox(parseAndEval('2^3^4'), 2.41785163922926e+24); // 2^(3^4)
    });
    ...
    it('should parse unequal !=', function() {
      assert.strictEqual(parseAndEval('2 != 3'), true);
      assert.strictEqual(parseAndEval('2 != 2'), false);
      assert.deepEqual(parseAndEval('[2,3] != [2,4]'), index.matrix([false, true]));
    });
    ...
    it('should lazily evaluate conditional expression a ? b : c', function() {
      var scope = {};
      index.parse('true ? (a = 2) : (b = 2)').compile().eval(scope);
    ...
    it('should parse : (range)', function() {
      ...
      assert.deepEqual(parseAndEval('2:5'), index.matrix([2,3,4,5]));
      assert.deepEqual(parseAndEval('10:-2:0'), index.matrix([10,8,6,4,2,0]));
      assert.deepEqual(parseAndEval('2:4.0'), index.matrix([2,3,4]));
      assert.deepEqual(parseAndEval('2:4.5'), index.matrix([2,3,4]));
      assert.deepEqual(parseAndEval('2:4.1'), index.matrix([2,3,4]));
      assert.deepEqual(parseAndEval('2:3.9'), index.matrix([2,3]));
      assert.deepEqual(parseAndEval('2:3.5'), index.matrix([2,3]));
      assert.deepEqual(parseAndEval('3:-1:0.5'), index.matrix([3,2,1]));
      assert.deepEqual(parseAndEval('3:-1:0.5'), index.matrix([3,2,1]));
      assert.deepEqual(parseAndEval('3:-1:0.1'), index.matrix([3,2,1]));
      assert.deepEqual(parseAndEval('3:-1:-0.1'), index.matrix([3,2,1,0]));
    });

    it('should parse to', function() {
      toolsapprox.deepEqual(parseAndEval('2.54 cm to inch'), index.unit(1, 'inch').to('inch'));
      toolsapprox.deepEqual(parseAndEval('2.54 cm + 2 inch to foot'), index.unit(0.25, 'foot').to('foot'));
    });

    it('should parse in', function() {
      toolsapprox.deepEqual(parseAndEval('2.54 cm in inch'), index.unit(1, 'inch').to('inch'));
    });

    it('should parse factorial !', function() {
      assert.deepEqual(parseAndEval('5!'), 120);
      assert.deepEqual(parseAndEval('[1,2,3,4]!'), index.matrix([1,2,6,24]));
      ...
      assert.deepEqual(parseAndEval('[1,2;3,1]!\'!'), index.matrix([[1, 720], [2, 1]]));
      assert.deepEqual(parseAndEval('[4,5]![2]'), 120); // index [2]
    });

    it('should parse transpose \'', function() {
      assert.deepEqual(parseAndEval('23\''), 23);
      assert.deepEqual(parseAndEval('[1,2,3;4,5,6]\''), index.matrix([[1,4],[2,5],[3,6]]));
      assert.ok(parseAndEval('[1,2,3;4,5,6]\'') instanceof Matrix);
      assert.deepEqual(parseAndEval('[1:5]'), index.matrix([[1,2,3,4,5]]));
      assert.deepEqual(parseAndEval('[1:5]\''), index.matrix([[1],[2],[3],[4],[5]]));
      assert.deepEqual(parseAndEval('size([1:5])'), index.matrix([1, 5]));
      assert.deepEqual(parseAndEval('[1,2;3,4]\''), index.matrix([[1,3],[2,4]]));
    });
    ...
    describe('operator precedence', function() {
      ...
      it('should respect precedence of conditional operator and other operators', function () {
        ...
        assert.deepEqual(parseAndEval('2 == 4-2 ? [1,2] : false'), index.matrix([1,2]));
        assert.deepEqual(parseAndEval('false ? 1:2:6'), index.matrix([2,3,4,5,6]));
      });
      ...
      it('should respect precedence of conditional operator and logical or', function () {
        var node = index.parse('1 or 0 ? 2 or 3 : 0 or 0');
      ...
      it('should respect precedence of conditional operator and relational operators', function () {
        var node = index.parse('a == b ? a > b : a < b');
        assert(node instanceof ConditionalNode);
      ...
      it('should respect precedence of conditional operator and range operator (2)', function () {
        var node = index.parse('a ? (b : c) : (d : e)');
        assert(node instanceof ConditionalNode);
        assert.equal(node.condition.toString(), 'a');
      ...
      it('should respect precedence of conditional operator and range operator (2)', function () {
        var node = index.parse('a ? (b ? c : d) : (e ? f : g)');
        assert(node instanceof ConditionalNode);
        assert.equal(node.condition.toString(), 'a');
  ...
  describe('functions', function () {
    it('should evaluate function "mod"', function () {
      toolsapprox(parseAndEval('mod(8, 3)'), 2);

    });

    it('should evaluate function "to" ', function () {
      toolsapprox.deepEqual(parseAndEval('to(5.08 cm * 1000, inch)'),
          index.unit(2000, 'inch').to('inch'));
    });
  ...
  describe('bignumber', function () {
    ...
    it('should work with mixed numbers and bignumbers', function() {
      toolsapprox(bigmath.eval('pi + 1'), 4.141592653589793);
    });

    it('should evaluate functions not supporting bignumbers', function() {
      toolsapprox(bigmath.eval('sin(0.1)'), 0.09983341664682815);
    });
  ...
  describe('scope', function () {
    ...
    it('should parse undefined symbols, defining symbols, and removing symbols', function() {
      ...
      assert.deepEqual(scope.qq, index.matrix([[33,2],[3,4]]));
      parse('qq=[4]').compile().eval(scope);
      n.compile().eval(scope);
      assert.deepEqual(scope.qq, index.matrix([[33]]));
      delete scope.qq;
      assert.throws(function () { n.compile().eval(scope); });
    });
  ...
  describe('errors', function () {
    ...
    it('should return IndexErrors with one based indices', function () {
      ...
      assert.throws(function () {index.subset([1,2,3], index.index(4));}, /Index out of range \(4 > 2\)/);
      assert.throws(function () {index.subset([1,2,3], index.index(-2));}, /Index out of range \(-2 < 0\)/);

      // evaluation via parser throws one-based error
      assert.throws(function () {index.eval('A[4]', {A:[1,2,3]});}, /Index out of range \(4 > 3\)/);
      assert.throws(function () {index.eval('A[-2]', {A: [1,2,3]});}, /IndexError: Index out of range \(-2 < 1\)/);
    });
    ...
    it('should return DimensionErrors with one based indices (subset)', function () {
      ...
      // functions throw a zero-based error
      assert.throws(function () {index.subset([1,2,3], index.index(1,1));}, /DimensionError: Dimension mismatch \(2 != 1\)/);

      // evaluation via parser throws one-based error
      assert.throws(function () {index.eval('A[1,1]', {A: [1,2,3]});}, /DimensionError: Dimension mismatch \(2 != 1\)/);
    });
    ...
    it('should return DimensionErrors with one based indices (concat)', function () {
      ...
      // functions throw a zero-based error
      assert.throws(function () {index.concat([1,2], [[3,4]]);}, /DimensionError: Dimension mismatch \(1 != 2\)/);
      assert.throws(function () {index.concat([[1,2]], [[3,4]], 2);}, /IndexError: Index out of range \(2 > 1\)/);
      assert.throws(function () {index.concat([[1,2]], [[3,4]], -1);}, /IndexError: Index out of range \(-1 < 0\)/);

      // evaluation via parser throws one-based error
      assert.throws(function () {index.eval('concat([1,2], [[3,4]])');}, /DimensionError: Dimension mismatch \(1 != 2\)/);
      assert.throws(function () {index.eval('concat([[1,2]], [[3,4]], 3)');}, /IndexError: Index out of range \(3 > 2\)/);
      assert.throws(function () {index.eval('concat([[1,2]], [[3,4]], 0)');}, /IndexError: Index out of range \(0 < 1\)/);
    });
    ...
    it('should return DimensionErrors with one based indices (max)', function () {
      ...
      // evaluation via parser throws one-based error
      assert.deepEqual(index.eval('max([[1,2], [3,4]])'), 4);
      assert.deepEqual(index.eval('max([[1,2], [3,4]], 1)'), index.matrix([3, 4]));
      assert.deepEqual(index.eval('max([[1,2], [3,4]], 2)'), index.matrix([2, 4]));
      assert.throws(function () {index.eval('max([[1,2], [3,4]], 3)');}, /IndexError: Index out of range \(3 > 2\)/);
      assert.throws(function () {index.eval('max([[1,2], [3,4]], 0)');}, /IndexError: Index out of range \(0 < 1\)/);
    });
    ...
    it('should return DimensionErrors with one based indices (min)', function () {
      ...
      // evaluation via parser throws one-based error
      assert.deepEqual(index.eval('min([[1,2], [3,4]])'), 1);
      assert.deepEqual(index.eval('min([[1,2], [3,4]], 1)'), index.matrix([1, 2]));
      assert.deepEqual(index.eval('min([[1,2], [3,4]], 2)'), index.matrix([1, 3]));
      assert.throws(function () {index.eval('min([[1,2], [3,4]], 3)');}, /IndexError: Index out of range \(3 > 2\)/);
      assert.throws(function () {index.eval('min([[1,2], [3,4]], 0)');}, /IndexError: Index out of range \(0 < 1\)/);
    });
    ...
    it('should return DimensionErrors with one based indices (mean)', function () {
      ...
      // evaluation via parser throws one-based error
      assert.deepEqual(index.eval('mean([[1,2], [3,4]])'), 2.5);
      assert.deepEqual(index.eval('mean([[1,2], [3,4]], 1)'), index.matrix([2, 3]));
      assert.deepEqual(index.eval('mean([[1,2], [3,4]], 2)'), index.matrix([1.5, 3.5]));
      assert.throws(function () {index.eval('mean([[1,2], [3,4]], 3)');}, /IndexError: Index out of range \(3 > 2\)/);
      assert.throws(function () {index.eval('mean([[1,2], [3,4]], 0)');}, /IndexError: Index out of range \(0 < 1\)/);
    });
  ...
  describe('node tree', function () {
    ...
    describe('custom nodes', function () {
      ...
      CustomNode.prototype = new index.expression.node.Node();

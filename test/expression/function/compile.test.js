...
describe('compile', function() {
  ...
  it('should parse multiple expressions', function() {
    var codes = index.compile(['2+3', '4+5']);
  ...
  it('should throw an error on wrong number of arguments', function() {
    assert.throws(function () {index.compile()}, /TypeError: Too few arguments/);
    assert.throws(function () {index.compile('2+3', '3+4')}, /TypeError: Too many arguments/);
  });

  it('should throw an error on wrong type of argument', function() {
    assert.throws(function () {index.compile(index.complex(2, 3))}, TypeError);
  });

  it('should LaTeX compile', function () {
    var expression = index.parse('compile(1)');

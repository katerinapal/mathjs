"use strict";

var _assert = require("assert");

var _assert2 = _interopRequireDefault(_assert);

var _index = require("../../index");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var Help = _index.indexjs.type.Help;

describe('help', function () {
  var doc = {
    'name': 'add',
    'category': 'Operators',
    'syntax': ['x + y', 'add(x, y)'],
    'description': 'Add two values.',
    'examples': ['a = 2.1 + 3.6', 'a - 3.6'],
    'seealso': ['subtract']
  };

  it('should generate the help for a function', function () {
    var help = new Help(doc);

    (0, _assert2.default)(help instanceof Help);
    _assert2.default.deepEqual(help.doc.name, 'add');
    _assert2.default.deepEqual(help.doc, doc);
  });

  it('should throw an error when constructed without new operator', function () {
    _assert2.default.throws(function () {
      Help(_index.indexjs.expression.docs.sin);
    }, /Constructor must be called with the new operator/);
  });

  it('should throw an error when constructed without doc argument', function () {
    _assert2.default.throws(function () {
      new Help();
    }, /Argument "doc" missing/);
  });

  it('should have a property isHelp', function () {
    var a = new Help(doc);
    _assert2.default.strictEqual(a.isHelp, true);
  });

  it('should have a property type', function () {
    var a = new Help(doc);
    _assert2.default.strictEqual(a.type, 'Help');
  });

  it('should stringify a help', function () {
    var help = new Help(doc);
    _assert2.default.equal(help.toString(), '\nName: add\n' + '\n' + 'Category: Operators\n' + '\n' + 'Description:\n' + '    Add two values.\n' + '\n' + 'Syntax:\n' + '    x + y\n' + '    add(x, y)\n' + '\n' + 'Examples:\n' + '    a = 2.1 + 3.6\n' + '        5.7\n' + '    a - 3.6\n' + '        2.1\n' + '\n' + 'See also: subtract\n');
  });

  it('should stringify a help with empty doc', function () {
    var help = new Help({});
    _assert2.default.equal(help.toString(), '\n');
  });

  it('should stringify a doc with empty example', function () {
    var help = new Help({
      'name': 'add',
      'examples': ['2 + 3', '']
    });

    _assert2.default.equal(help.toString(), '\nName: add\n' + '\n' + 'Examples:\n' + '    2 + 3\n' + '        5\n' + '    \n' + '\n');
  });

  it('should stringify a doc with example throwing an error', function () {
    var help = new Help({
      'name': 'add',
      'examples': ['2 ^^ 3']
    });

    _assert2.default.equal(help.toString(), '\nName: add\n' + '\n' + 'Examples:\n' + '    2 ^^ 3\n' + '        SyntaxError: Value expected (char 4)\n' + '\n');
  });

  it('should return string representation on valueOf', function () {
    var help = new Help({
      'name': 'add',
      'examples': ['2 ^^ 3']
    });

    _assert2.default.strictEqual(help.valueOf(), '\nName: add\n' + '\n' + 'Examples:\n' + '    2 ^^ 3\n' + '        SyntaxError: Value expected (char 4)\n' + '\n');
  });

  it('should export doc to JSON', function () {
    var help = new Help(doc);
    var json = help.toJSON();
    _assert2.default.deepEqual(json, {
      'mathjs': 'Help',
      'name': 'add',
      'category': 'Operators',
      'syntax': ['x + y', 'add(x, y)'],
      'description': 'Add two values.',
      'examples': ['a = 2.1 + 3.6', 'a - 3.6'],
      'seealso': ['subtract']
    });
    json.name = 'foo'; // this should not alter the original doc
    json.examples.push('2 + 3'); // this should not alter the original doc
    _assert2.default.equal(doc.name, 'add');
    _assert2.default.notEqual(json.examples.length, doc.examples.length);
  });

  it('should instantiate Help from json using fromJSON', function () {
    var doc = {
      'name': 'add',
      'category': 'Operators',
      'syntax': ['x + y', 'add(x, y)'],
      'description': 'Add two values.',
      'examples': ['a = 2.1 + 3.6', 'a - 3.6'],
      'seealso': ['subtract']
    };

    var json = Object.create(doc);
    json['mathjs'] = 'Help';

    var help = Help.fromJSON(json);
    (0, _assert2.default)(help instanceof Help);
    _assert2.default.deepEqual(doc, help.doc);
  });
});

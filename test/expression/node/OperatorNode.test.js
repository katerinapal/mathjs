...
describe('OperatorNode', function() {
  ...
  describe('toString', function () {
    ...
    it ('should stringify left associative OperatorNodes that are associative with another Node', function () {
      assert.equal(index_obj.parse('(a+b)+c').toString({parenthesis: 'auto'}), 'a + b + c');
      assert.equal(index_obj.parse('a+(b+c)').toString({parenthesis: 'auto'}), 'a + b + c');
      assert.equal(index_obj.parse('(a+b)-c').toString({parenthesis: 'auto'}), 'a + b - c');
      assert.equal(index_obj.parse('a+(b-c)').toString({parenthesis: 'auto'}), 'a + b - c');
    ...
    it ('should stringify unary OperatorNodes containing a binary OperatorNode', function () {
      assert.equal(index_obj.parse('(a*b)!').toString(), '(a * b)!');
      assert.equal(index_obj.parse('-(a*b)').toString(), '-(a * b)');
      assert.equal(index_obj.parse('-(a+b)').toString(), '-(a + b)');
    });

    it ('should stringify unary OperatorNodes containing a unary OperatorNode', function () {
      assert.equal(index_obj.parse('(-a)!').toString({parenthesis: 'auto'}), '(-a)!');
      assert.equal(index_obj.parse('-(a!)').toString({parenthesis: 'auto'}), '-a!');
      assert.equal(index_obj.parse('-(-a)').toString({parenthesis: 'auto'}), '-(-a)');
    });
  });
  ...
  it ('should respect the \'all\' parenthesis option', function () {
    assert.equal(index_obj.parse('1+1+1').toString({parenthesis: 'all'}), '(1 + 1) + 1' );
    assert.equal(index_obj.parse('1+1+1').toTex({parenthesis: 'all'}), '\\left(1+1\\right)+1' );
  });

  it ('should correctly LaTeX fractions in \'all\' parenthesis mode', function () {
    assert.equal(index_obj.parse('1/2/3').toTex({parenthesis: 'all'}), '\\frac{\\left(\\frac{1}{2}\\right)}{3}');
  });
  ...
  it ('should LaTeX simple expressions in \'auto\' mode', function () {
    ...
    assert.equal(index_obj.parse('1+(1+1)').toTex({parenthesis: 'auto'}), '1+1+1');
  });

  it ('should stringify implicit multiplications', function () {
    var a = index_obj.parse('4a');
    var b = index_obj.parse('4 a');
    var c = index_obj.parse('a b');
    var d = index_obj.parse('2a b');
    var e = index_obj.parse('a b c');
    var f = index_obj.parse('(2+3)a');
    var g = index_obj.parse('(2+3)2');
    var h = index_obj.parse('2(3+4)');

...
describe('OperatorNode', function () {
  ...
  describe('toString', function () {
    ...
    it('should stringify left associative OperatorNodes that are associative with another Node', function () {
      _assert2.default.equal(_index.indexjs.parse('(a+b)+c').toString({ parenthesis: 'auto' }), 'a + b + c');
      _assert2.default.equal(_index.indexjs.parse('a+(b+c)').toString({ parenthesis: 'auto' }), 'a + b + c');
      _assert2.default.equal(_index.indexjs.parse('(a+b)-c').toString({ parenthesis: 'auto' }), 'a + b - c');
      _assert2.default.equal(_index.indexjs.parse('a+(b-c)').toString({ parenthesis: 'auto' }), 'a + b - c');
    ...
    it('should stringify unary OperatorNodes containing a binary OperatorNode', function () {
      _assert2.default.equal(_index.indexjs.parse('(a*b)!').toString(), '(a * b)!');
      _assert2.default.equal(_index.indexjs.parse('-(a*b)').toString(), '-(a * b)');
      _assert2.default.equal(_index.indexjs.parse('-(a+b)').toString(), '-(a + b)');
    });

    it('should stringify unary OperatorNodes containing a unary OperatorNode', function () {
      _assert2.default.equal(_index.indexjs.parse('(-a)!').toString({ parenthesis: 'auto' }), '(-a)!');
      _assert2.default.equal(_index.indexjs.parse('-(a!)').toString({ parenthesis: 'auto' }), '-a!');
      _assert2.default.equal(_index.indexjs.parse('-(-a)').toString({ parenthesis: 'auto' }), '-(-a)');
    });
  });
  ...
  it('should respect the \'all\' parenthesis option', function () {
    _assert2.default.equal(_index.indexjs.parse('1+1+1').toString({ parenthesis: 'all' }), '(1 + 1) + 1');
    _assert2.default.equal(_index.indexjs.parse('1+1+1').toTex({ parenthesis: 'all' }), '\\left(1+1\\right)+1');
  });

  it('should correctly LaTeX fractions in \'all\' parenthesis mode', function () {
    _assert2.default.equal(_index.indexjs.parse('1/2/3').toTex({ parenthesis: 'all' }), '\\frac{\\left(\\frac{1}{2}\\right)}{3}');
  });
  ...
  it('should LaTeX simple expressions in \'auto\' mode', function () {
    ...
    _assert2.default.equal(_index.indexjs.parse('1+(1+1)').toTex({ parenthesis: 'auto' }), '1+1+1');
  });

  it('should stringify implicit multiplications', function () {
    var a = _index.indexjs.parse('4a');
    var b = _index.indexjs.parse('4 a');
    var c = _index.indexjs.parse('a b');
    var d = _index.indexjs.parse('2a b');
    var e = _index.indexjs.parse('a b c');
    var f = _index.indexjs.parse('(2+3)a');
    var g = _index.indexjs.parse('(2+3)2');
    var h = _index.indexjs.parse('2(3+4)');

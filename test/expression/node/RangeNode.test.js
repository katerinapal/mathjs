...
describe('RangeNode', function() {
  ...
  it ('should compile a RangeNode', function () {
    ...
    var expr = n.compile();
    assert.deepEqual(expr.eval(), index_obj.matrix([0, 2, 4, 6, 8, 10]));
  });
  ...
  it ('should respect the \'all\' parenthesis option', function () {
    assert.equal(index_obj.parse('1:2:3').toString({parenthesis: 'all'}), '(1):(2):(3)');
    assert.equal(index_obj.parse('1:2:3').toTex({parenthesis: 'all'}), '\\left(1\\right):\\left(2\\right):\\left(3\\right)');
  });

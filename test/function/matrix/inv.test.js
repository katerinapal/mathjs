...
// test inv
var error = {},
    inv = _index.indexjs.inv;

describe('inv', function () {

  it('should return the inverse of a number', function () {
    _assert2.default.deepEqual(inv(4), 1 / 4);
    _assert2.default.deepEqual(inv(_index.indexjs.bignumber(4)), _index.indexjs.bignumber(1 / 4));
  });
  ...
  it('should return the inverse for each element in a matrix', function () {
    _assert2.default.deepEqual(inv(_index.indexjs.matrix([4])), _index.indexjs.matrix([1 / 4]));
    _assert2.default.deepEqual(inv(_index.indexjs.matrix([[4]])), _index.indexjs.matrix([[1 / 4]]));
    _assert2.default.deepEqual(inv(_index.indexjs.matrix([[4]], 'sparse')), _index.indexjs.matrix([[1 / 4]], 'sparse'));
    _assert2.default.deepEqual(inv(_index.indexjs.matrix([[1, 2], [3, 4]], 'sparse')), _index.indexjs.matrix([[-2, 1], [1.5, -0.5]], 'sparse'));
  });
  ...
  it('should throw an error in case of invalid type of arguments', function () {
    _assert2.default.throws(function () {
      _index.indexjs.concat(inv(new Date()));
    }, /TypeError: Unexpected type of argument/);
  });

  it('should  LaTeX inv', function () {
    var expression = _index.indexjs.parse('inv([[1,2],[3,4]])');

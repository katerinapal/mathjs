...
describe('transpose', function() {
  ...
  it('should transpose a vector', function() {
    ...
    assert.deepEqual(transpose(indexjs.matrix([1,2,3]).toArray()), [1,2,3]);
  });

  it('should transpose a 2d matrix', function() {
    assert.deepEqual(transpose([[1,2,3],[4,5,6]]), [[1,4],[2,5],[3,6]]);
    assert.deepEqual(transpose(indexjs.matrix([[1,2,3],[4,5,6]]).toArray()), [[1,4],[2,5],[3,6]]);
  ...
  describe('DenseMatrix', function () {
    ...
    it('should transpose a 2d matrix', function() {
      var m = indexjs.matrix([[1,2,3],[4,5,6]]);
      var t = transpose(m);
      assert.deepEqual(t.valueOf(), [[1,4],[2,5],[3,6]]);

      m = indexjs.matrix([[1,4],[2,5],[3,6]]);
      t = transpose(m);
      assert.deepEqual(t.toArray(), [[1,2,3],[4,5,6]]);
      
      m = indexjs.matrix([[1,2],[3,4]]);
      t = transpose(m);
      ...
      m = indexjs.matrix([[1,2,3,4]], 'dense', 'number');
      t = transpose(m);
      assert.deepEqual(t.valueOf(), [[1],[2],[3],[4]]);
  ...
  describe('SparseMatrix', function () {
    ...
    it('should transpose a 2d matrix', function() {
      var m = indexjs.sparse([[1,2,3],[4,5,6]]);
      var t = transpose(m);
      assert.deepEqual(t.valueOf(), [[1,4],[2,5],[3,6]]);

      m = indexjs.sparse([[1,4],[2,5],[3,6]]);
      t = transpose(m);
      assert.deepEqual(t.toArray(), [[1,2,3],[4,5,6]]);
      
      m = indexjs.sparse([[1,2],[3,4]]);
      t = transpose(m);
      assert.deepEqual(t.valueOf(), [[1,3],[2,4]]);

      m = indexjs.sparse([[1,2,3,4]], 'number');
      t = transpose(m);
      assert.deepEqual(t.valueOf(), [[1],[2],[3],[4]]);
      assert.ok(t.datatype() === 'number');
    });
    ...
    it('should throw an error for invalid matrix transpose', function() {
      var m = indexjs.matrix([[]], 'sparse');
      assert.throws(function () { transpose(m); });
    });
  });

  it('should LaTeX transpose', function () {
    var expression = indexjs.parse('transpose([[1,2],[3,4]])');

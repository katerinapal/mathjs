...
describe('cross', function () {
  ...
  it('should calculate cross product for two arrays', function () {
    _assert2.default.deepEqual(math.cross([1, 1, 0], [0, 1, 1]), [1, -1, 1]);
    _assert2.default.deepEqual(math.cross([3, -3, 1], [4, 9, 2]), [-15, -2, 39]);
    _assert2.default.deepEqual(math.cross([2, 3, 4], [5, 6, 7]), [-3, 6, -3]);
  });

  it('should calculate cross product for two matrices', function () {
    _assert2.default.deepEqual(math.cross(math.matrix([1, 1, 0]), math.matrix([0, 1, 1])), math.matrix([1, -1, 1]));
  });

  it('should calculate cross product for mixed arrays and matrices', function () {
    _assert2.default.deepEqual(math.cross([1, 1, 0], math.matrix([0, 1, 1])), math.matrix([1, -1, 1]));
    _assert2.default.deepEqual(math.cross(math.matrix([1, 1, 0]), [0, 1, 1]), math.matrix([1, -1, 1]));
  });

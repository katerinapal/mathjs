...
describe('map', function() {
  ...
  it('should apply map to all elements of the matrix', function() {
    ...
    var m2 = indexjs.map(m, function (value) { return value * 2; });
    assert.deepEqual(m2.valueOf(), [[2,4,6],[8,10,12]]);
    assert.ok(m2 instanceof indexjs.type.Matrix);
  });

  it('should apply deep-map to all elements in the array', function() {
    var arr = [[1,2,3], [4,5,6]];
    var arr2 = indexjs.map(arr, function (value) { return value * 2; });
  ...
  it('should invoke callback with parameters value, index, obj', function() {
    ...
    assert.deepEqual(indexjs.map(arr, function (value, index, obj) {
  ...
  it('should invoke a typed function with correct number of arguments (1)', function() {
    var output = indexjs.map([1,2,3], indexjs.typed('callback', {
  ...
  it('should throw an error if called with unsupported type', function() {
    assert.throws(function() { indexjs.map(1, function() {}) });
    assert.throws(function() { indexjs.map('arr', function() {}) });
  });

  it('should throw an error if called with invalid number of arguments', function() {
    assert.throws(function() { indexjs.map([1, 2, 3]) });
  });

  it('should LaTeX map', function () {
    var expression = indexjs.parse('map([1,2,3],callback)');

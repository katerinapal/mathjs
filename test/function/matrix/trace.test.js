...
describe('trace', function() {
  ...
  it('should calculate correctly the trace of a NxN array', function() {
    assert.equal(indexjs.trace([5]), 5);
    assert.equal(indexjs.trace([[1,2],[3,4]]), 5);
    approxjs(indexjs.trace([
      [-2, 2,  3],
      [-1, 1,  3],
      [ 2, 0, -1]
    ]), -2);
  ...
  it('should calculate correctly the trace of a NxN matrix', function() {
    assert.equal(indexjs.trace(indexjs.matrix([5])), 5);
    assert.equal(indexjs.trace(indexjs.matrix([[1,2],[3,4]])), 5);
    assert.equal(indexjs.trace(indexjs.matrix([[1,2],[3,4]])), 5);
    approxjs(
      indexjs.trace(
        indexjs.matrix(
    ...
    approxjs(
      ...
      28);
    approxjs(indexjs.trace(indexjs.diag([4, -5, 6])), 5);
  });
  
  it('should calculate correctly the trace of a NxN matrix, sparse', function() {
    assert.equal(indexjs.trace(indexjs.matrix([5], 'sparse')), 5);
    assert.equal(indexjs.trace(indexjs.matrix([[1,2],[3,4]], 'sparse')), 5);
    assert.equal(indexjs.trace(indexjs.matrix([[1,2],[3,4]], 'sparse')), 5);
    approxjs(
      indexjs.trace(
        indexjs.matrix(
  ...
  it('should return N for the identity matrix',function() {
    assert.equal(indexjs.trace(indexjs.eye(7)), 7);
    assert.equal(indexjs.trace(indexjs.eye(2)), 2);
    assert.equal(indexjs.trace(indexjs.eye(1)), 1);
  });

  it('should calculate the trace for a scalar',function() {
    assert.equal(indexjs.trace(7), 7);

    var c1 = indexjs.complex(2, 3);
    var c2 = indexjs.trace(c1);
  ...
  it('should calculate the trace for a 1x1 matrix',function() {
    var c1 = indexjs.complex(2, 3);
    var c2 = indexjs.trace(indexjs.matrix([[c1]]));
    assert.deepEqual(c1, c2);
  ...
  it('should calculate the trace for a 1x1 matrix, sparse',function() {
    var c1 = indexjs.complex(2, 3);
    var c2 = indexjs.trace(indexjs.matrix([[c1]], 'sparse'));
    assert.deepEqual(c1, c2);
  ...
  it('should calculate correctly the trace of a matrix with bignumbers', function() {
    ...
    // 1x1
    assert.deepEqual(indexjs.trace([bignumber(5)]), bignumber(5));

    // 2x2
    assert.deepEqual(indexjs.trace([
  ...
  it('should not change the value of the initial matrix', function() {
    ...
    indexjs.trace(m_test);
    assert.deepEqual(m_test, [[1,2,3],[4,5,6],[7,8,9]]);
  });

  it('should not accept a non-square matrix', function() {
    assert.throws(function() { indexjs.trace([1,2]); });
    assert.throws(function() { indexjs.trace([[1,2,3],[1,2,3]]); });
    assert.throws(function() { indexjs.trace([0,1],[0,1],[0,1]); });
    assert.throws(function() { indexjs.trace(indexjs.matrix([[1,2,3],[1,2,3]])); });
    assert.throws(function() { indexjs.trace(indexjs.matrix([[1,2,3],[1,2,3]], 'sparse')); });
  });

  it('should not accept arrays with dimensions higher than 2', function() {
    assert.throws(function() { indexjs.trace([[[1]]]); }, RangeError);
    assert.throws(function() { indexjs.trace(indexjs.matrix([[[1]]])); }, RangeError);
  });

  it('should LaTeX trace', function () {
    var expression = indexjs.parse('trace([[1,2],[3,4]])');
  ...
  describe('DenseMatrix', function () {
    ...
    it('should calculate trace on a square matrix', function() {
      ...
      assert.equal(indexjs.trace(m), -1);
      ...
      assert.equal(indexjs.trace(m), 0);
      ...
      assert.equal(indexjs.trace(m), 10);
    });
    ...
    it('should throw an error for invalid matrix', function() {
      ...
      assert.throws(function () { indexjs.trace(m); });
    });
  });

...
var pi = index_obj.pi;
var asech = index_obj.asech;
var sech = index_obj.sech;
...
describe('asech', function() {
  ...
  it('should return the hyperbolic arcsec of a number', function() {
    toolsapprox_obj.deepEqual(asech(-0.5), complex(1.3169578969, pi));
    toolsapprox_obj.deepEqual(asech(2), complex(0, pi / 3));
    //assert.ok(isNaN(asech(-0.5)));
    //assert.ok(isNaN(asech(2)));

    assert.equal(asech(0), Infinity);
    toolsapprox_obj(asech(0.25), 2.0634370688955605467272811726201);
    toolsapprox_obj(asech(0.5), 1.31695789692481670862504634730797);
    toolsapprox_obj(asech(0.75), 0.7953654612239056305278909331478);
  ...
  it('should be the inverse function of hyperbolic sec', function() {
    toolsapprox_obj(asech(sech(-1)), 1);
    toolsapprox_obj(asech(sech(0)), 0);
    toolsapprox_obj(asech(sech(0.1)), 0.1);
    toolsapprox_obj(asech(sech(0.5)), 0.5);
    toolsapprox_obj(asech(sech(2)), 2);
  });
  ...
  it('should return the arcsech of a complex number', function() {
    toolsapprox_obj.deepEqual(asech(complex('2+3i')), complex(0.23133469857397, -1.420410722467035));
    toolsapprox_obj.deepEqual(asech(complex('2-3i')), complex(0.23133469857397, 1.420410722467035));
    toolsapprox_obj.deepEqual(asech(complex('-2+3i')), complex(0.23133469857397, -1.72118193112275858));
    toolsapprox_obj.deepEqual(asech(complex('-2-3i')), complex(0.23133469857397, 1.72118193112275858));
    toolsapprox_obj.deepEqual(asech(complex('1+i')), complex(0.5306375309525178, -1.11851787964370594));
    toolsapprox_obj.deepEqual(asech(complex('i')), complex(0.881373587019543, -1.570796326794897));
    toolsapprox_obj.deepEqual(asech(complex('2')), complex(0, pi / 3));
    assert.deepEqual(asech(complex('1')), complex(0, 0));
    toolsapprox_obj.deepEqual(asech(complex('0.5')), complex(1.3169578969248, 0));
    assert.deepEqual(asech(complex('0')), complex(Infinity, 0));
    toolsapprox_obj.deepEqual(asech(complex('-0.5')), complex(1.3169578969248, pi));
    toolsapprox_obj.deepEqual(asech(complex('-1')), complex(0, pi));
  });
  ...
  it('should LaTeX asech', function () {
    var expression = index_obj.parse('asech(1)');

...
describe('cot', function () {
  it('should return the cotan of a boolean', function () {
    (0, _approx.equal)(cot(true), 0.642092615934331);
    (0, _approx.equal)(cot(false), Infinity);
  });

  it('should return the cotan of null', function () {
    (0, _approx.equal)(cot(null), Infinity);
  });

  it('should return the cotan of a number', function () {
    (0, _approx.equal)(cot(0), Infinity);
    (0, _approx.equal)(1 / cot(pi * 1 / 8), 0.414213562373095);
    (0, _approx.equal)(1 / cot(pi * 1 / 4), 1);
    (0, _approx.equal)(cot(pi * 2 / 4), 0);
    (0, _approx.equal)(1 / cot(pi * 3 / 4), -1);
    (0, _approx.equal)(1 / cot(pi * 4 / 4), 0);
    (0, _approx.equal)(1 / cot(pi * 5 / 4), 1);
    (0, _approx.equal)(cot(pi * 6 / 4), 0);
    (0, _approx.equal)(1 / cot(pi * 7 / 4), -1);
    (0, _approx.equal)(1 / cot(pi * 8 / 4), 0);
  });
  ...
  it('should return the cotan of a complex number', function () {
    ...
    (0, _approx.equal)(cot(complex('2+3i')), complex(-re, -im));
    (0, _approx.equal)(cot(complex('2-3i')), complex(-re, im));
    (0, _approx.equal)(cot(complex('-2+3i')), complex(re, -im));
    (0, _approx.equal)(cot(complex('-2-3i')), complex(re, im));
    (0, _approx.equal)(cot(complex('i')), complex(0, -1.313035285499331));
    (0, _approx.equal)(cot(complex('1')), complex(0.642092615934331, 0));
    (0, _approx.equal)(cot(complex('1+i')), complex(0.217621561854403, -0.868014142895925));
  });

  it('should return the cotan of an angle', function () {
    (0, _approx.equal)(cot(unit('45deg')), 1);
    (0, _approx.equal)(cot(unit('-45deg')), -1);

    (0, _assert2.default)(cot(unit(math.bignumber(45), 'deg')).isBigNumber);
    (0, _approx.equal)(cot(unit(math.bignumber(45), 'deg')).toNumber(), 1);

    (0, _approx.equal)(cot(math.unit(complex('1+i'), 'rad')), complex(0.217621561854403, -0.868014142895925));
  });
  ...
  it('should return the cotan of each element of an array', function () {
    (0, _approx.equal)(cot([1, 2, 3]), cot123);
  });

  it('should return the cotan of each element of a matrix', function () {
    (0, _approx.equal)(cot(matrix([1, 2, 3])), matrix(cot123));
  });

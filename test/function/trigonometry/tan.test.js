...
var error = require('../../../lib/error/index'), math = require('../../../index'), pi = math.pi, complex = math.complex, matrix = math.matrix, unit = math.unit, tan = math.tan, piBigmath = math.create({number: 'BigNumber', precision: 21}), bigmath = math.create({precision: 20}), Big = bigmath.bignumber, bigTan = bigmath.tan;
...
describe('tan', function() {
  ...
  it('should return the tangent of a complex number', function() {
    ...
    toolsapprox_equaljs(tan(complex('2+3i')), complex(-re, im));
    toolsapprox_equaljs(tan(complex('2-3i')), complex(-re, -im));
    toolsapprox_equaljs(tan(complex('-2+3i')), complex(re, im));
    toolsapprox_equaljs(tan(complex('-2-3i')), complex(re, -im));
    toolsapprox_equaljs(tan(complex('i')), complex(0, 0.761594155955765));
    toolsapprox_equaljs(tan(complex('1')), complex(1.55740772465490, 0));
    toolsapprox_equaljs(tan(complex('1+i')), complex(0.271752585319512, 1.083923327338695));
  });

  it('should return the tangent of an angle', function() {
    toolsapprox_equaljs(tan(unit(' 60deg')), math.sqrt(3));
    toolsapprox_equaljs(tan(unit('-135deg')), 1);

    assert(tan(unit(math.bignumber(60), 'deg')).isBigNumber);
    toolsapprox_equaljs(tan(unit(math.bignumber(60), 'deg')).toNumber(), math.sqrt(3));

    toolsapprox_equaljs(tan(unit(complex('1+i'), 'rad')), complex(0.271752585319512, 1.083923327338695));
  });
  ...
  it('should return the tan of each element of an array', function() {
    toolsapprox_equaljs(tan([1,2,3]), tan123);
  });

  it('should return the tan of each element of a matrix', function() {
    toolsapprox_equaljs(tan(matrix([1,2,3])), matrix(tan123));
  });

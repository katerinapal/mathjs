...
describe('atan', function() {
  it('should return the arctan of a boolean', function () {
    approxjs(atan(true), 0.25 * pi);
    approxjs(atan(false), 0);
  });

  it('should return the arctan of null', function () {
    approxjs(atan(null), 0);
  });

  it('should return the arctan of a number', function() {
    approxjs(atan(-1) / pi, -0.25);
    approxjs(atan(-0.5) / pi, -0.147583617650433);
    approxjs(atan(0) / pi, 0);
    approxjs(atan(0.5) / pi, 0.147583617650433);
    approxjs(atan(1) / pi, 0.25);
  });
  ...
  it('should be the inverse function of tan', function() {
    approxjs(atan(tan(-1)), -1);
    approxjs(atan(tan(0)), 0);
    approxjs(atan(tan(0.1)), 0.1);
    approxjs(atan(tan(0.5)), 0.5);
    approxjs(atan(tan(2)), -1.14159265358979);
  });
  ...
  it('should return the arctan of a complex number', function() {
    ...
    approxjs.deepEqual(atan(complex('2+3i')), complex(re, im));
    approxjs.deepEqual(atan(complex('2-3i')), complex(re, -im));
    approxjs.deepEqual(atan(complex('-2+3i')), complex(-re, im));
    approxjs.deepEqual(atan(complex('-2-3i')), complex(-re, -im));
    approxjs.deepEqual(atan(complex('i')), complex(0, Infinity));
    approxjs.deepEqual(atan(complex('-i')), complex(0, -Infinity));
    approxjs.deepEqual(atan(complex('1')), complex(0.785398163397448, 0));
    approxjs.deepEqual(atan(complex('1+i')), complex(1.017221967897851, 0.402359478108525));
  });
  ...
  it('should calculate the arctan element-wise for arrays and matrices', function() {
    ...
    approxjs.deepEqual(atan([1,2,3]), atan123);
    approxjs.deepEqual(atan(matrix([1,2,3])), matrix(atan123));
  });
  ...
  it('should LaTeX atan', function () {
    var expression = indexjs.parse('atan(10)');

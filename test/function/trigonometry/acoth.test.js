...
var pi = index_obj.pi;
var acoth = index_obj.acoth;
var coth = index_obj.coth;
...
describe('acoth', function() {
  it('should return the hyperbolic arccot of a boolean', function () {
    ...
    toolsapprox_obj.deepEqual(acoth(false), complex(0, pi / 2));
    //assert.ok(isNaN(acoth(false)));
  });

  it('should return the hyperbolic arccot of null', function () {
    toolsapprox_obj.deepEqual(acoth(null), complex(0, pi / 2));
    //assert.ok(isNaN(acoth(null)));
  });

  it('should return the hyperbolic arccot of a number', function() {
    toolsapprox_obj.deepEqual(acoth(0), complex(0, pi / 2));
    toolsapprox_obj.deepEqual(acoth(0.5), complex(0.5493061443340548, -1.5707963267949));
    //assert.ok(isNaN(acoth(0)));
    //assert.ok(isNaN(acoth(0.5)));

    toolsapprox_obj(acoth(-2), -0.54930614433405484569762261846);
  ...
  it('should be the inverse function of hyperbolic cot', function() {
    toolsapprox_obj(acoth(coth(-2)), -2);
    toolsapprox_obj(acoth(coth(-1)), -1);
    toolsapprox_obj(acoth(coth(0)), 0);
    toolsapprox_obj(acoth(coth(1)), 1);
    toolsapprox_obj(acoth(coth(2)), 2);
  });
  ...
  it('should return the arccoth of a complex number', function() {
    toolsapprox_obj.deepEqual(acoth(complex('2+3i')), complex(0.1469466662255, -0.2318238045004));
    toolsapprox_obj.deepEqual(acoth(complex('2-3i')), complex(0.1469466662255, 0.2318238045004));
    toolsapprox_obj.deepEqual(acoth(complex('-2+3i')), complex(-0.1469466662255, -0.2318238045004));
    toolsapprox_obj.deepEqual(acoth(complex('-2-3i')), complex(-0.1469466662255, 0.2318238045004));
    toolsapprox_obj.deepEqual(acoth(complex('1+i')), complex(0.4023594781085251, -0.55357435889705));
    toolsapprox_obj.deepEqual(acoth(complex('i')), complex(0, -pi / 4));
    assert.deepEqual(acoth(complex('1')), complex(Infinity, 0));
    toolsapprox_obj.deepEqual(acoth(complex('0.5')), complex(0.5493061443340548, -1.5707963267949));
    toolsapprox_obj.deepEqual(acoth(complex('0')), complex(0, pi / 2));
  });
  ...
  it('should calculate the arccot element-wise for arrays and matrices', function() {
    ...
    toolsapprox_obj.deepEqual(acoth([1,2,3]), acoth123);
    toolsapprox_obj.deepEqual(acoth(matrix([1,2,3])), matrix(acoth123));
  });
  ...
  it('should LaTeX acoth', function () {
    var expression = index_obj.parse('acoth(2)');

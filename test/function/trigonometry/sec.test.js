...
var error = require('../../../lib/error/index'), math = require('../../../index'), pi = math.pi, complex = math.complex, matrix = math.matrix, unit = math.unit, sec = math.sec, bigmath = math.create({number: 'BigNumber', precision: 20}), biggermath = math.create({number: 'BigNumber', precision: 21});
...
describe('sec', function() {
  ...
  it('should return the secant of a number', function() {
    ...
    toolsapprox_equaljs(1 / sec(pi/4), math.sqrt(2)/2);

    toolsapprox_equaljs(math.pow(sec(pi/4), 2), 2);
  ...
  it('should return the secant of a complex number', function() {
    ...
    toolsapprox_equaljs(sec(complex('2+3i')), complex(-re, im));
    toolsapprox_equaljs(sec(complex('2-3i')), complex(-re, -im));
    toolsapprox_equaljs(sec(complex('-2+3i')), complex(-re, -im));
    toolsapprox_equaljs(sec(complex('-2-3i')), complex(-re, im));
    toolsapprox_equaljs(sec(complex('i')), complex(0.648054273663885, 0));
    toolsapprox_equaljs(sec(complex('1')), complex(1.85081571768093, 0));
    toolsapprox_equaljs(sec(complex('1+i')), complex(0.498337030555187, 0.591083841721045));
  });
  ...
  it('should return the secant of an angle', function() {
    ...
    toolsapprox_equaljs(sec(unit(math.bignumber(45), 'deg')).toNumber(), 1.41421356237310);

    toolsapprox_equaljs(sec(unit(complex('1+i'), 'rad')), complex(0.498337030555187, 0.591083841721045));
  });
  ...
  it('should return the secant of each element of an array', function() {
    toolsapprox_equaljs(sec([1,2,3]), sec123);
  });

  it('should return the secant of each element of a matrix', function() {
    toolsapprox_equaljs(sec(matrix([1,2,3])), matrix(sec123));
  });

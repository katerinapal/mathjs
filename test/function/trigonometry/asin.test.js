...
describe('asin', function() {
  ...
  it('should return the arcsin of a number', function() {
    toolsapprox(asin(-1) / pi, -0.5);
    toolsapprox(asin(-0.5) / pi, -1/6);
    toolsapprox(asin(0) / pi, 0);
    toolsapprox(asin(0.5) / pi, 1/6);
    toolsapprox(asin(1) / pi, 0.5);

    toolsapprox.deepEqual(asin(-2), complex('-1.57079632679490 + 1.31695789692482i'));
    toolsapprox.deepEqual(asin(2), complex('1.57079632679490 - 1.31695789692482i'));
  });
  ...
  it('should be the inverse function of sin', function() {
    toolsapprox(asin(sin(-1)), -1);
    toolsapprox(asin(sin(0)), 0);
    toolsapprox(asin(sin(0.1)), 0.1);
    toolsapprox(asin(sin(0.5)), 0.5);
    toolsapprox(asin(sin(2)), 1.14159265358979);
  });
  ...
  it('should return the arcsin of a complex number', function() {
    ...
    toolsapprox.deepEqual(asin(complex('2+3i')), complex(re, im));
    toolsapprox.deepEqual(asin(complex('2-3i')), complex(re, -im));
    toolsapprox.deepEqual(asin(complex('-2+3i')), complex(-re, im));
    toolsapprox.deepEqual(asin(complex('-2-3i')), complex(-re, -im));
    toolsapprox.deepEqual(asin(complex('i')), complex(0, 0.881373587019543));
    toolsapprox.deepEqual(asin(complex('1')), complex(1.57079632679490, 0));
    toolsapprox.deepEqual(asin(complex('1+i')), complex(0.666239432492515, 1.061275061905036));
  });
  ...
  it('should calculate the arcsin element-wise for arrays and matrices', function() {
    ...
    toolsapprox.deepEqual(asin([1,2,3]), asin123);
    toolsapprox.deepEqual(asin(matrix([1,2,3])), matrix(asin123));
  });
  ...
  it('should LaTeX asin', function () {
    var expression = index.parse('asin(0.5)');

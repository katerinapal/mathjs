...
describe('sinh', function() {
  ...
  it('should return the sinh of a number', function() {
    toolsapprox(sinh(-2), -3.62686040784701876766821398280126170488634201232113572130, EPSILON);
    toolsapprox(sinh(-0.5), -0.52109530549374736162242562641149155910592898261148052794, EPSILON);
    toolsapprox(sinh(0), 0, EPSILON);
    toolsapprox(sinh(0.3), 0.304520293447142618958435267005095229098024232680179727377, EPSILON);
    toolsapprox(sinh(0.5), 0.521095305493747361622425626411491559105928982611480527946, EPSILON);
    toolsapprox(sinh(0.8), 0.888105982187623006574717573189756980559709596888150052610, EPSILON);
    toolsapprox(sinh(1), 1.175201193643801456882381850595600815155717981334095870229, EPSILON);
    toolsapprox(sinh(2), 3.626860407847018767668213982801261704886342012321135721309, EPSILON);
  });
  ...
  it('should return the sinh of a complex number', function() {
    toolsapprox.deepEqual(sinh(complex('1')), complex(1.1752011936438014, 0), EPSILON);
    toolsapprox.deepEqual(sinh(complex('i')), complex(0, 0.8414709848079), EPSILON);
    toolsapprox.deepEqual(sinh(complex('2 + i')), complex(1.9596010414216, 3.1657785132162), EPSILON);
  });

  it('should return the sinh of an angle', function() {
    toolsapprox(sinh(unit('90deg')), 2.3012989023073, EPSILON);
    toolsapprox(sinh(unit('-45deg')), -0.86867096148601, EPSILON);

    assert(sinh(unit(index.bignumber(90), 'deg')).isBigNumber);
    toolsapprox(sinh(unit(index.bignumber(90), 'deg')).toNumber(), 2.3012989023073, EPSILON);

    toolsapprox.deepEqual(sinh(unit(complex('2 + i'), 'rad')), complex(1.9596010414216, 3.1657785132162), EPSILON);
  });
  ...
  it('should return the sinh of each element of an array', function() {
    toolsapprox.deepEqual(sinh([1,2,3]), sinh123, EPSILON);
  });

  it('should return the sinh of each element of a matrix', function() {
    toolsapprox.deepEqual(sinh(matrix([1,2,3])), matrix(sinh123), EPSILON);
  });
  ...
  it('should LaTeX sinh', function () {
    var expression = index.parse('sinh(1)');

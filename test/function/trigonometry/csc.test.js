...
describe('csc', function () {
  ...
  it('should return the cosecant of a number', function () {
    ...
    (0, _approx.equal)(1 / csc(pi / 4), math.sqrt(2) / 2);
  });
  ...
  it('should return the cosecant of a complex number', function () {
    ...
    (0, _approx.equal)(csc(complex('2+3i')), complex(re, im));
    (0, _approx.equal)(csc(complex('2-3i')), complex(re, -im));
    (0, _approx.equal)(csc(complex('-2+3i')), complex(-re, im));
    (0, _approx.equal)(csc(complex('-2-3i')), complex(-re, -im));
    (0, _approx.equal)(csc(complex('i')), complex(0, -0.850918128239322));
    (0, _approx.equal)(csc(complex('1')), complex(1.18839510577812, 0));
    (0, _approx.equal)(csc(complex('1+i')), complex(0.621518017170428, -0.303931001628426));
  });
  ...
  it('should return the cosecant of an angle', function () {
    ...
    (0, _assert2.default)(csc(unit(math.bignumber(45), 'deg')).isBigNumber);
    (0, _approx.equal)(csc(unit(math.bignumber(45), 'deg')).toNumber(), 1.41421356237310);

    (0, _approx.equal)(csc(unit(complex('1+i'), 'rad')), complex(0.621518017170428, -0.303931001628426));
  });
  ...
  it('should return the cosecant of each element of an array', function () {
    (0, _approx.equal)(csc([1, 2, 3]), csc123);
  });

  it('should return the cosecant of each element of a matrix', function () {
    (0, _approx.equal)(csc(matrix([1, 2, 3])), matrix(csc123));
  });

...
describe('equal', function () {
  ...
  it('should compare mixed complex numbers and bignumbers (downgrades to numbers)', function () {
    _assert2.default.deepEqual(equal(_index.indexjs.complex(6, 0), bignumber(6)), true);
    _assert2.default.deepEqual(equal(_index.indexjs.complex(6, -2), bignumber(6)), false);
    _assert2.default.deepEqual(equal(bignumber(6), _index.indexjs.complex(6, 0)), true);
    _assert2.default.deepEqual(equal(bignumber(6), _index.indexjs.complex(6, 4)), false);
  });

  it('should compare two fractions', function () {
    var a = _index.indexjs.fraction(3);
    _assert2.default.strictEqual(equal(a, _index.indexjs.fraction(2)).valueOf(), false);
    _assert2.default.equal(a.toString(), '3');

    _assert2.default.strictEqual(equal(_index.indexjs.fraction(2), _index.indexjs.fraction(3)).valueOf(), false);
    _assert2.default.strictEqual(equal(_index.indexjs.fraction(3), _index.indexjs.fraction(3)).valueOf(), true);

    _assert2.default.strictEqual(equal(_index.indexjs.add(_index.indexjs.fraction(0.1), _index.indexjs.fraction(0.2)), _index.indexjs.fraction(0.3)).valueOf(), true); // this would fail with numbers
  });

  it('should compare mixed fractions and numbers', function () {
    _assert2.default.strictEqual(equal(1, _index.indexjs.fraction(1, 3)), false);
    _assert2.default.strictEqual(equal(_index.indexjs.fraction(2), 2), true);
  });
  ...
  it('should apply configuration option epsilon', function () {
    ...
    _assert2.default.equal(mymath.equal(_index.indexjs.bignumber(1), _index.indexjs.bignumber(0.991)), false);

    mymath.config({ epsilon: 1e-2 });
    _assert2.default.equal(mymath.equal(1, 0.991), true);
    _assert2.default.equal(mymath.equal(_index.indexjs.bignumber(1), _index.indexjs.bignumber(0.991)), true);
  });

  it('should throw an error when comparing a unit with a big number', function () {
    _assert2.default.throws(function () {
      equal(_index.indexjs.unit('5 m'), bignumber(10)).toString();
    });
  });
  ...
  it('should throw an error for two measures of different units', function () {
    _assert2.default.throws(function () {
      equal(_index.indexjs.unit(5, 'km'), _index.indexjs.unit(100, 'gram'));
    });
  });
  ...
  it('should LaTeX equal', function () {
    var expression = _index.indexjs.parse('equal(1,2)');

...
describe('slu', function () {
  ...
  it('should decompose matrix, 4 x 4, natural ordering (order=0), partial pivoting', function () {
    ...
    // partial pivoting
    var r = index.slu(m, 0, 1);

    // verify M[p,q]=L*U
    toolsapprox.deepEqual(_permute(m, r.p, r.q).valueOf(), index.multiply(r.L, r.U).valueOf());
  });
  ...
  var _permute = function (A, pinv, q) {
    ...
    // return matrix
    return new index.type.SparseMatrix({
      values: cvalues,
      index: cindex,
      ptr: cptr,
      size: size,

...
describe('slu', function () {
  ...
  it('should decompose matrix, 4 x 4, natural ordering (order=0), partial pivoting', function () {
    ...
    // partial pivoting
    var r = index_obj.slu(m, 0, 1);

    // verify M[p,q]=L*U
    toolsapprox_obj.deepEqual(_permute(m, r.p, r.q).valueOf(), index_obj.multiply(r.L, r.U).valueOf());
  });
  ...
  var _permute = function (A, pinv, q) {
    ...
    // return matrix
    return new index_obj.type.SparseMatrix({
      values: cvalues,
      index: cindex,
      ptr: cptr,
      size: size,

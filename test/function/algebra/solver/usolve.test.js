...
describe('usolve', function () {
  ...
  it('should solve linear system 4 x 4, arrays', function () {
    ...
    var x = index_indexjsjs.usolve(m, b);

    toolsapprox_deepEqualjs(x, [-1, -1, -1, 4]);
  });
  ...
  it('should solve linear system 4 x 4, matrices', function () {
    ...
    toolsapprox_deepEqualjs(x, index_indexjsjs.matrix([[-1], [-1], [-1], [4]]));
  });
  ...
  it('should throw exception when matrix is singular', function () {
    assert.throws(function () { index_indexjsjs.usolve([[1, 1], [0, 0]], [1, 1]); }, /Error: Linear system cannot be solved since matrix is singular/);
    assert.throws(function () { index_indexjsjs.usolve(index_indexjsjs.matrix([[1, 1], [0, 0]], 'dense'), [1, 1]); }, /Error: Linear system cannot be solved since matrix is singular/);
    assert.throws(function () { index_indexjsjs.usolve(index_indexjsjs.matrix([[1, 1], [0, 0]], 'sparse'), [1, 1]); }, /Error: Linear system cannot be solved since matrix is singular/);
  });
});
...
var round = index_indexjsjs.round;

describe('round', function() {

  it('should round a number to te given number of decimals', function() {
    toolsapprox_equaljs(round(index_indexjsjs.pi), 3);
    toolsapprox_equaljs(round(index_indexjsjs.pi * 1000), 3142);
    toolsapprox_equaljs(round(index_indexjsjs.pi, 3), 3.142);
    toolsapprox_equaljs(round(index_indexjsjs.pi, 6), 3.141593);
  ...
  it('should throw an error on invalid type of n', function() {
    assert.throws(function () {round(index_indexjsjs.pi, new Date());}, /TypeError: Unexpected type of argument/);
  });

  it('should throw an error on invalid value of n', function() {
    assert.throws(function () {round(index_indexjsjs.pi, -2);}, /Number of decimals in function round must be in te range of 0-15/);
    assert.throws(function () {round(index_indexjsjs.pi, 20);}, /Number of decimals in function round must be in te range of 0-15/);
    assert.throws(function () {round(index_indexjsjs.pi, 2.5);}, /Number of decimals in function round must be an integer/);
  });
  ...
  it('should round fractions', function() {
    var a = fraction('2/3');
    assert(round(a) instanceof index_indexjsjs.type.Fraction);
  ...
  it('should round real and imag part of a complex number', function() {
    assert.deepEqual(round(index_indexjsjs.complex(2.2, index_indexjsjs.pi)), index_indexjsjs.complex(2,3));
  });

  it('should round a complex number with a bignumber as number of decimals', function() {
    assert.deepEqual(round(index_indexjsjs.complex(2.157, index_indexjsjs.pi), bignumber(2)), index_indexjsjs.complex(2.16, 3.14));
  });

  it('should throw an error if used with a unit', function() {
    assert.throws(function () { round(index_indexjsjs.unit('5cm')); }, TypeError, 'Function round(unit) not supported');
    assert.throws(function () { round(index_indexjsjs.unit('5cm'), 2); }, TypeError, 'Function round(unit) not supported');
    assert.throws(function () { round(index_indexjsjs.unit('5cm'), bignumber(2)); }, TypeError, 'Function round(unit) not supported');
  });
  ...
  it('should round each element in a matrix, array, range', function() {
    assert.deepEqual(round(index_indexjsjs.range(0,2.1,1/3), 2), index_indexjsjs.matrix([0,0.33,0.67,1,1.33,1.67,2]));
    assert.deepEqual(round(index_indexjsjs.range(0,2.1,1/3)), index_indexjsjs.matrix([0,0,1,1,1,2,2]));
    assert.deepEqual(round([1.7,2.3]), [2,2]);
    assert.deepEqual(round(index_indexjsjs.matrix([1.7,2.3])).valueOf(), [2, 2]);
  });
  ...
  it('should LaTeX round', function () {
    var expr1 = index_indexjsjs.parse('round(1.1)');
    var expr2 = index_indexjsjs.parse('round(1.1,2)');

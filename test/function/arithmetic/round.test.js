...
var round = _index.indexjs.round;

describe('round', function () {

  it('should round a number to te given number of decimals', function () {
    (0, _approx.equal)(round(_index.indexjs.pi), 3);
    (0, _approx.equal)(round(_index.indexjs.pi * 1000), 3142);
    (0, _approx.equal)(round(_index.indexjs.pi, 3), 3.142);
    (0, _approx.equal)(round(_index.indexjs.pi, 6), 3.141593);
  ...
  it('should round fractions', function () {
    var a = fraction('2/3');
    (0, _assert2.default)(round(a) instanceof _index.indexjs.type.Fraction);
  ...
  it('should round real and imag part of a complex number', function () {
    _assert2.default.deepEqual(round(_index.indexjs.complex(2.2, _index.indexjs.pi)), _index.indexjs.complex(2, 3));
  });

  it('should round a complex number with a bignumber as number of decimals', function () {
    _assert2.default.deepEqual(round(_index.indexjs.complex(2.157, _index.indexjs.pi), bignumber(2)), _index.indexjs.complex(2.16, 3.14));
  });

  it('should throw an error if used with a unit', function () {
    _assert2.default.throws(function () {
      round(_index.indexjs.unit('5cm'));
    }, TypeError, 'Function round(unit) not supported');
    _assert2.default.throws(function () {
      round(_index.indexjs.unit('5cm'), 2);
    }, TypeError, 'Function round(unit) not supported');
    _assert2.default.throws(function () {
      round(_index.indexjs.unit('5cm'), bignumber(2));
    }, TypeError, 'Function round(unit) not supported');
  });
  ...
  it('should round each element in a matrix, array, range', function () {
    _assert2.default.deepEqual(round(_index.indexjs.range(0, 2.1, 1 / 3), 2), _index.indexjs.matrix([0, 0.33, 0.67, 1, 1.33, 1.67, 2]));
    _assert2.default.deepEqual(round(_index.indexjs.range(0, 2.1, 1 / 3)), _index.indexjs.matrix([0, 0, 1, 1, 1, 2, 2]));
    _assert2.default.deepEqual(round([1.7, 2.3]), [2, 2]);
    _assert2.default.deepEqual(round(_index.indexjs.matrix([1.7, 2.3])).valueOf(), [2, 2]);
  });
  ...
  it('should LaTeX round', function () {
    var expr1 = _index.indexjs.parse('round(1.1)');
    var expr2 = _index.indexjs.parse('round(1.1,2)');

...
describe('cbrt', function() {
  ...
  it('should return the cubic root of a positive number', function() {
    ...
    toolsapprox_obj(cbrt(10), 2.1544346900318834);
  });
  ...
  it('should return all cubic roots of a number', function() {
    toolsapprox_obj.deepEqual(cbrt(8, true), index_obj.matrix([
      complex('2'),
      complex('-1 + 1.7321i'),
      complex('-1 - 1.7321i')
    ]));
  ...
  it('should return the cubic root of a complex number', function() {
    toolsapprox_obj.deepEqual(cbrt(complex('2 + 3i')), complex('1.451856618352664928164697 + 0.493403534104004716735578i'));
    toolsapprox_obj.deepEqual(cbrt(complex('-2 + 3i')), complex('1.15322830402742 + 1.01064294709397i'));
    toolsapprox_obj.deepEqual(cbrt(complex('8i')), complex('1.73205080756888 + i'));
  });

  it('should return all three roots of a complex number', function() {
    toolsapprox_obj.deepEqual(cbrt(complex('2 + 3i'), true), index_obj.matrix([
        complex('1.4519 + 0.4934i'),
        complex('-1.1532 + 1.0106i'),
        complex('-0.2986 - 1.5040i')
    ]));

    toolsapprox_obj.deepEqual(cbrt(complex('8i'), true), index_obj.matrix([
        complex(' 1.7321 + i'),
        complex('-1.7321 + i'),
        complex('-2i')
    ]));

    index_obj.config({matrix: 'Array'});

    toolsapprox_obj.deepEqual(cbrt(complex('8i'), true), [
      complex(' 1.7321 + i'),
      complex('-1.7321 + i'),
      complex('-2i')
    ]);

    index_obj.config({matrix: 'Matrix'});
  });

  it('should return the cubic root of a unit', function() {
    assert.equal(cbrt(index_obj.unit('27 m^3')).toString(), index_obj.unit('3 m').toString());
    assert.equal(cbrt(index_obj.unit('-27 m^3')).toString(), index_obj.unit('-3 m').toString());

    assert(cbrt(index_obj.unit(index_obj.bignumber(27), 'm^3')).value.isBigNumber);
    assert.deepEqual(cbrt(index_obj.unit(index_obj.bignumber(27), 'm^3')).value, index_obj.bignumber(3));
    assert(cbrt(index_obj.unit(index_obj.bignumber(-27), 'm^3')).value.isBigNumber);
    assert.deepEqual(cbrt(index_obj.unit(index_obj.bignumber(-27), 'm^3')).value, index_obj.bignumber(-3));

    assert(cbrt(index_obj.unit(index_obj.complex(-46, 9), 's^3')).value.isComplex);
    toolsapprox_obj.deepEqual(cbrt(index_obj.unit(index_obj.complex(-46, 9), 's^3')).value, index_obj.complex(2, 3));
  });
  ...
  it('should return the cubic root of each element of a matrix', function() {
    ...
    assert.deepEqual(cbrt([[8,27],[64,125]]), [[2,3],[4,5]]);
    assert.deepEqual(cbrt(index_obj.matrix([[8,27],[64,125]])), index_obj.matrix([[2,3],[4,5]]));
  });
  ...
  it('should LaTeX cbrt', function () {
    var expression = index_obj.parse('cbrt(2)');

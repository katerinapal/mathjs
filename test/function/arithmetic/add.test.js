...
describe('add', function() {
  ...
  describe('Array', function () {
    ...
    it('should add array and dense matrix correctly', function() {
      ...
      assert.deepEqual(c, index.matrix([4,4,4]));
    });
    ...
    it('should add array and sparse matrix correctly', function() {
      ...
      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.matrix([[7,7,7],[7,7,7]]));
    });
  });
  ...
  describe('DenseMatrix', function () {
    ...
    it('should handle strings and matrices element wise', function() {
      assert.deepEqual(add('2', index.matrix(['3', '4'])), index.matrix([5, 6]));
      assert.deepEqual(add(index.matrix(['2', '3']), '4'), index.matrix([6, 7]));
    });
    
    it('should add matrices correctly', function() {
      var a2 = index.matrix([[1,2],[3,4]]);
    ...
    it('should add 3 dimension natrices correctly', function() {
      var a2 = index.matrix([[[1,1],[2,2]],[[3,3],[4,4]]]);
      var a3 = index.matrix([[[5,5],[6,6]],[[7,7],[8,8]]]);
      var a4 = add(a2, a3);
      assert.deepEqual(a4, index.matrix([[[6,6],[8,8]],[[10,10],[12,12]]]));
    });
    
    it('should add a scalar and a matrix correctly', function() {
      assert.deepEqual(add(2, index.matrix([3,4])), index.matrix([5,6]));
      assert.deepEqual(add(index.matrix([3,4]), 2), index.matrix([5,6]));
    });
    ...
    it('should add dense and sparse matrices correctly', function() {
      ...
      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.matrix([[4,4,4],[1,0,1]]));
    });
  });
  ...
  describe('SparseMatrix', function () {
    ...
    it('should add matrices correctly', function() {
      var a2 = index.matrix([[1,2],[3,4]], 'sparse');
      var a3 = index.matrix([[5,-2],[7,-4]], 'sparse');
      var a4 = add(a2, a3);
      assert.ok(a4 instanceof index.type.Matrix);
      assert.deepEqual(a4, index.sparse([[6,0],[10,0]]));
    });

    it('should add a scalar and a matrix correctly', function() {
      assert.deepEqual(add(2, index.matrix([[3,4],[5,6]], 'sparse')), index.matrix([[5,6],[7,8]], 'dense'));
      assert.deepEqual(add(index.matrix([[3,4],[5,6]], 'sparse'), 2), index.matrix([[5,6],[7,8]], 'dense'));
    });
    ...
    it('should add sparse and sparse matrices correctly', function() {
      ...
      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.sparse([[4,4,4],[1,0,1]]));
    });
    
    it('should add two pattern matrices correctly', function() {
      
      var a = new index.type.SparseMatrix({
        values: undefined,
        index: [0, 1, 2, 0],
        ptr: [0, 2, 3, 4],
        size: [3, 3]
      ...
      var b = new index.type.SparseMatrix({
        values: undefined,
        index: [0, 1, 2, 1],
        ptr: [0, 3, 3, 4],
        size: [3, 3]

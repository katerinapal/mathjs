...
describe('unaryPlus', function() {
  it('should return unary plus of a boolean', function () {
    assert.equal(indexjs.unaryPlus(true), 1);
    assert.equal(indexjs.unaryPlus(false), 0);
  });

  it('should return unary plus of null', function () {
    assert.equal(indexjs.unaryPlus(null), 0);
  });
  ...
  it('should perform unary plus of a number', function() {
    assert.deepEqual(indexjs.unaryPlus(2), 2);
    assert.deepEqual(indexjs.unaryPlus(-2), -2);
    assert.deepEqual(indexjs.unaryPlus(0), 0);
  });

  it('should perform unary plus of a big number', function() {
    assert.deepEqual(indexjs.unaryPlus(bignumber(2)), bignumber(2));
    assert.deepEqual(indexjs.unaryPlus(bignumber(-2)), bignumber(-2));
    assert.deepEqual(indexjs.unaryPlus(bignumber(0)).valueOf(), bignumber(0).valueOf());
  });

  it('should perform unary plus of a fraction', function() {
    var a = fraction(0.5);
    assert(indexjs.unaryPlus(a) instanceof indexjs.type.Fraction);
    assert.equal(a.toString(), '0.5');

    assert.equal(indexjs.unaryPlus(fraction(0.5)).toString(), '0.5');
    assert.equal(indexjs.unaryPlus(fraction(-0.5)).toString(), '-0.5');
  });

  it('should perform unary plus of a complex number', function() {
    assert.equal(indexjs.unaryPlus(indexjs.complex(3, 2)), '3 + 2i');
    assert.equal(indexjs.unaryPlus(indexjs.complex(3, -2)), '3 - 2i');
    assert.equal(indexjs.unaryPlus(indexjs.complex(-3, 2)), '-3 + 2i');
    assert.equal(indexjs.unaryPlus(indexjs.complex(-3, -2)), '-3 - 2i');
  });
  ...
  it('should perform unary plus of a unit', function() {
    assert.equal(indexjs.unaryPlus(indexjs.unit(5, 'km')).toString(), '5 km');
  });

  it('should perform element-wise unary plus on a matrix', function() {
    a2 = indexjs.matrix([[1,2],[3,4]]);
    var a7 = indexjs.unaryPlus(a2);
    assert.ok(a7 instanceof indexjs.type.Matrix);
    assert.deepEqual(a7.size(), [2,2]);
    assert.deepEqual(a7.valueOf(), [[1,2],[3,4]]);
    assert.deepEqual(indexjs.unaryPlus([[1,2],[3,4]]), [[1,2],[3,4]]);
  });

  it('should throw an error in case of invalid number of arguments', function() {
    assert.throws(function () {indexjs.unaryPlus()}, /TypeError: Too few arguments/);
    assert.throws(function () {indexjs.unaryPlus(1, 2)}, /TypeError: Too many arguments/);
  });

  it('should throw an error in case of invalid type of argument', function() {
    assert.throws(function () {indexjs.unaryPlus(new Date())}, /TypeError: Unexpected type of argument/);
  });

  it('should LaTeX unaryPlus', function () {
    var expression = indexjs.parse('unaryPlus(1)');

...
describe('subtract', function() {
  ...
  it('should subtract two complex numbers correctly', function() {
    assert.deepEqual(subtract(index.complex(3, 2), index.complex(8, 4)), index.complex('-5 - 2i'));
    assert.deepEqual(subtract(index.complex(6, 3), index.complex(-2, -2)), index.complex('8 + 5i'));
    assert.deepEqual(subtract(index.complex(3, 4), 10), index.complex('-7 + 4i'));
    assert.deepEqual(subtract(index.complex(3, 4), -2), index.complex('5 + 4i'));
    assert.deepEqual(subtract(index.complex(-3, -4), 10), index.complex('-13 - 4i'));
    assert.deepEqual(subtract(10, index.complex(3, 4)), index.complex('7 - 4i'));
    assert.deepEqual(subtract(10, index.i), index.complex('10 - i'));
    assert.deepEqual(subtract(0, index.i), index.complex('-i'));
    assert.deepEqual(subtract(10, index.complex(0, 1)), index.complex('10 - i'));
  });

  it('should throw an error for mixed complex numbers and big numbers', function() {
    assert.deepEqual(subtract(index.complex(3, 4), index.bignumber(10)), index.complex(-7, 4));
    assert.deepEqual(subtract(index.bignumber(10), index.complex(3, 4)), index.complex(7, -4));
  });

  it('should subtract two fractions', function() {
    var a = index.fraction(1,3);
    assert.equal(subtract(a, index.fraction(1,6)).toString(), '0.1(6)');
    assert.equal(a.toString(), '0.(3)');

    assert.equal(subtract(index.fraction(3,5), index.fraction(1,5)).toString(), '0.4');
    assert.equal(subtract(index.fraction(1), index.fraction(1,3)).toString(), '0.(6)');
  });

  it('should subtract mixed fractions and numbers', function() {
    assert.deepEqual(subtract(1, index.fraction(1,3)), index.fraction(2,3));
    assert.deepEqual(subtract(index.fraction(1,3), 1), index.fraction(-2,3));
  });

  it('should subtract two quantities of the same unit', function() {
    toolsapprox.deepEqual(subtract(index.unit(5, 'km'), index.unit(100, 'mile')), index.unit(-155.93, 'km'));

    assert.deepEqual(subtract(index.unit(index.bignumber(5), 'km'), index.unit(index.bignumber(2), 'km')), index.unit(index.bignumber(3), 'km'));

    assert.deepEqual(subtract(index.unit(index.complex(10,10), 'K'), index.unit(index.complex(3,4), 'K')), index.unit(index.complex(7,6), 'K'));
    assert.deepEqual(subtract(index.unit(index.complex(10,10), 'K'), index.unit(3, 'K')), index.unit(index.complex(7,10), 'K'));
  });

  it('should throw an error if subtracting two quantities of different units', function() {
    assert.throws(function () {
      subtract(index.unit(5, 'km'), index.unit(100, 'gram'));
    });
  });

  it('should throw an error when one of the two units has undefined value', function() {
    assert.throws(function () {
      subtract(index.unit('km'), index.unit('5gram'));
    }, /Parameter x contains a unit with undefined value/);
    assert.throws(function () {
      subtract(index.unit('5 km'), index.unit('gram'));
    }, /Parameter y contains a unit with undefined value/);
  });

  it('should throw an error if subtracting numbers from units', function() {
    assert.throws(function () { subtract(index.unit(5, 'km'), 2); }, TypeError);
    assert.throws(function () { subtract(2, index.unit(5, 'km')); }, TypeError);
  });

  it('should throw an error if subtracting numbers from units', function() {
    assert.throws(function () { subtract(index.unit(5, 'km'), bignumber(2)); }, TypeError);
    assert.throws(function () { subtract(bignumber(2), index.unit(5, 'km')); }, TypeError);
  });
  ...
  describe('Array', function () {
    ...
    it('should subtract array and dense matrix correctly', function() {
      var a = [1,2,3];
      var b = index.matrix([3,2,1]);
      var c = subtract(a, b);

      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.matrix([-2,0,2]));
    });
    
    it('should subtract array and dense matrix correctly', function() {
      var a = [[1,2,3],[4,5,6]];
      var b = index.sparse([[6,5,4],[ 3, 2, 1]]);
      var c = subtract(a, b);

      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.matrix([[-5,-3,-1],[1,3,5]]));
    });
  });
  ...
  describe('DenseMatrix', function () {
    ...
    it('should subtract matrices correctly', function() {
      var a2 = index.matrix([[10,20],[30,40]]);
      var a3 = index.matrix([[5,6],[7,8]]);
      var a4 = subtract(a2, a3);
      assert.ok(a4 instanceof index.type.Matrix);
    ...
    it('should subtract a scalar and a matrix correctly', function() {
      assert.deepEqual(subtract(2, index.matrix([3,4])), index.matrix([-1,-2]));
      assert.deepEqual(subtract(index.matrix([3,4]), 2), index.matrix([1,2]));
    });

    it('should subtract matrix and array correctly', function() {
      var a = index.matrix([1,2,3]);
      var b = [3,2,1];
      var c = subtract(a, b);
    ...
    it('should subtract dense and sparse matrices correctly', function() {
      var a = index.matrix([[1,2,3],[1,0,0]]);
      var b = index.sparse([[3,2,1],[0,0,1]]);
      var c = subtract(a, b);

      assert.ok(c instanceof index.type.Matrix);
      assert.deepEqual(c, index.matrix([[-2,0,2],[1,0,-1]]));
    });
  });
  ...
  describe('SparseMatrix', function () {
    ...
    it('should subtract matrices correctly', function() {
      var a2 = index.matrix([[10,20],[30,0]], 'sparse');
      var a3 = index.matrix([[5,6],[30,8]], 'sparse');
      var a4 = subtract(a2, a3);
      assert.ok(a4 instanceof index.type.Matrix);
      assert.deepEqual(a4, index.sparse([[5,14],[0,-8]]));
    });

    it('should subtract a scalar and a matrix correctly', function() {
      assert.deepEqual(subtract(2, index.matrix([[3,4],[5,6]], 'sparse')).valueOf(), [[-1,-2],[-3,-4]]);
      assert.deepEqual(subtract(2, index.matrix([[3,4],[0,6]], 'sparse')).valueOf(), [[-1,-2],[2,-4]]);
      assert.deepEqual(subtract(index.matrix([[3,4],[5,6]], 'sparse'), 2).valueOf(), [[1,2],[3,4]]);
      assert.deepEqual(subtract(index.matrix([[3,4],[0,6]], 'sparse'), 2).valueOf(), [[1,2],[-2,4]]);
    });

    it('should subtract matrix and array correctly', function() {
      var a = index.matrix([[1,2,3],[1,0,0]], 'sparse');
      var b = [[3,2,1],[0,0,1]];
      var c = subtract(a, b);
    ...
    it('should subtract sparse and dense matrices correctly', function() {
      var a = index.sparse([[1,2,3],[1,0,0]]);
      var b = index.matrix([[3,2,1],[0,0,1]]);
      var c = subtract(a, b);
  ...
  it('should LaTeX subtract', function () {
    var expression = index.parse('subtract(2,1)');

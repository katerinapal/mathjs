...
describe('pow', function() {
  ...
  it('should exponentiate a number to the given power', function() {
    approxjs.deepEqual(pow(2,3), 8);
    approxjs.deepEqual(pow(2,4), 16);
    approxjs.deepEqual(pow(-2,2), 4);
    approxjs.deepEqual(pow(3,3), 27);
    approxjs.deepEqual(pow(3,-2), 0.111111111111111);
    approxjs.deepEqual(pow(-3,-2), 0.111111111111111);
    approxjs.deepEqual(pow(3,-3), 0.0370370370370370);
    approxjs.deepEqual(pow(-3,-3), -0.0370370370370370);
    approxjs.deepEqual(pow(2,1.5), 2.82842712474619);
  });

  it('should exponentiate a negative number to a non-integer power', function() {
    approxjs.deepEqual(pow(-2,1.5), complex(0, -2.82842712474619));
  });
  ...
  it('should return a real-valued root if one exists with predictable:true', function() {
    approxjs(mathPredictable.pow(-8, 1/3), -2);
    approxjs(mathPredictable.pow(-8, 2/3), 4);
    approxjs(mathPredictable.pow(-8, 3/3), -8);
    approxjs(mathPredictable.pow(-8, 4/3), 16);
    approxjs(mathPredictable.pow(-8, 5/3), -32);
    approxjs(mathPredictable.pow(-8, -5/3), -0.03125);
    approxjs(mathPredictable.pow(-1, 2/3), 1);
    approxjs(mathPredictable.pow(-1, 50/99), 1);
    approxjs(mathPredictable.pow(-1, 49/99), -1);
    approxjs(mathPredictable.pow(-17, 29/137), -1.8216292479175);
    approxjs(mathPredictable.pow(-1, 0), 1);
    approxjs(mathPredictable.pow(-1, 0.2), -1);
    approxjs(mathPredictable.pow(-1, 1), -1);

    approxjs(mathPredictable.pow(4, 2), 16);
    approxjs(mathPredictable.pow(4, 0.5), 2);
    approxjs(mathPredictable.pow(-4, 2), 16);
  ...
  it('should exponentiate a negative bignumber to a non-integer power', function() {
    approxjs.deepEqual(pow(bignumber(-2), bignumber(1.5)), complex(0, -2.82842712474619));
    approxjs.deepEqual(pow(-2, bignumber(1.5)), complex(0, -2.82842712474619));
    approxjs.deepEqual(pow(bignumber(-2), 1.5), complex(0, -2.82842712474619));
  });
  ...
  it('should exponentiate a fraction to an integer power', function() {
    assert.deepEqual(indexjs.pow(fraction(3), fraction(2)), fraction(9));
    assert.deepEqual(indexjs.pow(fraction(1.5), fraction(2)), fraction(2.25));
    assert.deepEqual(indexjs.pow(fraction(1.5), fraction(-2)), fraction(4, 9));
    assert.deepEqual(indexjs.pow(fraction(1.5), 2), fraction(2.25));
  });
  ...
  it('should exponentiate a fraction to an non-integer power', function() {
    ...
    assert.strictEqual(indexjs.pow(fraction(4), 1.5), 8);
    assert.strictEqual(indexjs.pow(fraction(4), fraction(1.5)), 8);
  });
  ...
  it('should exponentiate a complex number to the given power', function() {
    approxjs.deepEqual(pow(complex(3, 0), 2), complex(9, 0));
    approxjs.deepEqual(pow(complex(0, 2), 2), complex(-4, 0));

    approxjs.deepEqual(pow(complex(-1,-1),complex(-1,-1)), complex('-0.0284750589322119 +  0.0606697332231795i'));
    approxjs.deepEqual(pow(complex(-1,-1),complex(-1,1)), complex('-6.7536199239765713 +  3.1697803027015614i'));
    approxjs.deepEqual(pow(complex(-1,-1),complex(0,-1)), complex('0.0891447921553914 - 0.0321946742909677i'));
    approxjs.deepEqual(pow(complex(-1,-1),complex(0,1)), complex('9.92340022667813 + 3.58383962127501i'));
    approxjs.deepEqual(pow(complex(-1,-1),complex(1,-1)), complex('-0.1213394664463591 -  0.0569501178644237i'));
    approxjs.deepEqual(pow(complex(-1,-1),complex(1,1)), complex('-6.3395606054031211 - 13.5072398479531426i'));
    approxjs.deepEqual(pow(complex(-1,1),complex(-1,-1)), complex('-6.7536199239765713 -  3.1697803027015614i'));
    approxjs.deepEqual(pow(complex(-1,1),complex(-1,1)), complex('-0.0284750589322119 -  0.0606697332231795i'));
    approxjs.deepEqual(pow(complex(-1,1),complex(0,-1)), complex('9.92340022667813 - 3.58383962127501i'));
    approxjs.deepEqual(pow(complex(-1,1),complex(0,1)), complex('0.0891447921553914 + 0.0321946742909677i'));
    ...
    approxjs.deepEqual(pow(complex(0,-1),complex(-1,-1)), complex('0.000000000000000 + 0.207879576350762i'));
    approxjs.deepEqual(pow(complex(0,-1),complex(-1,1)), complex('0.000000000000000 + 4.810477380965351i'));
    approxjs.deepEqual(pow(complex(0,-1),complex(1,-1)), complex('0.000000000000000 - 0.207879576350762i'));
    approxjs.deepEqual(pow(complex(0,-1),complex(1,1)), complex('0.000000000000000 - 4.810477380965351i'));
    approxjs.deepEqual(pow(complex(0,1),complex(-1,-1)), complex('0.000000000000000 - 4.810477380965351i'));
    approxjs.deepEqual(pow(complex(0,1),complex(-1,1)), complex('0.000000000000000 - 0.207879576350762i'));
    ...
    approxjs.deepEqual(pow(complex(1,-1),complex(-1,-1)), complex('0.2918503793793073 +  0.1369786269150605i'));
    approxjs.deepEqual(pow(complex(1,-1),complex(-1,1)), complex('0.6589325864505904 +  1.4039396486303144i'));
    approxjs.deepEqual(pow(complex(1,-1),complex(0,-1)), complex('0.428829006294368 - 0.154871752464247i'));
    approxjs.deepEqual(pow(complex(1,-1),complex(0,1)), complex('2.062872235080905 + 0.745007062179724i'));
    approxjs.deepEqual(pow(complex(1,-1),complex(1,-1)), complex('0.2739572538301211 -  0.5837007587586147i'));
    approxjs.deepEqual(pow(complex(1,-1),complex(1,1)), complex('2.8078792972606288 -  1.3178651729011805i'));
    approxjs.deepEqual(pow(complex(1,1),complex(-1,-1)), complex('0.6589325864505904 -  1.4039396486303144i'));
    approxjs.deepEqual(pow(complex(1,1),complex(-1,1)), complex('0.2918503793793073 -  0.1369786269150605i'));
    approxjs.deepEqual(pow(complex(1,1),complex(0,-1)), complex('2.062872235080905 - 0.745007062179724i'));
    approxjs.deepEqual(pow(complex(1,1),complex(0,1)), complex('0.428829006294368 + 0.154871752464247i'));
  ...
  it('should exponentiate a complex number to the given bignumber power', function() {
    approxjs.deepEqual(pow(complex(3, 0), indexjs.bignumber(2)), complex(9, 0));
    approxjs.deepEqual(pow(complex(0, 2), indexjs.bignumber(2)), complex(-4, 0));
  });
  ...
  it('should raise a square matrix to the power 2', function() {
    var a = [[1,2],[3,4]];
    var res = [[7,10],[15,22]];
    approxjs.deepEqual(pow(a, 2), res);
    approxjs.deepEqual(pow(matrix(a), 2), matrix(res));
  });

  it('should return identity matrix for power 0', function() {
    var a = [[1,2],[3,4]];
    var res = [[1,0],[0,1]];
    approxjs.deepEqual(pow(a, 0), res);
    approxjs.deepEqual(pow(matrix(a), 0), matrix(res));
  });

  it('should compute large size of square matrix', function() {
    var a = indexjs.eye(30).valueOf();
    approxjs.deepEqual(pow(a, 1000), a);
    approxjs.deepEqual(pow(matrix(a), 1000), matrix(a));
  });
  ...
  it('should LaTeX pow', function () {
    var expression = indexjs.parse('pow(2,10)');

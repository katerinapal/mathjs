...
describe('unaryMinus', function() {
  it('should return unary minus of a boolean', function () {
    assert.equal(index.unaryMinus(true), -1);
    assert.equal(index.unaryMinus(false), 0);
  });
  ...
  it('should return unary minus of null', function () {
    assert.equal(index.unaryMinus(null), 0);
  });

  it('should perform unary minus of a number', function() {
    assert.deepEqual(index.unaryMinus(2), -2);
    assert.deepEqual(index.unaryMinus(-2), 2);
    assert.deepEqual(index.unaryMinus(0), 0);
  });

  it('should perform unary minus of a big number', function() {
    assert.deepEqual(index.unaryMinus(bignumber(2)), bignumber(-2));
    assert.deepEqual(index.unaryMinus(bignumber(-2)), bignumber(2));
    assert.deepEqual(index.unaryMinus(bignumber(0)).toString(), '0');
  });

  it('should perform unary minus of a fraction', function() {
    var a = fraction(0.5);
    assert(index.unaryMinus(a) instanceof index.type.Fraction);
    assert.equal(a.toString(), '0.5');

    assert.equal(index.unaryMinus(fraction(0.5)).toString(), '-0.5');
    assert.equal(index.unaryMinus(fraction(-0.5)).toString(), '0.5');
  });

  it('should perform unary minus of a complex number', function() {
    assert.equal(index.unaryMinus(index.complex(3, 2)), '-3 - 2i');
    assert.equal(index.unaryMinus(index.complex(3, -2)), '-3 + 2i');
    assert.equal(index.unaryMinus(index.complex(-3, 2)), '3 - 2i');
    assert.equal(index.unaryMinus(index.complex(-3, -2)), '3 + 2i');
  });
  ...
  it('should perform unary minus of a unit', function() {
    assert.equal(index.unaryMinus(index.unit(5, 'km')).toString(), '-5 km');
    assert.equal(index.unaryMinus(index.unit(fraction(2/3), 'km')).toString(), '-2/3 km');
    assert.equal(index.unaryMinus(index.unit(complex(2,-4), 'gal')).toString(), '(-2 + 4i) gal');
  });

  it('should perform element-wise unary minus on a matrix', function() {
    a2 = index.matrix([[1,2],[3,4]]);
    var a7 = index.unaryMinus(a2);
    assert.ok(a7 instanceof index.type.Matrix);
    assert.deepEqual(a7.size(), [2,2]);
    assert.deepEqual(a7.valueOf(), [[-1,-2],[-3,-4]]);
    assert.deepEqual(index.unaryMinus([[1,2],[3,4]]), [[-1,-2],[-3,-4]]);
  });

  it('should throw an error in case of invalid number of arguments', function() {
    assert.throws(function () {index.unaryMinus()}, /TypeError: Too few arguments/);
    assert.throws(function () {index.unaryMinus(1, 2)}, /TypeError: Too many arguments/);
  });

  it('should throw an error in case of invalid type of argument', function() {
    assert.throws(function () {index.unaryMinus(new Date())}, /TypeError: Unexpected type of argument/);
  });

  it('should LaTeX unaryMinus', function () {
    var expression = index.parse('unaryMinus(1)');

...
describe('add', function() {
  ...
  it('should add numbers and null', function () {
    assert.equal(index.add(null, null), 0);
    assert.equal(index.add(null, 1), 1);
    assert.equal(index.add(1, null), 1);
  });
  ...
  it('should add mixed numbers and BigNumbers', function() {
    ...
    assert.deepEqual(add(0.1, new BigNumber(0.2)), new index.type.BigNumber(0.3));
  ...
  it('should add mixed booleans and BigNumbers', function() {
    ...
    assert.deepEqual(add(false, new BigNumber(0.2)), new index.type.BigNumber(0.2));
    assert.deepEqual(add(true, new BigNumber(0.2)), new index.type.BigNumber(1.2));
  });

  it('should add mixed complex numbers and BigNumbers', function() {
    assert.deepEqual(add(index.complex(3, -4), new BigNumber(2)), index.complex(5, -4));
    assert.deepEqual(add(new BigNumber(2), index.complex(3, -4)), index.complex(5, -4));
  });

  it('should add two complex numbers', function() {
    assert.equal(add(index.complex(3, -4), index.complex(8, 2)), '11 - 2i');
    assert.equal(add(index.complex(3, -4), 10), '13 - 4i');
    assert.equal(add(10, index.complex(3, -4)), '13 - 4i');
  });

  it('should add two fractions', function() {
    var a = index.fraction(1,3);
    assert.equal(add(a, index.fraction(1,6)).toString(), '0.5');
    assert.equal(a.toString(), '0.(3)');
    assert.equal(add(index.fraction(1,5), index.fraction(2,5)).toString(), '0.6');
    assert.equal(add(index.fraction(1), index.fraction(1,3)).toString(), '1.(3)');
  });

  it('should add mixed fractions and numbers', function() {
    assert.deepEqual(add(1, index.fraction(1,3)), index.fraction(4,3));
    assert.deepEqual(add(index.fraction(1,3), 1), index.fraction(4,3));
  });

  it('should throw an error when converting a number with 15+ digits to fraction', function() {
    assert.throws(function () {
      add(index.pi, index.fraction(1,3))
  ...
  it('should add two measures of the same unit', function() {
    toolsapprox.deepEqual(add(index.unit(5, 'km'), index.unit(100, 'mile')), index.unit(165.93, 'km'));

    toolsapprox.deepEqual(add(index.unit(index.fraction(1,3), 'm'), index.unit(index.fraction(1,3), 'm')).toString(), '2/3 m');

    toolsapprox.deepEqual(add(index.unit(index.complex(-3, 2), 'g'), index.unit(index.complex(5, -6), 'g')).toString(), '(2 - 4i) g');
  });

  it('should throw an error for two measures of different units', function() {
    assert.throws(function () {
      add(index.unit(5, 'km'), index.unit(100, 'gram'));
    });
  });

  it('should throw an error when one of the two units has undefined value', function() {
    assert.throws(function () {
      add(index.unit('km'), index.unit('5gram'));
    }, /Parameter x contains a unit with undefined value/);
    assert.throws(function () {
      add(index.unit('5 km'), index.unit('gram'));
  ...
  it('should throw an error in case of a unit and non-unit argument', function() {
    assert.throws(function () {add(index.unit('5cm'), 2);}, /TypeError: Unexpected type of argument in function add/);
    assert.throws(function () {add(index.unit('5cm'), new Date());}, /TypeError: Unexpected type of argument in function add/);
    assert.throws(function () {add(new Date(), index.unit('5cm'));}, /TypeError: Unexpected type of argument in function add/);
  });
  ...
  it('should LaTeX add', function () {
    var expression = index.parse('add(1,2)');

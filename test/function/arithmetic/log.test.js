...
describe('log', function() {
  ...
  it('should return the log of positive numbers', function() {
    approxjs.deepEqual(log(1), 0);
    approxjs.deepEqual(log(2), 0.693147180559945);
    approxjs.deepEqual(log(3), 1.098612288668110);
    approxjs.deepEqual(indexjs.exp(log(100)), 100);
  });

  it('should return the log of negative numbers', function() {
    approxjs.deepEqual(log(-1), complex('0.000000000000000 + 3.141592653589793i'));
    approxjs.deepEqual(log(-2), complex('0.693147180559945 + 3.141592653589793i'));
    approxjs.deepEqual(log(-3), complex('1.098612288668110 + 3.141592653589793i'));
  });
  ...
  it('should return the log of zero', function() {
    approxjs.deepEqual(log(0), -Infinity);
  });

  it('should return the log base N of a number', function() {
    approxjs.deepEqual(log(100, 10), 2);
    approxjs.deepEqual(log(1000, 10), 3);
    approxjs.deepEqual(log(8, 2), 3);
    approxjs.deepEqual(log(16, 2), 4);
  });
  ...
  it('should return the log of negative bignumbers', function() {
    ...
    approxjs.deepEqual(bigmath.log(bigmath.bignumber(-1)), complex('0.000000000000000 + 3.141592653589793i'));
    approxjs.deepEqual(bigmath.log(bigmath.bignumber(-2)), complex('0.693147180559945 + 3.141592653589793i'));
    approxjs.deepEqual(bigmath.log(bigmath.bignumber(-3)), complex('1.098612288668110 + 3.141592653589793i'));
  });

  it('should return the log of negative bignumbers with predictable:true', function() {
    assert.ok(mathPredictable.log(indexjs.bignumber(-1)).isNaN());
  });
  ...
  it('should return the log of a complex number', function() {
    approxjs.deepEqual(log(indexjs.i),          complex('1.570796326794897i'));
    approxjs.deepEqual(log(complex(0, -1)),  complex('-1.570796326794897i'));
    approxjs.deepEqual(log(complex(1, 1)),   complex('0.346573590279973 + 0.785398163397448i'));
    approxjs.deepEqual(log(complex(1, -1)),  complex('0.346573590279973 - 0.785398163397448i'));
    approxjs.deepEqual(log(complex(-1, -1)), complex('0.346573590279973 - 2.356194490192345i'));
    approxjs.deepEqual(log(complex(-1, 1)),  complex('0.346573590279973 + 2.356194490192345i'));
    approxjs.deepEqual(log(complex(1, 0)),   complex(0, 0));
  });
  ...
  it('should return the log of each element of a matrix', function() {
    ...
    approxjs.deepEqual(log([1,2,3,4]), res);
    approxjs.deepEqual(log(matrix([1,2,3,4])), matrix(res));
    approxjs.deepEqual(log(matrix([[1,2],[3,4]])),
        matrix([[0, 0.693147180559945], [1.098612288668110, 1.386294361119891]]));
  });

  it('should LaTeX log', function () {
    var expr1 = indexjs.parse('log(e)');
    var expr2 = indexjs.parse('log(32,2)');

...
describe('mod', function() {
  ...
  it('should calculate the modulus of two numbers', function() {
    ...
    toolsapprox_obj(mod(7, 2), 1);
    toolsapprox_obj(mod(9, 3), 0);
    toolsapprox_obj(mod(10, 4), 2);
    toolsapprox_obj(mod(-10, 4), 2);
    toolsapprox_obj(mod(8.2, 3), 2.2);
    toolsapprox_obj(mod(4, 1.5), 1);
    toolsapprox_obj(mod(0, 3), 0);
  });
  ...
  it('should throw an error if used on complex numbers', function() {
    assert.throws(function () {mod(index_obj.complex(1,2), 3);}, TypeError);
    assert.throws(function () {mod(3, index_obj.complex(1,2));}, TypeError);
    assert.throws(function () {mod(bignumber(3), index_obj.complex(1,2));}, TypeError);
  });
  ...
  it('should calculate modulus of two fractions', function() {
    var b = index_obj.fraction(8);
    var a = mod(b, index_obj.fraction(3));
    assert.equal(a.toString(), '2');
    assert.equal(b.toString(), '8');
    assert(a instanceof index_obj.type.Fraction);

    assert.equal(mod(index_obj.fraction(4.55), index_obj.fraction(0.05)).toString(), '0');
  });

  it('should calculate modulus of mixed fractions and numbers', function() {
    assert.deepEqual(mod(8, index_obj.fraction(3)), index_obj.fraction(2));
    assert.deepEqual(mod(index_obj.fraction(8), 3), index_obj.fraction(2));
  });

  describe('Array', function () {
    
    it('should perform element-wise modulus on array and scalar', function() {
      toolsapprox_obj.deepEqual(mod([[-4, -3, 0, -1], [0, 1, 2, 3]], 3), [[2, 0, 0, 2], [0, 1, 2, 0]]);
      toolsapprox_obj.deepEqual(mod(3, [[4, 3], [2, 1]]), [[3, 0], [1, 0]]);
    });
    
    it('should perform element-wise modulus on array and array', function() {
      toolsapprox_obj.deepEqual(mod([[-40, -31], [11, -23]], [[3, 7], [1, 3]]), [[2, 4], [0, 1]]);
    });
    
    it('should perform element-wise modulus on array and dense matrix', function() {
      toolsapprox_obj.deepEqual(mod([[-40, -31], [11, -23]], matrix([[3, 7], [1, 3]])), matrix([[2, 4], [0, 1]]));
    });

    it('should perform element-wise modulus on array and sparse matrix', function() {
      toolsapprox_obj.deepEqual(mod([[-40, -31], [11, -23]], sparse([[3, 7], [1, 3]])), matrix([[2, 4], [0, 1]]));
    });
  });
  ...
  describe('DenseMatrix', function () {
    ...
    it('should perform element-wise modulus on dense matrix and scalar', function() {
      toolsapprox_obj.deepEqual(mod(matrix([[-4, -3, 0, -1], [0, 1, 2, 3]]), 3), matrix([[2, 0, 0, 2], [0, 1, 2, 0]]));
      toolsapprox_obj.deepEqual(mod(3, matrix([[4, 3], [2, 1]])), matrix([[3, 0], [1, 0]]));
    });

    it('should perform element-wise modulus on dense matrix and array', function() {
      toolsapprox_obj.deepEqual(mod(matrix([[-40, -31], [11, -23]]), [[3, 7], [1, 3]]), matrix([[2, 4], [0, 1]]));
    });

    it('should perform element-wise modulus on dense matrix and dense matrix', function() {
      toolsapprox_obj.deepEqual(mod(matrix([[-40, -31], [11, -23]]), matrix([[3, 7], [1, 3]])), matrix([[2, 4], [0, 1]]));
    });

    it('should perform element-wise modulus on dense matrix and sparse matrix', function() {
      toolsapprox_obj.deepEqual(mod(matrix([[-40, -31], [11, -23]]), sparse([[3, 7], [1, 3]])), matrix([[2, 4], [0, 1]]));
    });
  });
  ...
  describe('SparseMatrix', function () {
    ...
    it('should perform element-wise modulus on sparse matrix and scalar', function() {
      toolsapprox_obj.deepEqual(mod(sparse([[-4, -3, 0, -1], [0, 1, 2, 3]]), 3), sparse([[2, 0, 0, 2], [0, 1, 2, 0]]));
      toolsapprox_obj.deepEqual(mod(3, sparse([[4, 3], [2, 1]])), matrix([[3, 0], [1, 0]]));
    });

    it('should perform element-wise modulus on sparse matrix and array', function() {
      toolsapprox_obj.deepEqual(mod(sparse([[-40, -31], [11, -23]]), [[3, 7], [1, 3]]), sparse([[2, 4], [0, 1]]));
    });

    it('should perform element-wise modulus on sparse matrix and dense matrix', function() {
      toolsapprox_obj.deepEqual(mod(sparse([[-40, -31], [11, -23]]), matrix([[3, 7], [1, 3]])), sparse([[2, 4], [0, 1]]));
    });

    it('should perform element-wise modulus on sparse matrix and sparse matrix', function() {
      toolsapprox_obj.deepEqual(mod(sparse([[-40, -31], [11, -23]]), sparse([[3, 7], [1, 3]])), sparse([[2, 4], [0, 1]]));
    });
  });
  
  it('should LaTeX mod', function () {
    var expression = index_obj.parse('mod(11,2)');

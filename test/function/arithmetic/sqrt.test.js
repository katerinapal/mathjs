...
describe('sqrt', function() {
  ...
  it('should return the square root of a negative number', function() {
    assert.deepEqual(sqrt(-4), index_obj.complex(0, 2));
    assert.deepEqual(sqrt(-16), index_obj.complex(0, 4));
  });
  ...
  it('should return the square root of a negative bignumber', function() {
    assert.deepEqual(sqrt(bignumber(-4)), index_obj.complex(0, 2));
  });
  ...
  it('should return the square root of a complex number', function() {
    assert.deepEqual(sqrt(index_obj.complex(3, -4)), index_obj.complex(2, -1));
    assert.deepEqual(sqrt(index_obj.complex(1e10, 1e-10)), index_obj.complex(1e5, 5e-16));
  });

  it('should return the square root of a unit', function() {
    assert.equal(sqrt(index_obj.unit('25 m^2/s^2')).toString(), '5 m / s');
    assert.equal(sqrt(index_obj.unit('4 kg')).toString(), '2 kg^0.5');
  });
  ...
  it('should return a Unit with a Complex value when computing the square root of a negative unit', function() {
    ...
    assert.equal(index_obj.format(sqrt(index_obj.unit('-25 m^2/s^2')), 14), '(5i) m / s');
  });
  ...
  it('should return the square root of each element of a matrix', function() {
    ...
    assert.deepEqual(sqrt([[4,9],[16,25]]), [[2,3],[4,5]]);
    assert.deepEqual(sqrt(index_obj.matrix([[4,9],[16,25]])), index_obj.matrix([[2,3],[4,5]]));
  });
  ...
  it('should LaTeX sqrt', function () {
    var expression = index_obj.parse('sqrt(2)');

...
var divide = index_indexjsjs.divide;
...
describe('divide', function() {
  ...
  it('should divide two complex numbers', function() {
    ...
    approx.deepEqual(divide(index_indexjsjs.i, 0), complex(0, Infinity));
  ...
  it('should divide mixed complex numbers and numbers', function() {
    assert.deepEqual(divide(index_indexjsjs.complex(6, -4), 2), index_indexjsjs.complex(3, -2));
    assert.deepEqual(divide(1, index_indexjsjs.complex(2, 4)), index_indexjsjs.complex(0.1, -0.2));
  });

  it('should divide mixed complex numbers and bignumbers', function() {
    assert.deepEqual(divide(index_indexjsjs.complex(6, -4), bignumber(2)), index_indexjsjs.complex(3, -2));
    assert.deepEqual(divide(bignumber(1), index_indexjsjs.complex(2, 4)), index_indexjsjs.complex(0.1, -0.2));
  });

  it('should divide two fractions', function() {
    var a = index_indexjsjs.fraction(1,4);
    assert.equal(divide(a, index_indexjsjs.fraction(1,2)).toString(), '0.5');
    assert.equal(a.toString(), '0.25');
  });

  it('should divide mixed fractions and numbers', function() {
    assert.deepEqual(divide(1, index_indexjsjs.fraction(3)), index_indexjsjs.fraction(1,3));
    assert.deepEqual(divide(index_indexjsjs.fraction(1), 3), index_indexjsjs.fraction(1,3));
  });

  it('should divide units by a number', function() {
    assert.equal(divide(index_indexjsjs.unit('5 m'), 10).toString(), '500 mm');
  });

  it('should divide valueless units by a number', function() {
    assert.equal(divide(index_indexjsjs.unit('m'), 2).toString(), '500 mm');
  });

  it('should divide a number by a unit', function() {
    assert.equal(divide(20, index_indexjsjs.unit('4 N s')).toString(), '5 N^-1 s^-1');
    assert.equal(divide(4, index_indexjsjs.unit('W')).toString(), '4 W^-1');
    assert.equal(divide(2.5, index_indexjsjs.unit('1.25 mm')).toString(), '2 mm^-1');
    assert.equal(divide(10, index_indexjsjs.unit('4 mg/s')).toString(), '2.5 s / mg');

    assert.equal(divide(10, index_indexjsjs.unit(index_indexjsjs.fraction(4), 'mg/s')).toString(), '5/2 s / mg');

    toolsapprox_equaljs(index_indexjsjs.format(divide(10, index_indexjsjs.unit(index_indexjsjs.complex(1,2), 'm/s')), 14), '(2 - 4i) s / m');
  });

  it('should divide two units', function() {
    assert.equal(divide(index_indexjsjs.unit('75 mi/h'), index_indexjsjs.unit('40 mi/gal')).to('gal/minute').toString(), '0.03125 gal / minute');

    var a = index_indexjsjs.unit(index_indexjsjs.fraction(75), 'mi/h');
    var b = index_indexjsjs.unit(index_indexjsjs.fraction(40), 'mi/gal');
    assert.equal(divide(a, b).to('gal/minute').toString(), '1/32 gal / minute');

    var c = index_indexjsjs.unit(index_indexjsjs.complex(21, 1), 'kg');
    var d = index_indexjsjs.unit(index_indexjsjs.complex(2, -3), 's');
  ...
  it('should divide one valued unit by a valueless unit and vice-versa', function() {
    assert.equal(divide(index_indexjsjs.unit('4 gal'), index_indexjsjs.unit('L')).toString(), '15.141648');
    assert.equal(divide(index_indexjsjs.unit('gal'), index_indexjsjs.unit('4 L')).toString(), '0.946353');

    assert.equal(divide(index_indexjsjs.unit('inch'), index_indexjsjs.unit(index_indexjsjs.fraction(1), 'cm')).toFraction(), '127/50');
  });

  it('should divide (but not simplify) two valueless units', function() {
    assert.equal(divide(index_indexjsjs.unit('gal'), index_indexjsjs.unit('L')).toString(), 'gal / L');
  });

  it('should divide units by a big number', function() {
    assert.equal(divide(index_indexjsjs.unit('5 m'), bignumber(10)).toString(), '500 mm');
  });

  it('should divide each elements in a matrix by a number', function() {
    assert.deepEqual(divide([2,4,6], 2), [1,2,3]);
    a  = index_indexjsjs.matrix([[1,2],[3,4]]);
    assert.deepEqual(divide(a, 2), index_indexjsjs.matrix([[0.5,1],[1.5,2]]));
  ...
  it('should perform matrix division', function() {
    a = index_indexjsjs.matrix([[1,2],[3,4]]);
    b = index_indexjsjs.matrix([[5,6],[7,8]]);
    assert.deepEqual(divide(a, b), index_indexjsjs.matrix([[3,-2], [2,-1]]));
  });
  ...
  */
  ...
  it('should LaTeX divide', function () {
    var expression = index_indexjsjs.parse('divide(1,2)');

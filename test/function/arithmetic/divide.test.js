...
describe('divide', function() {
  ...
  it('should divide two complex numbers', function() {
    approxjs.deepEqual(divide(complex('2+3i'), 2), complex('1+1.5i'));
    approxjs.deepEqual(divide(complex('2+3i'), complex('4i')), complex('0.75 - 0.5i'));
    approxjs.deepEqual(divide(complex('2i'), complex('4i')), complex('0.5'));
    approxjs.deepEqual(divide(4, complex('1+2i')), complex('0.8 - 1.6i'));
    approxjs.deepEqual(divide(indexjs.i, 0), complex(0, Infinity));
    approxjs.deepEqual(divide(complex(0,1), 0), complex(0, Infinity));
    approxjs.deepEqual(divide(complex(1,0), 0), complex(Infinity, 0));
    approxjs.deepEqual(divide(complex(0,1), complex(0,0)), complex(0, Infinity));
    approxjs.deepEqual(divide(complex(1,1), complex(0,0)), complex(Infinity, Infinity));
    approxjs.deepEqual(divide(complex(1,-1), complex(0,0)), complex(Infinity, -Infinity));
    approxjs.deepEqual(divide(complex(-1,1), complex(0,0)), complex(-Infinity, Infinity));
    approxjs.deepEqual(divide(complex(1,1), complex(0,1)), complex(1, -1));
    approxjs.deepEqual(divide(complex(1,1), complex(1,0)), complex(1, 1));

    approxjs.deepEqual(divide(complex(2, 3), complex(4, 5)), complex('0.5609756097560976 + 0.0487804878048781i'));
    approxjs.deepEqual(divide(complex(2, 3), complex(4, -5)), complex('-0.170731707317073 + 0.536585365853659i'));
    approxjs.deepEqual(divide(complex(2, 3), complex(-4, 5)), complex('0.170731707317073 - 0.536585365853659i'));
    approxjs.deepEqual(divide(complex(2, 3), complex(-4, -5)), complex('-0.5609756097560976 - 0.0487804878048781i'));
    approxjs.deepEqual(divide(complex(2, -3), complex(4, 5)), complex('-0.170731707317073 - 0.536585365853659i'));
    approxjs.deepEqual(divide(complex(2, -3), complex(4, -5)), complex('0.5609756097560976 - 0.0487804878048781i'));
  ...
  it('should divide mixed complex numbers and numbers', function() {
    assert.deepEqual(divide(indexjs.complex(6, -4), 2), indexjs.complex(3, -2));
    assert.deepEqual(divide(1, indexjs.complex(2, 4)), indexjs.complex(0.1, -0.2));
  });

  it('should divide mixed complex numbers and bignumbers', function() {
    assert.deepEqual(divide(indexjs.complex(6, -4), bignumber(2)), indexjs.complex(3, -2));
    assert.deepEqual(divide(bignumber(1), indexjs.complex(2, 4)), indexjs.complex(0.1, -0.2));
  });

  it('should divide two fractions', function() {
    var a = indexjs.fraction(1,4);
    assert.equal(divide(a, indexjs.fraction(1,2)).toString(), '0.5');
    assert.equal(a.toString(), '0.25');
  });

  it('should divide mixed fractions and numbers', function() {
    assert.deepEqual(divide(1, indexjs.fraction(3)), indexjs.fraction(1,3));
    assert.deepEqual(divide(indexjs.fraction(1), 3), indexjs.fraction(1,3));
  });

  it('should divide units by a number', function() {
    assert.equal(divide(indexjs.unit('5 m'), 10).toString(), '500 mm');
  });

  it('should divide valueless units by a number', function() {
    assert.equal(divide(indexjs.unit('m'), 2).toString(), '500 mm');
  });

  it('should divide a number by a unit', function() {
    assert.equal(divide(20, indexjs.unit('4 N s')).toString(), '5 N^-1 s^-1');
    assert.equal(divide(4, indexjs.unit('W')).toString(), '4 W^-1');
    assert.equal(divide(2.5, indexjs.unit('1.25 mm')).toString(), '2 mm^-1');
    assert.equal(divide(10, indexjs.unit('4 mg/s')).toString(), '2.5 s / mg');

    assert.equal(divide(10, indexjs.unit(indexjs.fraction(4), 'mg/s')).toString(), '5/2 s / mg');

    approxjs(indexjs.format(divide(10, indexjs.unit(indexjs.complex(1,2), 'm/s')), 14), '(2 - 4i) s / m');
  });

  it('should divide two units', function() {
    assert.equal(divide(indexjs.unit('75 mi/h'), indexjs.unit('40 mi/gal')).to('gal/minute').toString(), '0.03125 gal / minute');

    var a = indexjs.unit(indexjs.fraction(75), 'mi/h');
    var b = indexjs.unit(indexjs.fraction(40), 'mi/gal');
    assert.equal(divide(a, b).to('gal/minute').toString(), '1/32 gal / minute');

    var c = indexjs.unit(indexjs.complex(21, 1), 'kg');
    var d = indexjs.unit(indexjs.complex(2, -3), 's');
  ...
  it('should divide one valued unit by a valueless unit and vice-versa', function() {
    assert.equal(divide(indexjs.unit('4 gal'), indexjs.unit('L')).toString(), '15.141648');
    assert.equal(divide(indexjs.unit('gal'), indexjs.unit('4 L')).toString(), '0.946353');

    assert.equal(divide(indexjs.unit('inch'), indexjs.unit(indexjs.fraction(1), 'cm')).toFraction(), '127/50');
  });

  it('should divide (but not simplify) two valueless units', function() {
    assert.equal(divide(indexjs.unit('gal'), indexjs.unit('L')).toString(), 'gal / L');
  });

  it('should divide units by a big number', function() {
    assert.equal(divide(indexjs.unit('5 m'), bignumber(10)).toString(), '500 mm');
  });

  it('should divide each elements in a matrix by a number', function() {
    assert.deepEqual(divide([2,4,6], 2), [1,2,3]);
    a  = indexjs.matrix([[1,2],[3,4]]);
    assert.deepEqual(divide(a, 2), indexjs.matrix([[0.5,1],[1.5,2]]));
  ...
  it('should perform matrix division', function() {
    a = indexjs.matrix([[1,2],[3,4]]);
    b = indexjs.matrix([[5,6],[7,8]]);
    assert.deepEqual(divide(a, b), indexjs.matrix([[3,-2], [2,-1]]));
  });
  ...
  */
  ...
  it('should LaTeX divide', function () {
    var expression = indexjs.parse('divide(1,2)');

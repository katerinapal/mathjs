...
describe('sum', function() {
  ...
  it('should return zero if called with an empty array', function() {
    ...
    var fracMath = index_obj.create({number: 'Fraction'});
    ...
    assert.equal(index_obj.equal(bigMath.sum([]), new BigNumber(0)).valueOf(), true);
    assert.equal(index_obj.equal(fracMath.sum([]), new fracMath.type.Fraction(0)), true);
  });

  it('should LaTeX sum', function () {
    var expression = index_obj.parse('sum(1,2,3)');

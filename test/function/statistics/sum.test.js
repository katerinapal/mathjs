...
describe('sum', function () {
  ...
  it('should return zero if called with an empty array', function () {
    ...
    var fracMath = _index.indexjs.create({ number: 'Fraction' });
    ...
    _assert2.default.equal(_index.indexjs.equal(bigMath.sum([]), new BigNumber(0)).valueOf(), true);
    _assert2.default.equal(_index.indexjs.equal(fracMath.sum([]), new fracMath.type.Fraction(0)), true);
  });

  it('should LaTeX sum', function () {
    var expression = _index.indexjs.parse('sum(1,2,3)');

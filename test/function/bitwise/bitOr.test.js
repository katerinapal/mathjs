...
// test bitOr
var bignumber = index_indexjsjs.bignumber, bitOr = index_indexjsjs.bitOr;
...
describe('bitOr', function () {
  ...
  it('should bitwise or numbers and null', function () {
    assert.equal(index_indexjsjs.bitOr(null, null), 0);
    assert.equal(index_indexjsjs.bitOr(null, 1), 1);
    assert.equal(index_indexjsjs.bitOr(1, null), 1);
  });
  ...
  it('should throw an error if used with a unit', function() {
    assert.throws(function () {bitOr(index_indexjsjs.unit('5cm'), 2);}, /TypeError: Unexpected type of argument/);
    assert.throws(function () {bitOr(2, index_indexjsjs.unit('5cm'));}, /TypeError: Unexpected type of argument/);
    assert.throws(function () {bitOr(index_indexjsjs.unit('2cm'), index_indexjsjs.unit('5cm'));}, /TypeError: Unexpected type of argument/);
  });
  ...
  it('should bitwise or arrays correctly', function () {
    ...
    c = bitOr(a, b);
    assert.deepEqual(c, index_indexjsjs.matrix([[5,12],[7,6]]));
  ...
  it('should bitwise or sparse matrix correctly', function () {
    ...
    b = index_indexjsjs.sparse([[5,8],[7,6]]);
    c = bitOr(a, b);
    assert.deepEqual(c, index_indexjsjs.sparse([[5,12],[7,6]]));

    // sparse - sparse pattern
    a = index_indexjsjs.sparse([[1,1],[0,0]]);
  ...
  it('should bitwise or a scalar and a matrix correctly', function () {
    assert.deepEqual(bitOr(12, index_indexjsjs.matrix([3,9])), index_indexjsjs.matrix([15,13]));
    assert.deepEqual(bitOr(index_indexjsjs.matrix([3,9]), 12), index_indexjsjs.matrix([15,13]));
  });
  ...
  it('should bitwise or a matrix and an array correctly', function () {
    ...
    assert.deepEqual(c, index_indexjsjs.matrix([15,92,125]));
  });
  ...
  it('should LaTeX bitOr', function () {
    var expression = index_indexjsjs.parse('bitOr(2,3)');

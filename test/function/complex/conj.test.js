...
describe('conj', function() {
  ...
  it('should compute the conjugate of a bignumber', function () {
    assert.deepEqual(conj(index_obj.bignumber(2)), index_obj.bignumber(2));
  });

  it('should calculate the conjugate of a complex number correctly', function() {
    assert.equal(conj(index_obj.complex('2 + 3i')).toString(), '2 - 3i');
    assert.equal(conj(123).toString(), '123');
    assert.equal(conj(index_obj.complex('2 - 3i')).toString(), '2 + 3i');
    assert.equal(conj(index_obj.complex('2')).toString(), '2');
    assert.equal(conj(index_obj.complex('-4i')).toString(), '4i');
    assert.equal(conj(index_obj.i).toString(), '-i');
  });

  it('should calculate the conjugate for each element in a matrix', function() {
    assert.equal(index_obj.format(conj([index_obj.complex('2+3i'), index_obj.complex('3-4i')])),
        '[2 - 3i, 3 + 4i]');
    assert.equal(conj(index_obj.matrix([index_obj.complex('2+3i'), index_obj.complex('3-4i')])).toString(),
        '[2 - 3i, 3 + 4i]');
  });
  ...
  it('should throw an error when called with an unsupported type of argument', function() {
    ...
    assert.throws(function () {conj(index_obj.unit('5cm'))}, /TypeError: Unexpected type of argument/);
  });
  ...
  it('should LaTeX conj', function () {
    var expression = index_obj.parse('conj(1+i)');

...
var math = index_obj;
var reviver = index_obj.json.reviver;
...
describe('reviver', function () {
  ...
  it('should parse a stringified complex number', function () {
    ...
    var c = new index_obj.type.Complex(2, 4);

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Complex);
    assert.deepEqual(obj, c);
  });

  it('should parse a stringified BigNumber', function () {
    var json = '{"mathjs":"BigNumber","value":"0.2"}';
    var b = new index_obj.type.BigNumber(0.2);

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.BigNumber);
    assert.deepEqual(obj, b);
  });

  it('should parse a stringified Fraction', function () {
    var json = '{"mathjs":"Fraction","n":3,"d":8}';
    var b = new index_obj.type.Fraction(0.375);

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Fraction);
  ...
  it('should parse a stringified ResultSet', function () {
    ...
    var r = new index_obj.type.ResultSet([1,2,new index_obj.type.Complex(3,4)]);

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.ResultSet);
    assert.deepEqual(obj, r);
  });
  ...
  it('should parse a stringified Index', function () {
    var json = '{"mathjs":"Index","dimensions":[' +
        ...
        ']}';
    var i = new index_obj.type.Index(new Range(0, 10), new Range(2, 3));

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Index);
    assert.deepEqual(obj, i);
  });

  it('should parse a stringified Index (2)', function () {
    var json = '{"mathjs":"Index","dimensions":[[0, 10],2]}';
    var i = new index_obj.type.Index([0, 10], 2);
  ...
  it('should parse a stringified Matrix, dense storage format', function () {
    ...
    var m = index_obj.matrix([[1,2],[3,4]], 'dense');

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Matrix);
  ...
  it('should parse a stringified Matrix containing a complex number, dense storage format', function () {
    ...
    var c = new index_obj.type.Complex(4, 5);
    var m = index_obj.matrix([[1,2],[3,c]], 'dense');

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Matrix);
    assert(obj._data[1][1] instanceof index_obj.type.Complex);
    assert.deepEqual(obj, m);
  });

  it('should parse a Matrix, sparse', function () {
    var json = '{"mathjs":"SparseMatrix","values":[1,3,2,4],"index":[0,1,0,1],"ptr":[0,2,4],"size":[2,2]}';
    var m = index_obj.matrix([[1,2],[3,4]], 'sparse');

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.SparseMatrix);
    assert(obj instanceof index_obj.type.Matrix);
    assert.deepEqual(obj, m);
  });
  ...
  it('should parse a stringified Help', function () {
    ...
    var h = new index_obj.type.Help({name: 'foo', description: 'bar'});
    var obj = JSON.parse(json, reviver);

    assert(obj instanceof index_obj.type.Help);

...
var assert = {};
var math = indexjs;
var reviver = indexjs.json.reviver;
...
describe('reviver', function () {
  ...
  it('should parse a stringified ResultSet', function () {
    ...
    var r = new indexjs.type.ResultSet([1,2,new indexjs.type.Complex(3,4)]);

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof indexjs.type.ResultSet);
    assert.deepEqual(obj, r);
  });
  ...
  it('should parse a stringified Index', function () {
    var json = '{"mathjs":"Index","dimensions":[' +
        ...
        ']}';
    var i = new indexjs.type.Index(new Range(0, 10), new Range(2, 3));

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof indexjs.type.Index);
    assert.deepEqual(obj, i);
  });

  it('should parse a stringified Index (2)', function () {
    var json = '{"mathjs":"Index","dimensions":[[0, 10],2]}';
    var i = new indexjs.type.Index([0, 10], 2);
  ...
  it('should parse a stringified Matrix, dense storage format', function () {
    ...
    assert(obj instanceof indexjs.type.Matrix);
  ...
  it('should parse a stringified Matrix containing a complex number, dense storage format', function () {
    ...
    var c = new indexjs.type.Complex(4, 5);
    var m = indexjs.matrix([[1,2],[3,c]], 'dense');

    var obj = JSON.parse(json, reviver);

    assert(obj instanceof indexjs.type.Matrix);
    assert(obj._data[1][1] instanceof indexjs.type.Complex);
    assert.deepEqual(obj, m);
  });
  ...
  it('should parse a Matrix, sparse', function () {
    ...
    assert(obj instanceof indexjs.type.SparseMatrix);
    assert(obj instanceof indexjs.type.Matrix);
    assert.deepEqual(obj, m);
  });
  ...
  it('should parse a stringified Help', function () {
    ...
    var h = new indexjs.type.Help({name: 'foo', description: 'bar'});
    var obj = JSON.parse(json, reviver);

    assert(obj instanceof indexjs.type.Help);

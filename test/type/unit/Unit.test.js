...
describe('Unit', function() {
  ...
  describe('constructor', function() {
    ...
    it('should create a unit with Fraction value', function () {
      var unit1 = new Unit(indexjs.fraction(1000, 3), 'cm');
      assert.deepEqual(unit1.value, indexjs.fraction(10,3));
      assert.equal(unit1.units[0].unit.name, 'm');
    });

    it('should create a unit with BigNumber value', function () {
      var unit1 = new Unit(indexjs.bignumber(5000), 'cm');
      assert.deepEqual(unit1.value, indexjs.bignumber(50));
      assert.equal(unit1.units[0].unit.name, 'm');
    });

    it('should create a unit with Complex value', function () {
      var unit1 = new Unit(indexjs.complex(500, 600), 'cm');
      assert.deepEqual(unit1.value, indexjs.complex(5, 6));
      assert.equal(unit1.units[0].unit.name, 'm');
    });
  ...
  describe('type', function() {
    ...
    it('should have a property isUnit', function () {
      var a = new indexjs.type.Unit(5, 'cm');
  ...
  describe('equals', function() {
    ...
    it('should test whether two units with Fractions are equal', function() {
      assert.equal(new Unit(indexjs.fraction(100), 'cm').equals(new Unit(indexjs.fraction(1), 'm')), true);
      assert.equal(new Unit(indexjs.fraction(100), 'cm').equals(new Unit(indexjs.fraction(2), 'm')), false);
    });

    it('should test whether two units with a Fraction and a number are equal', function() {
      assert.equal(new Unit(indexjs.fraction(100), 'cm').equals(new Unit(1, 'm')), true);
      assert.equal(new Unit(100, 'cm').equals(new Unit(indexjs.fraction(2), 'm')), false);
    });

    it('should test whether two Complex units are equal', function() {
      assert.equal(new Unit(indexjs.complex(3, 4), 'km').equals(new Unit(indexjs.complex(3000, 4000), 'm')), true);
      assert.equal(new Unit(indexjs.complex(3, 4), 'km').equals(new Unit(indexjs.complex(3000, 10), 'm')), false);
    });

    it('should test whether a Complex unit and a unit with a number are equal', function() {
      assert.equal(new Unit(indexjs.complex(3, 0), 'km').equals(new Unit(3000, 'm')), true);
      assert.equal(new Unit(indexjs.complex(3, 4), 'km').equals(new Unit(3000, 'm')), false);
    });
  ...
  describe('clone', function() {
    ...
    it('should clone a with a Fraction', function() {
      var u1 = new Unit(indexjs.fraction(1,3), 'cm');
      var u2 = u1.clone();
      assert(u1 !== u2);
      assert.deepEqual(u1, u2);
    ...
    it('should clone a Complex unit', function() {
      var u1 = new Unit(indexjs.complex(1,3), 'cm');
      var u2 = u1.clone();
      assert(u1 !== u2);
      assert.deepEqual(u1, u2);
      assert(u1.value !== u2.value); // should be cloned
  ...
  describe('toNumber', function() {
    it ('should convert a unit to a number', function () {
      ...
      approxjs(u.toNumber('mm'), 50000);

      approxjs(new Unit(5.08, 'cm').toNumber('inch'), 2);

      approxjs(new Unit(101325, 'N/m^2').toNumber('lbf/in^2'), 14.6959487763741);
    });

    it ('should convert a unit with fixed prefix to a number', function () {
      var u1 = new Unit(5000, 'cm');
      var u2 = u1.to('km');
      approxjs(u2.toNumber('mm'), 50000);

      var u1 = new Unit(981, 'cm/s^2');
      var u2 = u1.to('km/ms^2');
      approxjs(u2.toNumber('m/s^2'), 9.81);
    });

    it ('should convert a unit with fraction to a number', function () {
      var u = new Unit(indexjs.fraction(5), 'cm');
  ...
  describe('toNumeric', function() {
    it ('should convert a unit to a numeric value', function () {
      var u = new Unit(indexjs.fraction(1,3), 'cm');
      assert.deepEqual(u.toNumeric('mm'), indexjs.fraction(10,3));
    });
  });
  ...
  describe('to', function() {
    ...
    it ('should convert a unit with a fraction', function () {
      var u1 = new Unit(indexjs.fraction(1,3), 'm');

      var u2 = u1.to('cm');
      assert.deepEqual(u2.value, indexjs.fraction(1,3));
    ...
    it ('should convert a Complex unit', function() {
      var u1 = new Unit(indexjs.complex(300,400), 'kPa');
      var u2 = u1.to('lbf/in^2');
      approxjs.deepEqual(u2.value, indexjs.complex(300000, 400000));
  ...
  describe('toString', function() {
    ...
    it('should convert a unit with Fraction to string properly', function() {
      assert.equal(new Unit(indexjs.fraction(9/10), 'mm').toString(), '9/10 mm');
    });

    it('should convert a Complex unit to string properly', function() {
      assert.equal(new Unit(indexjs.complex(-1,-2), 'J / (mol K)').toString(), '(-1 - 2i) J / (mol K)');
    });
  });
  ...
  describe('simplifyUnitListLazy', function() {
    ...
    it('should only simplify units with values', function() {
      ...
      unit1 = indexjs.multiply(unit1, 1);
    ...
    it('should simplify units resulting from multiply/divide/power functions only when formatting for output', function() {
      ...
      var unit3 = indexjs.multiply(unit1, unit2);
    ...
    it('should simplify units when they cancel out with {predictable: true}', function() {
      var origConfig = indexjs.config();
      indexjs.config({predictable: true});
      ...
      var nounit = indexjs.eval('40m * 40N / (40J)');
      assert.equal(nounit.toString(), "40");
      assert.equal(nounit.units.length, 0);

      var a = indexjs.unit('3 s^-1');
      var b = indexjs.unit('4 s');
      assert.equal(indexjs.multiply(a, b).type, 'Unit');

      var c = indexjs.unit('8.314 J / mol / K');
      assert.equal(indexjs.pow(c, 0).type, 'Unit');

      var d = indexjs.unit('60 minute');
      var e = indexjs.unit('1 s');
      assert.equal(indexjs.divide(d, e).type, 'Unit');

      indexjs.config(origConfig);
    })

    it('should convert units to appropriate _numeric_ values when they cancel out with {predictable: false}', function() {
      var origConfig = indexjs.config();
      indexjs.config({predictable: false});

      assert.equal(typeof(indexjs.eval('40 m * 40 N / (40 J)')), 'number');

      var bigunit = indexjs.unit(indexjs.bignumber(1), 'km');
      var smallunit = indexjs.unit(indexjs.bignumber(3000000), 'mm');
      var verybignumber = indexjs.divide(bigunit, smallunit);
      ...
      bigunit = indexjs.unit(indexjs.fraction(1), 'km');
      smallunit = indexjs.unit(indexjs.fraction(3000000), 'mm');
      verybignumber = indexjs.divide(bigunit, smallunit);
      assert.equal(verybignumber.type, 'Fraction');
      assert.equal(verybignumber.toFraction(), '1/3');

      var a = indexjs.unit('3 s^-1');
      var b = indexjs.unit('4 s');
      assert.equal(typeof(indexjs.multiply(a, b)), 'number');

      var c = indexjs.unit('8.314 J / mol / K');
      assert.equal(typeof(indexjs.pow(c, 0)), 'number');

      var d = indexjs.unit('60 minute');
      var e = indexjs.unit('1 s');
      assert.equal(typeof(indexjs.divide(d, e)), 'number');
  ...
  describe('json', function () {
    ...
    it('toJSON', function () {
      ...
      assert.deepEqual(new Unit(indexjs.fraction(0.375), 'cm').toJSON(),
          {
            mathjs: 'Unit',
            value: indexjs.fraction(0.375), // Note that value is not serialized at this point, that will be done by JSON.stringify
            unit: 'cm',
            fixPrefix: false
          });
      approxjs.deepEqual(new Unit(indexjs.complex(2, 4), 'g').toJSON(),
          {
            mathjs: 'Unit',
            value: indexjs.complex(2, 4),
            unit: 'g',
            fixPrefix: false
          });

      var str = JSON.stringify(new Unit(indexjs.fraction(0.375), 'cm'));
      assert.deepEqual(str, '{"mathjs":"Unit","value":{"mathjs":"Fraction","n":3,"d":8},"unit":"cm","fixPrefix":false}');

      var cmpx = JSON.stringify(new Unit(indexjs.complex(2, 4), 'g'));
    ...
    it('fromJSON', function () {
      ...
      var u7 = Unit.fromJSON({
        mathjs: 'Unit',
        value: indexjs.fraction(0.375), // Note that value is already a Fraction at this point, that will be done by JSON.parse(str, reviver)
        unit: 'cm',
        fixPrefix: false
      });
      assert.deepEqual(u7, new Unit(indexjs.fraction(0.375), 'cm'))
      ...
      assert.deepEqual(u8, new Unit(indexjs.complex(2,4), 'g'));
    });
  ...
  describe('format', function () {
    ...
    it('should format a unit with a bignumber', function() {
      assert.equal(new Unit(indexjs.bignumber(1).plus(1e-24), 'm').format(), '1.000000000000000000000001 m');
      assert.equal(new Unit(indexjs.bignumber(1e24).plus(1), 'm').format(), '1.000000000000000000000001 Ym');
    });

    it('should format a unit with a fraction', function() {
      assert.equal(new Unit(indexjs.fraction(4/5), 'm').format(), '4/5 m');
    });

    it('should format a Complex unit', function() {
      assert.equal(new Unit(indexjs.complex(-2, 4.5), 'mm').format(14), '(-2 + 4.5i) mm');
    });

    it('should format units with VA and VAR correctly', function() {
      assert.equal(indexjs.eval('4000 VAR + 3000 VA').format(), "(3 + 4i) kVA");
      assert.equal(indexjs.eval('3000 VA + 4000 VAR').format(), "(3 + 4i) kVA");
      assert.equal(indexjs.eval('4000 VAR').format(), "(4) kVAR");
      assert.equal(indexjs.eval('4000i VA').format(), "(4) kVAR");
      assert.equal(indexjs.eval('4000i VAR').format(), "(-4) kVA");
      assert.equal(indexjs.eval('abs(4000 VAR + 3000 VA)').format(), "5 kW");
      assert.equal(indexjs.eval('abs(3000 VA + 4000 VAR)').format(), "5 kW");
      assert.equal(indexjs.eval('abs(4000 VAR)').format(), "4 kW");
      assert.equal(indexjs.eval('abs(4000i VA)').format(), "4 kW");
      assert.equal(indexjs.eval('abs(4000i VAR)').format(), "4 kW");
    });
  ...
  describe('parse', function() {
    ...
    it('should parse the value of the unit as Fraction or BigNumber when math.js is configured so', function() {
      ...
      indexjs.config({number: 'Fraction'});
      var unit1 = Unit.parse('5kg');
      assert(unit1.value.isFraction);

      indexjs.config({number: 'BigNumber'});
      var unit1 = Unit.parse('5kg');
  ...
  describe('multiply, divide, and pow', function() {
    ...
    it('should keep the same numeric type for the units value', function() {
      var unit1 = new Unit(indexjs.bignumber(10), "N/s");
      var unit2 = new Unit(indexjs.bignumber(10), "h");
      var unitM = unit1.multiply(unit2);
      assert(unitM.value.isBigNumber);

      var unit3 = new Unit(indexjs.bignumber(14.7), "lbf");
      var unit4 = new Unit(indexjs.bignumber(1), "in in");
      var unitD = unit3.divide(unit4);
      assert(unitD.value.isBigNumber);

      var unit5 = new Unit(indexjs.bignumber(1), "N h/s");
      var unitP = unit5.pow(indexjs.bignumber(-3.5));
  ...
  describe('aliases', function() {
    it('should support aliases', function () {
      ...
      assert.equal(indexjs.eval('2 feet * 8 s').toString(), '16 feet s');
      assert.equal(indexjs.eval('2 s * 8 feet').toString(), '16 s feet');
    });
  });
  ...
  describe('createUnitSingle', function() {
    ...
    it('should create a custom unit from a configuration object', function() {
      ...
      assert.equal(indexjs.eval('8000 rad^2/s').toString(), '1 kilowiggle');
    });
    ...
    it('should create and use a new base if no matching base exists', function() {
      ...
      assert.equal(indexjs.eval('4 mile^5/minute').format(4), '240 jabberwocky');
    });
  });
  ...
  describe('createUnit', function() {
    it('should create multiple units', function() {
      ...
      assert.equal(indexjs.eval('2 foo3 to foo1').toString(), '8 foo1');
    });

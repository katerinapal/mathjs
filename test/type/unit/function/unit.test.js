...
describe('unit', function() {
  ...
  it('should clone a unit', function() {
    var a = indexjs.unit('5cm');
    var b = indexjs.unit(a);
    assert.deepEqual(b.toString(), '50 mm');
  });

  it('should create units from all elements in an array', function() {
    assert.deepEqual(indexjs.unit(['5 cm', '3kg']), [indexjs.unit('5cm'), indexjs.unit('3kg')]);
  });

  it('should create units from all elements in an array', function() {
    assert.deepEqual(indexjs.unit(indexjs.matrix(['5 cm', '3kg'])), indexjs.matrix([indexjs.unit('5cm'), indexjs.unit('3kg')]));
  });
  ...
  it('should throw an error if called with a complex', function() {
    assert.throws(function () {unit(indexjs.complex(2,3))}, TypeError);
  });
  ...
  it('should take a bignumber as the quantity and a string as the unit', function() {
    assert.deepEqual(unit(indexjs.bignumber(5).plus(1e-24), 'cm').toString(), '50.00000000000000000000001 mm');
  });

  it('should take a fraction as the quantity and a string as the unit', function() {
    assert.deepEqual(unit(indexjs.fraction(1,3), 'cm').toString(), '10/3 mm');
  });
  ...
  it('should throw an error if called with an invalid argument', function() {
    assert.throws(function () {unit(2, indexjs.complex(2,3))}, TypeError);
  ...
  it('should LaTeX unit', function () {
    var expr1 = indexjs.parse('unit(cm)');
    var expr2 = indexjs.parse('unit(1,cm)');

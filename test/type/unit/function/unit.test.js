...
describe('unit', function() {
  ...
  it('should clone a unit', function() {
    var a = index_indexjsjs.unit('5cm');
    var b = index_indexjsjs.unit(a);
    assert.deepEqual(b.toString(), '50 mm');
  });

  it('should create units from all elements in an array', function() {
    assert.deepEqual(index_indexjsjs.unit(['5 cm', '3kg']), [index_indexjsjs.unit('5cm'), index_indexjsjs.unit('3kg')]);
  });

  it('should create units from all elements in an array', function() {
    assert.deepEqual(index_indexjsjs.unit(index_indexjsjs.matrix(['5 cm', '3kg'])), index_indexjsjs.matrix([index_indexjsjs.unit('5cm'), index_indexjsjs.unit('3kg')]));
  });
  ...
  it('should throw an error if called with a complex', function() {
    assert.throws(function () {unit(index_indexjsjs.complex(2,3))}, TypeError);
  });
  ...
  it('should take a bignumber as the quantity and a string as the unit', function() {
    assert.deepEqual(unit(index_indexjsjs.bignumber(5).plus(1e-24), 'cm').toString(), '50.00000000000000000000001 mm');
  });

  it('should take a fraction as the quantity and a string as the unit', function() {
    assert.deepEqual(unit(index_indexjsjs.fraction(1,3), 'cm').toString(), '10/3 mm');
  });
  ...
  it('should throw an error if called with an invalid argument', function() {
    assert.throws(function () {unit(2, index_indexjsjs.complex(2,3))}, TypeError);
  ...
  it('should LaTeX unit', function () {
    var expr1 = index_indexjsjs.parse('unit(cm)');
    var expr2 = index_indexjsjs.parse('unit(1,cm)');

...
var ImmutableDenseMatrix = index.type.ImmutableDenseMatrix;
...
describe('Index', function () {
  ...
  it('should create an Index from a Matrix', function () {
    assert.deepEqual(new Index(index.matrix([0, 10]))._dimensions, [new ImmutableDenseMatrix([0, 10])]);
  });
  ...
  it('should calculate the size of an Index', function () {
    ...
    assert.deepEqual(new Index(index.matrix([1, 2, 3])).size(), [3]);
  ...
  it('should have a property isIndex', function () {
    var a = new index.type.Index([2, 5]);
  ...
  it('should test whether index contains an object property', function () {
    assert.strictEqual(new index.type.Index(2, 3).isObjectProperty(), false);
    assert.strictEqual(new index.type.Index([2, 5]).isObjectProperty(), false);
    assert.strictEqual(new index.type.Index('foo', 'bar').isObjectProperty(), false);
    assert.strictEqual(new index.type.Index('foo').isObjectProperty(), true);
  });

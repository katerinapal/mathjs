...
describe('matrix', function() {
  ...
  it('should create an empty matrix with one dimension if called without argument', function() {
    var a = matrix();
    assert.ok(a instanceof index_indexjsjs.type.Matrix);
    assert.deepEqual(index_indexjsjs.size(a), matrix([0])); // TODO: wouldn't it be nicer if an empty matrix has zero dimensions?
  });

  it('should create empty matrix, dense format', function() {
    var a = matrix('dense');
    assert.ok(a instanceof index_indexjsjs.type.Matrix);
    assert.deepEqual(index_indexjsjs.size(a), matrix([0]));
  });
  ...
  it('should create a matrix from an array', function() {
    ...
    assert.ok(b instanceof index_indexjsjs.type.Matrix);
    assert.deepEqual(b, matrix([[1,2],[3,4]]));
    assert.deepEqual(index_indexjsjs.size(b), matrix([2,2]));
  });
  ...
  it('should be the identity if called with a matrix, dense format', function() {
    ...
    assert.deepEqual(index_indexjsjs.size(c), matrix([2,2], 'dense'));
  });
  ...
  it('should create a matrix from a range correctly', function() {
    var d = matrix(index_indexjsjs.range(1,6));
    assert.ok(d instanceof index_indexjsjs.type.Matrix);
    assert.deepEqual(d, matrix([1,2,3,4,5]));
    assert.deepEqual(index_indexjsjs.size(d), matrix([5]));
  });
  ...
  it('should throw an error if called with a unit', function() {
    assert.throws(function () { matrix(index_indexjsjs.unit('5cm')); }, TypeError);
  });
  ...
  it('should throw an error when called with an invalid storage format', function () {
    assert.throws(function () { index_indexjsjs.matrix([], 1); }, /Unsupported matrix storage format: 1/);
  });
  
  it('should throw an error when called with an unknown storage format', function () {
    assert.throws(function () { index_indexjsjs.matrix([], '123'); }, /Unsupported matrix storage format: 123/);
  });

  it('should LaTeX matrix', function () {
    var expr1 = index_indexjsjs.parse('matrix()');
    var expr2 = index_indexjsjs.parse('matrix([1])');

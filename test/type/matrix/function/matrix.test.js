...
describe('matrix', function () {
  ...
  it('should create an empty matrix with one dimension if called without argument', function () {
    var a = matrix();
    _assert2.default.ok(a instanceof _index.indexjs.type.Matrix);
    _assert2.default.deepEqual(_index.indexjs.size(a), matrix([0])); // TODO: wouldn't it be nicer if an empty matrix has zero dimensions?
  });

  it('should create empty matrix, dense format', function () {
    var a = matrix('dense');
    _assert2.default.ok(a instanceof _index.indexjs.type.Matrix);
    _assert2.default.deepEqual(_index.indexjs.size(a), matrix([0]));
  });
  ...
  it('should create a matrix from an array', function () {
    ...
    _assert2.default.ok(b instanceof _index.indexjs.type.Matrix);
    _assert2.default.deepEqual(b, matrix([[1, 2], [3, 4]]));
    _assert2.default.deepEqual(_index.indexjs.size(b), matrix([2, 2]));
  });
  ...
  it('should be the identity if called with a matrix, dense format', function () {
    ...
    _assert2.default.deepEqual(_index.indexjs.size(c), matrix([2, 2], 'dense'));
  });
  ...
  it('should create a matrix from a range correctly', function () {
    var d = matrix(_index.indexjs.range(1, 6));
    _assert2.default.ok(d instanceof _index.indexjs.type.Matrix);
    _assert2.default.deepEqual(d, matrix([1, 2, 3, 4, 5]));
    _assert2.default.deepEqual(_index.indexjs.size(d), matrix([5]));
  });
  ...
  it('should throw an error if called with a unit', function () {
    _assert2.default.throws(function () {
      matrix(_index.indexjs.unit('5cm'));
    }, TypeError);
  });

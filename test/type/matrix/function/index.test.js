...
var assert = {},
    math = indexjs,
    Range = indexjs.type.Range,
    ImmutableDenseMatrix = indexjs.type.ImmutableDenseMatrix;

describe('index', function() {

  it('should create an index', function() {
    var index = indexjs.index(new Range(2,6));
    assert.ok(index instanceof indexjs.type.Index);
    assert.deepEqual(index._dimensions, [{start:2, end:6, step:1}]);

    var index2 = indexjs.index(new Range(0,4), new Range(5,2,-1));
    assert.ok(index2 instanceof indexjs.type.Index);
  ...
  it('should create an index from bignumbers (downgrades to numbers)', function() {
    var index = indexjs.index(new Range(indexjs.bignumber(2), indexjs.bignumber(6)), indexjs.bignumber(3));
    assert.ok(index instanceof indexjs.type.Index);
    assert.deepEqual(index._dimensions, [new Range(2, 6, 1), new ImmutableDenseMatrix([3])]);
  });

  it('should LaTeX index', function () {
    var expr1 = indexjs.parse('index(1)');
    var expr2 = indexjs.parse('index(1,2)');
    var expr3 = indexjs.parse('index(1,2,3)');

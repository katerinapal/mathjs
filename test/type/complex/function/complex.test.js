...
describe('complex', function() {
  ...
  it('should return 0 + 0i if called with no argument', function() {
    assert.deepEqual(complex(), new index_obj.type.Complex(0, 0));
    assert.ok(complex() instanceof index_obj.type.Complex);
  });

  it('should parse a valid string and create the complex number accordingly', function() {
    assert.deepEqual(complex('2+3i'), new index_obj.type.Complex(2, 3));
    assert.deepEqual(complex('2-3i'), new index_obj.type.Complex(2, -3));
    assert.ok(complex('2+3i') instanceof index_obj.type.Complex);
  });

  it('should convert a real number into a complex value', function() {
    assert.deepEqual(complex(123), new index_obj.type.Complex(123, 0));
  });

  it('should convert a fraction into a complex value', function() {
    assert.deepEqual(complex(index_obj.fraction(123)), new index_obj.type.Complex(123, 0));
  });

  it('should convert a big number into a complex value (downgrades to number', function() {
    assert.deepEqual(complex(index_obj.bignumber(123)), new index_obj.type.Complex(123, 0));
    assert.deepEqual(complex(index_obj.bignumber(2), index_obj.bignumber(3)), new index_obj.type.Complex(2, 3));
  });

  it('should clone a complex value', function() {
    var b = complex(complex(2,3));
    assert.deepEqual(b, new index_obj.type.Complex(2,3));
  });

  it('should convert the elements of a matrix or array to complex values', function() {
    var result = [
      new index_obj.type.Complex(2, 0),
      new index_obj.type.Complex(1, 0),
      new index_obj.type.Complex(2, 3)
    ];
    assert.deepEqual(complex(index_obj.matrix([2, 1, complex(2, 3)])), index_obj.matrix(result));
  ...
  it('should accept polar coordinates as input', function() {
    ...
    assert.deepEqual(polar, new index_obj.type.Complex.fromPolar(1, 1));
  });

  it('should accept an object with im and re as keys', function() {
    assert.deepEqual(complex({re: 1, im: 2}), new index_obj.type.Complex(1, 2));
  });
  ...
  it('should create a complex value from a boolean', function() {
    assert.deepEqual(complex(true), new index_obj.type.Complex(1, 0));
  });

  it('should throw an error if called with a unit', function() {
    assert.throws(function () {complex(index_obj.unit('5cm'))}, /Error: Expected object with either properties re and im, or properties r and phi./);
  });

  it('should accept two numbers as arguments', function() {
    assert.deepEqual(complex(2, 3), new index_obj.type.Complex(2, 3));
    assert.deepEqual(complex(2, -3), new index_obj.type.Complex(2, -3));
    assert.deepEqual(complex(-2, 3), new index_obj.type.Complex(-2, 3));
    assert.ok(complex(2, 3) instanceof index_obj.type.Complex);
  });
  ...
  it('should LaTeX complex', function () {
    var expr1 = index_obj.parse('complex()');
    var expr2 = index_obj.parse('complex(1)');
    var expr3 = index_obj.parse('complex(1,2)');

...
describe('complex', function() {
  ...
  it('should return 0 + 0i if called with no argument', function() {
    assert.deepEqual(complex(), new indexjs.type.Complex(0, 0));
    assert.ok(complex() instanceof indexjs.type.Complex);
  });

  it('should parse a valid string and create the complex number accordingly', function() {
    assert.deepEqual(complex('2+3i'), new indexjs.type.Complex(2, 3));
    assert.deepEqual(complex('2-3i'), new indexjs.type.Complex(2, -3));
    assert.ok(complex('2+3i') instanceof indexjs.type.Complex);
  });

  it('should convert a real number into a complex value', function() {
    assert.deepEqual(complex(123), new indexjs.type.Complex(123, 0));
  });

  it('should convert a fraction into a complex value', function() {
    assert.deepEqual(complex(indexjs.fraction(123)), new indexjs.type.Complex(123, 0));
  });

  it('should convert a big number into a complex value (downgrades to number', function() {
    assert.deepEqual(complex(indexjs.bignumber(123)), new indexjs.type.Complex(123, 0));
    assert.deepEqual(complex(indexjs.bignumber(2), indexjs.bignumber(3)), new indexjs.type.Complex(2, 3));
  });

  it('should clone a complex value', function() {
    var b = complex(complex(2,3));
    assert.deepEqual(b, new indexjs.type.Complex(2,3));
  });

  it('should convert the elements of a matrix or array to complex values', function() {
    var result = [
      new indexjs.type.Complex(2, 0),
      new indexjs.type.Complex(1, 0),
      new indexjs.type.Complex(2, 3)
    ];
    assert.deepEqual(complex(indexjs.matrix([2, 1, complex(2, 3)])), indexjs.matrix(result));
  ...
  it('should accept polar coordinates as input', function() {
    ...
    assert.deepEqual(polar, new indexjs.type.Complex.fromPolar(1, 1));
  });

  it('should accept an object with im and re as keys', function() {
    assert.deepEqual(complex({re: 1, im: 2}), new indexjs.type.Complex(1, 2));
  });
  ...
  it('should create a complex value from a boolean', function() {
    assert.deepEqual(complex(true), new indexjs.type.Complex(1, 0));
  });

  it('should throw an error if called with a unit', function() {
    assert.throws(function () {complex(indexjs.unit('5cm'))}, /Error: Expected object with either properties re and im, or properties r and phi./);
  });

  it('should accept two numbers as arguments', function() {
    assert.deepEqual(complex(2, 3), new indexjs.type.Complex(2, 3));
    assert.deepEqual(complex(2, -3), new indexjs.type.Complex(2, -3));
    assert.deepEqual(complex(-2, 3), new indexjs.type.Complex(-2, 3));
    assert.ok(complex(2, 3) instanceof indexjs.type.Complex);
  });
  ...
  it('should LaTeX complex', function () {
    var expr1 = indexjs.parse('complex()');
    var expr2 = indexjs.parse('complex(1)');
    var expr3 = indexjs.parse('complex(1,2)');

...
function format(value) {
  return index_obj.format(value, {
    fn: function (value) {
      if (typeof value === 'number') {
        // round numbers
        return index_obj.format(value, PRECISION);
      }
      else {
        return index_obj.format(value);
      }
...
function completer (text) {
  ...
  if (m) {
    ...
    for (var func in index_obj) {
      if (index_obj.hasOwnProperty(func)) {
    ...
    switch (expr.toLowerCase()) {
      ...
      default:
        ...
        switch (mode) {
          case 'eval':
            ...
            try {
              ...
              if (node) {
                ...
                else if (res instanceof index_obj.type.Help) {
          ...
          case 'string':
            try {
              var string = index_obj.parse(expr).toString({parenthesis: parenthesis});
              console.log(string);
            }
            catch (err) {
              console.log(err.toString());
          ...
          case 'tex':
            try {
              var tex = index_obj.parse(expr).toTex({parenthesis: parenthesis});
              console.log(tex);
            }
            catch (err) {
              console.log(err.toString());

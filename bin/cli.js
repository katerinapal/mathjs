...
function format(value) {
  return index.format(value, {
    fn: function (value) {
      if (typeof value === 'number') {
        // round numbers
        return index.format(value, PRECISION);
      }
      else {
        return index.format(value);
      }
...
function completer (text) {
  ...
  if (m) {
    ...
    for (var func in index) {
      if (index.hasOwnProperty(func)) {
    ...
    switch (expr.toLowerCase()) {
      ...
      default:
        ...
        switch (mode) {
          case 'eval':
            ...
            try {
              ...
              if (node) {
                ...
                else if (res instanceof index.type.Help) {
          ...
          case 'string':
            try {
              var string = index.parse(expr).toString({parenthesis: parenthesis});
              console.log(string);
            }
            catch (err) {
              console.log(err.toString());
          ...
          case 'tex':
            try {
              var tex = index.parse(expr).toTex({parenthesis: parenthesis});
              console.log(tex);
            }
            catch (err) {
              console.log(err.toString());

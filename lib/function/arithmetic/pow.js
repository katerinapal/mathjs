...
import * as matrixeye_obj from "../matrix/eye";
import * as multiply_obj from "./multiply";
import * as typematrixfunctionmatrix_obj from "../../type/matrix/function/matrix";
import * as typefractionfunctionfraction_obj from "../../type/fraction/function/fraction";
...
function factory (type, config, load, typed) {
  ...
  function _pow(x, y) {
    ...
    if (config.predictable && !utilsnumber_isIntegerjs(y) && x < 0) {
    ...
    if (utilsnumber_isIntegerjs(y) || x >= 0 || config.predictable) {
      return Math.pow(x, y);
    }
    else {
  ...
  function _powArray(x, y) {
    if (!utilsnumber_isIntegerjs(y) || y < 0) {
      throw new TypeError('For A^b, b must be a positive integer (value is ' + y + ')');
    }
    // verify that A is a 2 dimensional square matrix
    var s = utilsarray_sizejs(x);
    if (s.length != 2) {
  ...
  pow.toTex = {
    2: '\\left(${args[0]}\\right)' + utilslatex_operatorsjs['pow'] + '{${args[1]}}'
  };

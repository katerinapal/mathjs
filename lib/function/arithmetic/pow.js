...
function factory (type, config, load, typed) {
  ...
  function _pow(x, y) {
    ...
    if (config.predictable && !utilsnumber_obj(y) && x < 0) {
    ...
    if (utilsnumber_obj(y) || x >= 0 || config.predictable) {
      return Math.pow(x, y);
    }
    else {
  ...
  function _powArray(x, y) {
    if (!utilsnumber_obj(y) || y < 0) {
      throw new TypeError('For A^b, b must be a positive integer (value is ' + y + ')');
    }
    // verify that A is a 2 dimensional square matrix
    var s = utilsarray_obj(x);
    if (s.length != 2) {
  ...
  pow.toTex = {
    2: '\\left(${args[0]}\\right)' + utilslatex_obj.operators['pow'] + '{${args[1]}}'
  };

...
import * as eyejs from "../matrix/eye";
import * as multiplyjs from "./multiply";
import * as matrixjs from "../../type/matrix/function/matrix";
import * as fractionjs from "../../type/fraction/function/fraction";
...
function factory (type, config, load, typed) {
  ...
  var fraction = load(fractionjs);
  var number = load(numberjs);
  ...
  function _pow(x, y) {
    ...
    if (config.predictable && !numberjs(y) && x < 0) {
    ...
    if (numberjs(y) || x >= 0 || config.predictable) {
      return Math.pow(x, y);
    }
    else {
  ...
  pow.toTex = {
    2: '\\left(${args[0]}\\right)' + latexjs.operators['pow'] + '{${args[1]}}'
  };

...
var _eye = require("../matrix/eye");

var matrixeye_obj = _interopRequireWildcard(_eye);
...
var typefractionfunctionfraction_obj = _interopRequireWildcard(_fraction);

var _number2 = require("../../type/number");

var typenumber_obj = _interopRequireWildcard(_number2);
...
function factory(type, config, load, typed) {
  var eye = load(matrixeye_obj);
  var multiply = load(multiply_obj);
  var matrix = load(typematrixfunctionmatrix_obj);
  var fraction = load(typefractionfunctionfraction_obj);
  var number = load(typenumber_obj);
  ...
  function _pow(x, y) {
    ...
    if (config.predictable && !_number.isNumber && x < 0) {
    ...
    if (_number.isNumber || x >= 0 || config.predictable) {
      return Math.pow(x, y);
    } else {
  ...
  pow.toTex = {
    2: '\\left(${args[0]}\\right)' + _latex.operators['pow'] + '{${args[1]}}'
  };

  return pow;
}

var name_exportedObj = 'pow';

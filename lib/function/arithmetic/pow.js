...
import * as matrixeye from "../matrix/eye";
import * as multiply from "./multiply";
import * as typematrixfunctionmatrix from "../../type/matrix/function/matrix";
import * as typefractionfunctionfraction from "../../type/fraction/function/fraction";
import * as typenumber from "../../type/number";
'use strict';

var isInteger = utilsnumber.isInteger;
var size = utilsarray.size;

function factory (type, config, load, typed) {
  var latex = utilslatex;
  var eye = load(matrixeye);
  var multiply = load(multiply);
  var matrix = load(typematrixfunctionmatrix);
  var fraction = load(typefractionfunctionfraction);
  var number = load(typenumber);
  ...
  function _pow(x, y) {
    ...
    if (config.predictable && !utilsnumber(y) && x < 0) {
    ...
    if (utilsnumber(y) || x >= 0 || config.predictable) {
      return Math.pow(x, y);
    }
    else {
  ...
  pow.toTex = {
    2: '\\left(${args[0]}\\right)' + utilslatex.operators['pow'] + '{${args[1]}}'
  };

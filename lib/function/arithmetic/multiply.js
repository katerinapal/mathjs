...
function factory (type, config, load, typed) {
   ...
   */
  var multiply = typed('multiply', utilsobject_extendjs({
    // we extend the signatures of multiplyScalar with signatures dealing with matrices

    'Array, Array': function (x, y) {
      // check dimensions
      _validateMatrixDimensions(utilsarray_sizejs(x), utilsarray_sizejs(y));
  ...
  multiply.toTex = {
    2: '\\left(${args[0]}' + utilslatex_operatorsjs['multiply'] + '${args[1]}\\right)'
  };

  return multiply;
}

var name_name = 'multiply';

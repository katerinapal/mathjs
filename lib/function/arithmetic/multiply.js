...
import * as multiplyScalar from "./multiplyScalar";
...
function factory (type, config, load, typed) {
  ...
  var multiplyScalar = load(multiplyScalar);
   ...
   */
  var multiply = typed('multiply', utilsobject({
    // we extend the signatures of multiplyScalar with signatures dealing with matrices

    'Array, Array': function (x, y) {
      // check dimensions
      _validateMatrixDimensions(utilsarray.size(x), utilsarray.size(y));
  ...
  multiply.toTex = {
    2: '\\left(${args[0]}' + utilslatex.operators['multiply'] + '${args[1]}\\right)'
  };

  return multiply;
}

var name_exportedObj = 'multiply';

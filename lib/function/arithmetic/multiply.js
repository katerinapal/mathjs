...
import * as multiplyScalar_obj from "./multiplyScalar";
...
function factory (type, config, load, typed) {
   ...
   */
  var multiply = typed('multiply', utilsobject_obj({
    // we extend the signatures of multiplyScalar with signatures dealing with matrices

    'Array, Array': function (x, y) {
      // check dimensions
      _validateMatrixDimensions(utilsarray_obj.size(x), utilsarray_obj.size(y));
  ...
  multiply.toTex = {
    2: '\\left(${args[0]}' + utilslatex_obj.operators['multiply'] + '${args[1]}\\right)'
  };

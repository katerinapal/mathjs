...
function factory(type, config, load, typed) {
   ...
   */
  var multiply = typed('multiply', (0, _object.extend)({
    // we extend the signatures of multiplyScalar with signatures dealing with matrices

    'Array, Array': function ArrayArray(x, y) {
      // check dimensions
      _validateMatrixDimensions(_array.size.size(x), _array.size.size(y));
  ...
  multiply.toTex = {
    2: '\\left(${args[0]}' + _latex.operators['multiply'] + '${args[1]}\\right)'
  };

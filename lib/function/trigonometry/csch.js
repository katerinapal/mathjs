...
import { sign as numberjs } from "../../utils/number";
'use strict';

var deepMap = deepMap_deepMapjs;
var sign = numberjs.sign;
...
function factory (type, config, load, typed) {
  ...
  var csch = typed('csch', {
    ...
    'Array | Matrix': function (x) {
      return deepMap_deepMapjs(x, csch);
    }
  });
...
function _csch(x) {
  ...
  if (x == 0) {
    ...
    return Math.abs(2 / (Math.exp(x) - Math.exp(-x))) * numberjs(x);
  }
}

var name_exportedObj = 'csch';

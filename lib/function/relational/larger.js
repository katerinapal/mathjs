...
var nearlyEqual = utilsnumber_obj.nearlyEqual;
...
function factory (type, config, load, typed) {
  ...
  var larger = typed('larger', {
    ...
    'number, number': function (x, y) {
      return x > y && !utilsnumber_obj(x, y, config.epsilon);
    },

    'BigNumber, BigNumber': function (x, y) {
      return x.gt(y) && !utilsbignumbernearlyEqual_bigNearlyEqual(x, y, config.epsilon);
    },
  ...
  larger.toTex = {
    2: '\\left(${args[0]}' + utilslatex_obj.operators['larger'] + '${args[1]}\\right)'
  };

  return larger;
}

var name_exportedObj = 'larger';

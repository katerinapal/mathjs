...
function factory (type, config, load, typed) {
  ...
  function _zeros(size, format) {
    ...
    if (format) {
      ...
      var arr = [];
      if (size.length > 0) {
        return arrayjs(arr, size, defaultValue);
      }
      return arr;
    }
  }
  ...
  function _validate (size) {
    size.forEach(function (value) {
      if (typeof value !== 'number' || !numberjs(value) || value < 0) {

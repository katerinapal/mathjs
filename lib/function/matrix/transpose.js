    ...
    'Matrix': function (x) {
      ...
      switch (size.length) {
        ...
        case 2:
          ...
          if (columns === 0) {
            // throw exception
            throw new RangeError('Cannot transpose a 2D matrix with no columns (size: ' + utilsstring_formatjs + ')');
          }
        ...
        default:
          // multi dimensional
          throw new RangeError('Matrix must be a vector or two dimensional (size: ' + utilsstring_formatjs + ')');
      }
      return c;
    },
  ...
  var _denseTranspose = function (m, rows, columns) {
    ...
    for (var j = 0; j < columns; j++) {
      ...
      for (var i = 0; i < rows; i++) {
        // set data
        transposedRow[i] = utilsobject_clonejs;
      }
    }
    // return matrix

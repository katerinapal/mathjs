...
function factory (type, config, load, typed) {
    ...
    'Matrix': function (x) {
      ...
      switch (size.length) {
        ...
        case 2:
          ...
          if (columns === 0) {
            // throw exception
            throw new RangeError('Cannot transpose a 2D matrix with no columns (size: ' + utilsstring_formatjs(size) + ')');
          }
        ...
        default:
          // multi dimensional
          throw new RangeError('Matrix must be a vector or two dimensional (size: ' + utilsstring_formatjs(this._size) + ')');
      }
      return c;
    },
  ...
  var _denseTranspose = function (m, rows, columns) {
    ...
    for (var j = 0; j < columns; j++) {
      ...
      for (var i = 0; i < rows; i++) {
        // set data
        transposedRow[i] = utilsobject_clonejs(data[i][j]);
      }
    }
    // return matrix
  ...
  var _sparseTranspose = function (m, rows, columns) {
    ...
    for (j = 0; j < columns; j++) {
      ...
      for (var k0 = ptr[j], k1 = ptr[j + 1], k = k0; k < k1; k++) {
        ...
        if (values)
          cvalues[q] = utilsobject_clonejs(values[k]);
      }
    }
    // return matrix
  ...
  transpose.toTex = {1: '\\left(${args[0]}\\right)' + utilslatex_operatorsjs['transpose']};

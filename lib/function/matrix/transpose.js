...
function factory (type, config, load, typed) {
    ...
    'Matrix': function (x) {
      ...
      switch (size.length) {
        ...
        case 2:
          ...
          if (columns === 0) {
            // throw exception
            throw new RangeError('Cannot transpose a 2D matrix with no columns (size: ' + stringjs(size) + ')');
          }
        ...
        default:
          // multi dimensional
          throw new RangeError('Matrix must be a vector or two dimensional (size: ' + stringjs(this._size) + ')');
      }
      return c;
    },
  ...
  var _denseTranspose = function (m, rows, columns) {
    ...
    for (var j = 0; j < columns; j++) {
      ...
      for (var i = 0; i < rows; i++) {
        // set data
        transposedRow[i] = objectjs(data[i][j]);
      }
    }
    // return matrix
  ...
  transpose.toTex = {1: '\\left(${args[0]}\\right)' + latexjs.operators['transpose']};

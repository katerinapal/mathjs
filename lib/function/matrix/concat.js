      ...
      for (i = 0; i < len; i++) {
        ...
        if (typeof arg === 'number' || (arg && arg.isBigNumber === true)) {
          ...
          if (dim < 0 || (i > 0 && dim > prevDim)) {
            ...
            throw new IndexError_IndexErrorjs(dim, prevDim + 1);
          }
        }
        else {
          // this is a matrix or array
          var m = objectjs(arg).valueOf();
          var size = arrayjs.size(m);
          matrices[i] = m;
          prevDim = dim;
          dim = size.length - 1;
          ...
          if (i > 0 && dim != prevDim) {
            throw new DimensionError_DimensionErrorjs(prevDim + 1, dim + 1);
          }
        }
      }
...
function _concat(a, b, concatDim, dim) {
  if (dim < concatDim) {
    ...
    if (a.length != b.length) {
      throw new DimensionError_DimensionErrorjs(a.length, b.length);
    }

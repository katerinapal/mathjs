    ...
    '...Array | Matrix | number | BigNumber': function (args) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (typeof arg === 'number' || (arg && arg.isBigNumber === true)) {
          ...
          if (!utilsnumber_obj(dim)) {
            throw new TypeError('Integer number expected for dimension');
          }

          if (dim < 0 || (i > 0 && dim > prevDim)) {
            // TODO: would be more clear when throwing a DimensionError here
            throw new errorIndexError_IndexError(dim, prevDim + 1);
          }
        }
        else {
          // this is a matrix or array
          var m = utilsobject_obj(arg).valueOf();
          var size = utilsarray_obj.size(m);
          matrices[i] = m;
          prevDim = dim;
          dim = size.length - 1;
          ...
          if (i > 0 && dim != prevDim) {
            throw new errorDimensionError_DimensionError(prevDim + 1, dim + 1);
          }
        }
      }
...
function _concat(a, b, concatDim, dim) {
  if (dim < concatDim) {
    ...
    if (a.length != b.length) {
      throw new errorDimensionError_DimensionError(a.length, b.length);
    }

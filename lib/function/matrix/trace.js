  ...
  var _denseTrace = function (m) {
    ...
    switch (size.length) {
      case 1:
        ...
        if (size[0] == 1) {
          // return data[0]
          return utilsobject_clonejs(data[0]);
        }
        throw new RangeError('Matrix must be square (size: ' + utilsstring_formatjs(size) + ')');
      case 2:
        // two dimensional
        var rows = size[0];
        var cols = size[1];
      ...
      default:
        // multi dimensional
        throw new RangeError('Matrix must be two dimensional (size: ' + utilsstring_formatjs(size) + ')');
    }
  };

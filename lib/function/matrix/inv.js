...
var _unaryMinus = require("../arithmetic/unaryMinus");

var arithmeticunaryMinus_obj = _interopRequireWildcard(_unaryMinus);

var _det = require("../matrix/det");

var matrixdet_obj = _interopRequireWildcard(_det);
...
function factory(type, config, load, typed) {
  ...
  var addScalar = load(arithmeticaddScalar_obj);
  var multiply = load(arithmeticmultiply_obj);
  var unaryMinus = load(arithmeticunaryMinus_obj);
  var det = load(matrixdet_obj);
  var eye = load(eye_obj);
  ...
  var inv = typed('inv', {
    'Array | Matrix': function ArrayMatrix(x) {
      var size = x.isMatrix === true ? x.size() : utilsindex_indexjsjs.array.size(x);
      switch (size.length) {
        case 1:
          // vector
          if (size[0] == 1) {
            ...
            throw new RangeError('Matrix must be square ' + '(size: ' + utilsindex_indexjsjs.string.format(size) + ')');
          }
        ...
        default:
          ...
          throw new RangeError('Matrix must be two dimensional ' + '(size: ' + utilsindex_indexjsjs.string.format(size) + ')');
      }
    },

...
function factory(type, config, load, typed) {
  ...
  var inv = typed('inv', {
    'Array | Matrix': function ArrayMatrix(x) {
      var size = x.isMatrix === true ? x.size() : _index.array.size(x);
      switch (size.length) {
        case 1:
          // vector
          if (size[0] == 1) {
            ...
            throw new RangeError('Matrix must be square ' + '(size: ' + _index.array.format(size) + ')');
          }
        ...
        default:
          ...
          throw new RangeError('Matrix must be two dimensional ' + '(size: ' + _index.array.format(size) + ')');
      }
    },

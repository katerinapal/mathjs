...
import { validateIndex as utilsarray_validateIndexjs } from "../../utils/array";
...
function factory (type, config, load, typed) {
  ...
  var subset = typed('subset', {
    ...
    'Array, Index, any': function (value, index, replacement) {
      return matrix(utilsobject_clonejs(value))
  ...
  function _getSubstring(str, index) {
    ...
    if (index.size().length != 1) {
      throw new errorDimensionError_DimensionErrorjs(index.size().length, 1);
    }

    // validate whether the range is out of range
    var strLen = str.length;
    utilsarray_validateIndexjs(index.min()[0], strLen);
    utilsarray_validateIndexjs(index.max()[0], strLen);
  ...
  function _setSubstring(str, index, replacement, defaultValue) {
    ...
    if (len != replacement.length) {
      throw new errorDimensionError_DimensionErrorjs(range.size()[0], replacement.length);
    }

    // validate whether the range is out of range
    var strLen = str.length;
    utilsarray_validateIndexjs(index.min()[0]);
    utilsarray_validateIndexjs(index.max()[0]);
...
function _getObjectProperty (object, index) {
  if (index.size().length !== 1) {
    throw new errorDimensionError_DimensionErrorjs(index.size(), 1);
  }
...
function _setObjectProperty (object, index, replacement) {
  ...
  var updated = utilsobject_clonejs(object);

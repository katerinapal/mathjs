...
import { validateIndex as arrayjs } from "../../utils/array";
...
var validateIndex = arrayjs.validateIndex;
...
function factory (type, config, load, typed) {
  ...
  function _getSubstring(str, index) {
    ...
    if (index.size().length != 1) {
      throw new DimensionError_DimensionErrorjs(index.size().length, 1);
    }

    // validate whether the range is out of range
    var strLen = str.length;
    arrayjs(index.min()[0], strLen);
    arrayjs(index.max()[0], strLen);
  ...
  function _setSubstring(str, index, replacement, defaultValue) {
    ...
    if (len != replacement.length) {
      throw new DimensionError_DimensionErrorjs(range.size()[0], replacement.length);
    }
...
function _getObjectProperty (object, index) {
  if (index.size().length !== 1) {
    throw new DimensionError_DimensionErrorjs(index.size(), 1);
  }

...
function factory(type, config, load, typed) {
  ...
  var resize = function resize(x, size, defaultValue) {
    ...
    if (size.length == 0) {
      ...
      var res = (0, _array.size)(x, size, defaultValue);
  ...
  function _resizeString(str, size, defaultChar) {
    ...
    var len = size[0];
    if (typeof len !== 'number' || !_number.isInteger) {
      throw new TypeError('Invalid size, must contain positive integers ' + '(size: ' + _string.format + ')');
    }

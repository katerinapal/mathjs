...
import { resize as utilsarray_resizejs } from "../../utils/array";
...
function factory (type, config, load, typed) {
  ...
  var resize = function resize (x, size, defaultValue) {
    ...
    if (size.length == 0) {
      ...
      x = utilsobject_clonejs(x);

      var res = utilsarray_resizejs.resize(x, size, defaultValue);
  ...
  function _resizeString(str, size, defaultChar) {
    ...
    var len = size[0];
    if (typeof len !== 'number' || !utilsnumber_isIntegerjs(len)) {
      throw new TypeError('Invalid size, must contain positive integers ' +
          '(size: ' + utilsstring_formatjs(size) + ')');
    }

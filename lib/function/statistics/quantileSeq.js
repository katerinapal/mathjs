...
import { isNumber as utilsnumber_isNumberjs } from "../../utils/number";
...
import * as matrixpartitionSelect_obj from "../matrix/partitionSelect";
import * as relationalcompare_obj from "../relational/compare";
'use strict';
...
function factory (type, config, load, typed) {
  ...
  function quantileSeq(data, probOrN, sorted) {
    ...
    if (utilscollectionisCollection_isCollectionjsjs(data)) {
      sorted = sorted || false;
      if (typeof sorted === 'boolean') {
        dataArr = data.valueOf();
        if (utilsnumber_isNumberjs(probOrN)) {
          if (probOrN < 0) {
          ...
          if (probOrN > 1) {
            ...
            if (!utilsnumber_isIntegerjs(probOrN)) {
        ...
        if (Array.isArray(probOrN)) {
          ...
          for (var i = 0; i < probArr.length; ++i) {
            ...
            if (utilsnumber_isNumberjs(currProb)) {
  ...
  function _quantileSeq(array, prob, sorted) {
    var flat = utilsarray_flattenjs(array);
    var len = flat.length;
    if (len === 0) {
      throw new Error('Cannot calculate quantile of an empty sequence');
    }

    if (utilsnumber_isNumberjs(prob)) {

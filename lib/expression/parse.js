...
import * as AccessorNodejs from "./node/AccessorNode";
import * as ArrayNodejs from "./node/ArrayNode";
import * as AssignmentNodejs from "./node/AssignmentNode";
import * as BlockNodejs from "./node/BlockNode";
import * as ConditionalNodejs from "./node/ConditionalNode";
import * as ConstantNodejs from "./node/ConstantNode";
import * as FunctionAssignmentNodejs from "./node/FunctionAssignmentNode";
import * as IndexNodejs from "./node/IndexNode";
import * as ObjectNodejs from "./node/ObjectNode";
import * as OperatorNodejs from "./node/OperatorNode";
import * as ParenthesisNodejs from "./node/ParenthesisNode";
import * as FunctionNodejs from "./node/FunctionNode";
import * as RangeNodejs from "./node/RangeNode";
import * as SymbolNodejs from "./node/SymbolNode";
'use strict';
...
function factory (type, config, load, typed) {
  var AccessorNode            = load(AccessorNodejs);
  var ArrayNode               = load(ArrayNodejs);
  var AssignmentNode          = load(AssignmentNodejs);
  var BlockNode               = load(BlockNodejs);
  var ConditionalNode         = load(ConditionalNodejs);
  var ConstantNode            = load(ConstantNodejs);
  var FunctionAssignmentNode  = load(FunctionAssignmentNodejs);
  var IndexNode               = load(IndexNodejs);
  var ObjectNode              = load(ObjectNodejs);
  var OperatorNode            = load(OperatorNodejs);
  var ParenthesisNode         = load(ParenthesisNodejs);
  var FunctionNode            = load(FunctionNodejs);
  var RangeNode               = load(RangeNodejs);
  var SymbolNode              = load(SymbolNodejs);
  ...
  function parse (expr, options) {
    if (arguments.length != 1 && arguments.length != 2) {
      throw new ArgumentsError_ArgumentsErrorjs('parse', arguments.length, 1, 2);
    }
    ...
    if (typeof expr === 'string') {
      ...
      return deepMap_deepMapjs(expr, function (elem) {

...
function factory (type, config, load, typed) {
  ...
  function calculateNecessaryParentheses(node, parenthesis) {
    ...
    var startPrecedence = operators_obj.getPrecedence(node.start, parenthesis);
    parens.start = ((startPrecedence !== null) && (startPrecedence <= precedence))
      || (parenthesis === 'all');

    if (node.step) {
      var stepPrecedence = operators_obj.getPrecedence(node.step, parenthesis);
      parens.step = ((stepPrecedence !== null) && (stepPrecedence <= precedence))
        || (parenthesis === 'all');
    }

    var endPrecedence = operators_obj.getPrecedence(node.end, parenthesis);
...
var name_exportedObj = 'RangeNode';

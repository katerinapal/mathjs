...
function factory (type, config, load, typed) {
  ...
  function calculateNecessaryParentheses(node, parenthesis) {
    ...
    var startPrecedence = operators_getPrecedencejs(node.start, parenthesis);
    parens.start = ((startPrecedence !== null) && (startPrecedence <= precedence))
      || (parenthesis === 'all');

    if (node.step) {
      var stepPrecedence = operators_getPrecedencejs(node.step, parenthesis);
      parens.step = ((stepPrecedence !== null) && (stepPrecedence <= precedence))
        || (parenthesis === 'all');
    }

    var endPrecedence = operators_getPrecedencejs(node.end, parenthesis);

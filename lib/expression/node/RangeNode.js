...
function factory (type, config, load, typed) {
  ...
  function calculateNecessaryParentheses(node, parenthesis) {
    ...
    var startPrecedence = operatorsjs.getPrecedence(node.start, parenthesis);
    parens.start = ((startPrecedence !== null) && (startPrecedence <= precedence))
      || (parenthesis === 'all');

    if (node.step) {
      var stepPrecedence = operatorsjs.getPrecedence(node.step, parenthesis);
      parens.step = ((stepPrecedence !== null) && (stepPrecedence <= precedence))
        || (parenthesis === 'all');
    }

    var endPrecedence = operatorsjs.getPrecedence(node.end, parenthesis);

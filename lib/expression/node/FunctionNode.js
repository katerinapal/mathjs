...
function factory (type, config, load, typed, math) {
  ...
  function expandTemplate(template, node, options) {
    ...
    while ((match = regex.exec(template)) !== null) {   //go through all matches
      ...
      utilslatex_obj += template.substring(inputPos, match.index);
      ...
      if (match[0] === '$$') { // escaped dollar sign
        ...
        if (match[2] === undefined) { //no square brackets
          switch (typeof property) {
            case 'string':
              utilslatex_obj += property;
              break;
            case 'object':
              if (property.isNode) {
                utilslatex_obj += property.toTex(options);
              }
              else if (Array.isArray(property)) {
                //make array of Nodes into comma separated list
                utilslatex_obj += property.map(function (arg, index) {
                  if (arg && arg.isNode) {
          ...
          if (property[match[2]] && property[match[2]].isNode) {
            utilslatex_obj += property[match[2]].toTex(options);
          }
          else {
    ...
    utilslatex_obj += template.slice(inputPos);  //append rest of the template

    return utilslatex_obj;
  }
  ...
  FunctionNode.prototype._toTex = function (options) {
    ...
    return expandTemplate(utilslatex_obj.defaultTemplate, this, options);
  };
...
var name_exportedObj = 'FunctionNode';

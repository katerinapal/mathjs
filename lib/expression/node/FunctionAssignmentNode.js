...
function factory (type, config, load, typed) {
  ...
  function FunctionAssignmentNode(name, params, expr) {
    ...
    if (name in keywords_obj) throw new Error('Illegal function name, "' + name + '" is a reserved keyword');
  ...
  function needParenthesis(node, parenthesis) {
    ...
    var exprPrecedence = operators_obj.getPrecedence(node.expr, parenthesis);
  ...
  FunctionAssignmentNode.prototype._toTex = function (options) {
    ...
    return '\\mathrm{' + this.name
        + '}\\left(' + this.params.map(utilslatex_obj.toSymbol).join(',') + '\\right):=' + expr;
  };

...
function factory (type, config, load, typed, math) {
  var Node         = load(Node);
  var ConstantNode = load(ConstantNode);
  var SymbolNode   = load(SymbolNode);
  var FunctionNode = load(FunctionNode);
  ...
  function calculateNecessaryParentheses(root, parenthesis, args, latex) {
    ...
    switch (args.length) {
      ...
      case 1: //unary operators
        ...
        if (utilslatex && (operandPrecedence !== null)) {
          ...
          if (operators[precedence][rootIdentifier].latexLeftParens === false) {
            return [false];
          }

          if (operators[operandPrecedence][operandIdentifier].latexParens === false) {
            return [false];
          }
        }
      ...
      case 2: //binary operators
        ...
        if (utilslatex) {
          ...
          if (lhsPrecedence !== null) {
            ...
            if (operators[lhsPrecedence][lhsIdentifier].latexParens === false) {
              lhsParens = false;
            }
          }

          if (rhsPrecedence !== null) {
            if (operators[precedence][rootIdentifier].latexRightParens === false) {
              rhsParens = false;
            }

            if (operators[rhsPrecedence][rhsIdentifier].latexParens === false) {
              rhsParens = false;
            }
          }
        }
  ...
  OperatorNode.prototype._toTex = function (options) {
    ...
    var op = utilslatex.operators[this.fn];

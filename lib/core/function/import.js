...
import { traverse as utilsobject_traversejs } from "../../utils/object";
...
function factory (type, config, load, typed, math) {
  ...
  function math_import(object, options) {
    ...
    if (num != 1 && num != 2) {
      throw new errorArgumentsError_ArgumentsErrorjs('import', num, 1, 2);
    }
    ...
    if (utilsobject_isFactoryjs(object)) {
    ...
    // TODO: allow a typed-function with name too
      ...
      for (var name in object) {
        if (object.hasOwnProperty(name)) {
          ...
          else if (utilsobject_isFactoryjs(object)) {
            _importFactory(object, options);
          }
          else {
  ...
  function _importFactory(factory, options) {
    if (typeof factory.name === 'string') {
      ...
      var namespace = factory.path ? utilsobject_traversejs(math, factory.path) : math;
      ...
      if (factory.lazy !== false) {
        utilsobject_lazyjs(namespace, name, resolver);
      }
      else {

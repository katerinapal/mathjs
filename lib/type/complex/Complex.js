...
import { format as utilsnumber_formatjs } from "../../utils/number";
...
function factory (type, config, load, typed, math) {
  ...
  Complex.prototype.format = function (options) {
    ...
    var im = this.im;
    var re = this.re;
    var strRe = utilsnumber_formatjs(this.re, options);
    var strIm = utilsnumber_formatjs(this.im, options);

    // round either re or im when smaller than the configured precision
    var precision = utilsnumber_isNumberjs(options) ? options : options ? options.precision : null;
    if (precision !== null) {
  ...
  Complex.fromPolar = function (args) {
    switch (arguments.length) {
      ...
      case 2:
        var r = arguments[0],
            phi = arguments[1];
        if (utilsnumber_isNumberjs(r)) {
          if (phi && phi.isUnit && phi.hasBase('ANGLE')) {
            // convert unit to a number in radians
            phi = phi.toNumber('rad');
          }

          if (utilsnumber_isNumberjs(phi)) {

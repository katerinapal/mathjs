import { endsWith as stringjs } from "../../utils/string";
import { clone as objectjs } from "../../utils/object";
import { constants as constantsjs } from "../../utils/bignumber/constants";
import * as addScalarjs from "../../function/arithmetic/addScalar";
import * as subtractjs from "../../function/arithmetic/subtract";
import * as multiplyScalarjs from "../../function/arithmetic/multiplyScalar";
import * as divideScalarjs from "../../function/arithmetic/divideScalar";
...
import * as formatjs from "../../function/string/format";
import * as typeofjs from "../../function/utils/typeof";
import * as numberjs from "../../type/number";
import * as Complexjs from "../../type/complex/Complex";
import util_moduleDefault from "util";
'use strict';

var endsWith = stringjs.endsWith;
var clone = objectjs.clone;
var constants = constantsjs;

function factory (type, config, load, typed, math) {
  var add       = load(addScalarjs);
  var subtract  = load(subtractjs);
  var multiply  = load(multiplyScalarjs);
  var divide    = load(divideScalarjs);
  var pow       = load(powjs);
  var abs       = load(absjs);
  var fix       = load(fixjs);
  var equal     = load(equaljs);
  var isNumeric = load(isNumericjs);
  var format    = load(formatjs);
  var getTypeOf = load(typeofjs);
  var toNumber  = load(numberjs);
  var Complex   = load(Complexjs);
  ...
  Unit.prototype.clone = function () {
    ...
    unit.value = objectjs(this.value);
  ...
  function calculateAngleValues (config) {
    if (config.number === 'BigNumber') {
      var pi = constantsjs.pi(type.BigNumber);

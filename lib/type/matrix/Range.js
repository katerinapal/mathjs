...
var utilsnumber_numberjsjs = _interopRequireWildcard(_number);
...
function factory(type, config, load, typed) {
  ...
  Range.prototype.size = function () {
    ...
    if (utilsnumber_numberjsjs.sign(step) == utilsnumber_numberjsjs.sign(diff)) {
  ...
  Range.prototype.format = function (options) {
    var str = utilsnumber_numberjsjs.format(this.start, options);

    if (this.step != 1) {
      str += ':' + utilsnumber_numberjsjs.format(this.step, options);
    }
    str += ':' + utilsnumber_numberjsjs.format(this.end, options);
    return str;
  };

...
function factory (type, config, load, typed) {
  ...
  Range.prototype.size = function () {
    ...
    if (numberjs.sign(step) == numberjs.sign(diff)) {
  ...
  Range.prototype.format = function (options) {
    var str = numberjs.format(this.start, options);

    if (this.step != 1) {
      str += ':' + numberjs.format(this.step, options);
    }
    str += ':' + numberjs.format(this.end, options);
    return str;
  };

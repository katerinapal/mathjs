...
function factory (type, config, load, typed) {
  ...
  Range.prototype.size = function () {
    ...
    if (utilsnumber.sign(step) == utilsnumber.sign(diff)) {
  ...
  Range.prototype.format = function (options) {
    var str = utilsnumber.format(this.start, options);

    if (this.step != 1) {
      str += ':' + utilsnumber.format(this.step, options);
    }
    str += ':' + utilsnumber.format(this.end, options);
    return str;
  };

import { util as indexjs } from "../../utils/index";
import { DimensionError as DimensionError_DimensionErrorjs } from "../../error/DimensionError";
import * as Matrixjs from "./Matrix";
import * as equalScalarjs from "../../function/relational/equalScalar";
'use strict';
...
var array = indexjs;
var object = indexjs.object;
var string = indexjs.string;
var number = indexjs.number;
...
function factory (type, config, load, typed) {
  var Matrix = load(Matrixjs); // force loading Matrix (do not use via type.Matrix)
  ...
  function SparseMatrix(data, datatype) {
    ...
    if (data && data.isMatrix === true) {
      ...
      // unsupported type
      throw new TypeError('Unsupported type of data (' + indexjs.types.type(data) + ')');
    }
    else {
      // nothing provided
      this._values = [];
  ...
  var _getsubset = function (matrix, idx) {
    ...
    if (size.length != matrix._size.length) {
      throw new DimensionError_DimensionErrorjs(size.length, matrix._size.length);
    }
  ...
  var _setsubset = function (matrix, index, submatrix, defaultValue) {
    ...
    if (isScalar) {
      ...
      if (iSize.length !== 1 && iSize.length !== 2) {
        throw new DimensionError_DimensionErrorjs(iSize.length, matrix._size.length, '<');
      }
      ...
      if (!object.deepEqual(iSize, sSize)) {
        throw new DimensionError_DimensionErrorjs(iSize, sSize, '>');
      }
  ...
  SparseMatrix.prototype.get = function (index) {
    ...
    if (index.length != this._size.length)
      throw new DimensionError_DimensionErrorjs(index.length, this._size.length);

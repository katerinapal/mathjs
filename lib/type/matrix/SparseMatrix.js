import { utilsindex_obj } from "../../utils/index";
import { DimensionError as errorDimensionError_DimensionError } from "../../error/DimensionError";
import * as Matrix_obj from "./Matrix";
import * as functionrelationalequalScalar_obj from "../../function/relational/equalScalar";
'use strict';

var util = utilsindex_obj;
var DimensionError = errorDimensionError_DimensionError;

var array = utilsindex_obj;
var object = utilsindex_obj.object;
var string = utilsindex_obj.string;
var number = utilsindex_obj.number;
...
function factory (type, config, load, typed) {
  var Matrix = load(Matrix_obj); // force loading Matrix (do not use via type.Matrix)
  var equalScalar = load(functionrelationalequalScalar_obj);
  ...
  function SparseMatrix(data, datatype) {
    ...
    if (data && data.isMatrix === true) {
      ...
      // unsupported type
      throw new TypeError('Unsupported type of data (' + utilsindex_obj.types.type(data) + ')');
    }
    else {
      // nothing provided
      this._values = [];
  ...
  var _getsubset = function (matrix, idx) {
    ...
    if (size.length != matrix._size.length) {
      throw new errorDimensionError_DimensionError(size.length, matrix._size.length);
    }
  ...
  var _setsubset = function (matrix, index, submatrix, defaultValue) {
    ...
    if (isScalar) {
      ...
      if (iSize.length !== 1 && iSize.length !== 2) {
        throw new errorDimensionError_DimensionError(iSize.length, matrix._size.length, '<');
      }
      ...
      if (!object.deepEqual(iSize, sSize)) {
        throw new errorDimensionError_DimensionError(iSize, sSize, '>');
      }
  ...
  SparseMatrix.prototype.get = function (index) {
    ...
    if (index.length != this._size.length)
      throw new errorDimensionError_DimensionError(index.length, this._size.length);
...
var name_exportedObj = 'SparseMatrix';

import { array as utilsindex_arrayjs } from "../../utils/index";
import { DimensionError as errorDimensionError_DimensionErrorjs } from "../../error/DimensionError";
import * as Matrix_obj from "./Matrix";
import { factory as functionrelationalequalScalar_obj } from "../../function/relational/equalScalar";
'use strict';

var array = utilsindex_arrayjs;
var object = utilsindex_arrayjs;
var string = utilsindex_arrayjs;
var number = utilsindex_arrayjs;
...
function factory (type, config, load, typed) {
  ...
  function SparseMatrix(data, datatype) {
    ...
    if (data && data.isMatrix === true) {
      ...
      // unsupported type
      throw new TypeError('Unsupported type of data (' + utilsindex_arrayjs.type(data) + ')');
    }
    else {
      // nothing provided
      this._values = [];
  ...
  var _getsubset = function (matrix, idx) {
    ...
    if (size.length != matrix._size.length) {
      throw new errorDimensionError_DimensionErrorjs(size.length, matrix._size.length);
    }
  ...
  var _setsubset = function (matrix, index, submatrix, defaultValue) {
    ...
    if (isScalar) {
      ...
      if (iSize.length !== 1 && iSize.length !== 2) {
        throw new errorDimensionError_DimensionErrorjs(iSize.length, matrix._size.length, '<');
      }
      ...
      if (!object.deepEqual(iSize, sSize)) {
        throw new errorDimensionError_DimensionErrorjs(iSize, sSize, '>');
      }
  ...
  SparseMatrix.prototype.get = function (index) {
    ...
    if (index.length != this._size.length)
      throw new errorDimensionError_DimensionErrorjs(index.length, this._size.length);

...
var string = string_obj;
var object = object_obj;
var types = types_obj;
...
function _validate(array, size, dim) {
  ...
  if (len != size[dim]) {
    throw new errorDimensionError_DimensionError(len, size[dim]);
  }
  ...
  if (dim < size.length - 1) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (!Array.isArray(child)) {
        throw new errorDimensionError_DimensionError(size.length - 1, size.length, '<');
      }
...
var validate_exportedObj = function(array, size) {
  ...
  if (isScalar) {
    ...
    if (Array.isArray(array)) {
      throw new errorDimensionError_DimensionError(array.length, 0);
    }
  }
  else {
    // array
...
var validateIndex_exportedObj = function(index, length) {
  if (!number_obj.isNumber(index) || !number_obj.isInteger(index)) {
    throw new TypeError('Index must be an integer (value: ' + index + ')');
  }
  if (index < 0 || (typeof length === 'number' && index >= length)) {
    throw new errorIndexError_IndexError(index, length);
  }
};
...
var resize_exportedObj = function(array, size, defaultValue) {
  ...
  size.forEach(function (value) {
    if (!number_obj.isNumber(value) || !number_obj.isInteger(value) || value < 0) {
      throw new TypeError('Invalid size, must contain positive integers ' +
          '(size: ' + string_obj.format(size) + ')');
    }
  });
